...
module Balance_event = struct
  type t =
    ...
    | Balance_snapshot of
        { venue: Venue.t
        ; balances: (string * float * float * float) list
...
let create_envelope ?(sequence = 0L) event =
  {id= Event_id.create (); timestamp= Time.now (); sequence; event}

(** Serialize envelope to binary string *)
let serialize envelope = Bin_prot.Writer.to_string bin_writer_envelope envelope

(** Deserialize envelope from binary string *)
let deserialize data =
  try Ok (Bin_prot.Reader.of_string bin_reader_envelope data) with
...
let describe = function
  ...
  | System (Bot_started {bot_id; _}) -> sprintf "START %s" bot_id
  | System (Bot_stopped {reason; _}) -> sprintf "STOP: %s" reason
  | System (Connection_state_changed {venue; new_state; _}) ->
    sprintf
      "conn %s -> %s"
      (Venue.to_string venue)
      (Sexp.to_string (Connection_state.sexp_of_t new_state))
  | System (Error {message; is_fatal; _}) ->
    sprintf
      ...
      (match is_fatal with
       ...
       | false -> "error")
      message
  | System (Heartbeat {sequence}) -> sprintf "heartbeat #%Ld" sequence
  | Strategy (Signal_generated {signal_type; reason; _}) ->
    sprintf
      "signal %s: %s"
      (Sexp.to_string (Strategy_event.sexp_of_signal_type signal_type))
      reason
  | Strategy (Position_changed {symbol; new_qty; _}) ->
    sprintf "pos %s -> %.8f" symbol new_qty
  | Strategy (Pnl_update {symbol; total; _}) -> sprintf "pnl %s %.2f" symbol total
