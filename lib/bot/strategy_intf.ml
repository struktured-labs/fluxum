...
module Signal = struct
  ...
  let cancel ~order_id = Cancel (Cancel_request.Cancel_order {order_id})
  let cancel_all ?symbol ?venue () = Cancel (Cancel_request.Cancel_all {symbol; venue})
end
...
module Book_snapshot = struct
  ...
  let spread_bps t =
    ...
    | Some mid, Some s when Float.(mid > 0.) -> Some (s /. mid *. 10000.)
    | _ -> None
end
...
module Context = struct
  ...
  let position t ~symbol =
    List.Assoc.find t.positions ~equal:String.equal symbol |> Option.value ~default:0.

  let balance t ~currency =
    List.Assoc.find t.balances ~equal:String.equal currency |> Option.value ~default:0.
...
module type S = sig
  ...
  (** Called on timer tick
      Returns list of signals and new state *)
  val on_tick : state -> time:Event.Time.t -> context:Context.t -> Signal.t list * state
