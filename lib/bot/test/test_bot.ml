...
let%expect_test "event serialization - system bot_started" =
  let event =
    Bot.Event.System
      (Bot.Event.System_event.Bot_started
         {bot_id= "test-bot"; config_json= "{\"key\": \"value\"}"; version= "1.0.0"})
  in
...
let%expect_test "event serialization - system bot_stopped" =
  let event =
    Bot.Event.System
      (Bot.Event.System_event.Bot_stopped {reason= "user request"; exit_code= Some 0})
  in
  let envelope = Bot.Event.create_envelope ~sequence:1L event in
  let serialized = Bot.Event.serialize envelope in
...
let%expect_test "event serialization - system connection_state_changed" =
  let event =
    Bot.Event.System
      (Bot.Event.System_event.Connection_state_changed
...
let%expect_test "event serialization - system fatal error" =
  let event =
    Bot.Event.System
      (Bot.Event.System_event.Error {venue= None; message= "Out of memory"; is_fatal= true})
  in
  let envelope = Bot.Event.create_envelope ~sequence:1L event in
    (match Bot.Event.deserialize (Bot.Event.serialize envelope) with
     | Ok env -> printf "Description: %s\n" (Bot.Event.describe env.event)
     | Error _ -> printf "Error\n");
    [%expect
      {|
...
let%expect_test "event serialization - order submitted" =
  let event =
    Bot.Event.Order
      (Bot.Event.Order_event.Order_submitted
         { order_id= "ord-123"
         ; symbol= "ETHUSD"
...
let%expect_test "event serialization - order accepted" =
  let event =
    Bot.Event.Order
      (Bot.Event.Order_event.Order_accepted
         {order_id= "ord-123"; exchange_id= "exch-456"; venue= Bot.Event.Venue.Binance})
  in
  let envelope = Bot.Event.create_envelope ~sequence:1L event in
    (match Bot.Event.deserialize (Bot.Event.serialize envelope) with
     | Ok env ->
...
let%expect_test "event serialization - order partially filled" =
  let event =
    Bot.Event.Order
      (Bot.Event.Order_event.Order_partially_filled
         { order_id= "ord-123"
         ; venue= Bot.Event.Venue.Kraken
         ; fill_qty= 0.3
         ; fill_price= 49000.0
...
let%expect_test "event serialization - order cancelled" =
  let event =
    Bot.Event.Order
      (Bot.Event.Order_event.Order_cancelled
         {order_id= "ord-123"; venue= Bot.Event.Venue.Mexc; reason= "user requested"})
  in
  let envelope = Bot.Event.create_envelope ~sequence:1L event in
    (match Bot.Event.deserialize (Bot.Event.serialize envelope) with
     | Ok env ->
...
let%expect_test "event serialization - order rejected" =
  let event =
    Bot.Event.Order
      (Bot.Event.Order_event.Order_rejected
         { order_id= "ord-123"
...
let%expect_test "event serialization - market book_update snapshot" =
  let event =
    Bot.Event.Market
      (Bot.Event.Market_event.Book_update
         { symbol= "BTCUSD"
         ; venue= Bot.Event.Venue.Gemini
...
let%expect_test "event serialization - balance update" =
  let event =
    Bot.Event.Balance
      (Bot.Event.Balance_event.Balance_update
         { venue= Bot.Event.Venue.Gemini
         ; currency= "USD"
         ; available= 10000.0
         ; locked= 500.0
...
let%expect_test "event serialization - balance snapshot" =
  let event =
    Bot.Event.Balance
      (Bot.Event.Balance_event.Balance_snapshot
         { venue= Bot.Event.Venue.Kraken
...
let%expect_test "event serialization - strategy signal_generated" =
  let event =
    Bot.Event.Strategy
      (Bot.Event.Strategy_event.Signal_generated
...
let%expect_test "event serialization - strategy position_changed" =
  let event =
    Bot.Event.Strategy
      (Bot.Event.Strategy_event.Position_changed
         { symbol= "BTCUSD"
         ; venue= Bot.Event.Venue.Gemini
         ; old_qty= 0.0
         ; new_qty= 0.5
...
let%expect_test "event serialization - strategy pnl_update" =
  let event =
    Bot.Event.Strategy
      (Bot.Event.Strategy_event.Pnl_update
         { symbol= "BTCUSD"
         ; venue= Bot.Event.Venue.Kraken
         ; realized= 100.0
         ; unrealized= 50.0
...
let%expect_test "state apply_event - bot stopped" =
  ...
  in
  let state =
    Bot.State.apply_event state (Bot.Event.create_envelope ~sequence:1L start_event)
  in
  let stop_event =
    Bot.Event.System
      (Bot.Event.System_event.Bot_stopped {reason= "shutdown"; exit_code= Some 0})
  in
  let state =
    Bot.State.apply_event state (Bot.Event.create_envelope ~sequence:2L stop_event)
  in
    printf "Is Running: %b\n" state.is_running;
    printf "Event Count: %d\n" state.event_count;
    [%expect
...
let%expect_test "state apply_event - connection state changed" =
  ...
  in
  let state =
    Bot.State.apply_event state (Bot.Event.create_envelope ~sequence:1L event)
  in
...
let%expect_test "state apply_event - error tracking" =
  ...
  in
  let state =
    Bot.State.apply_event state (Bot.Event.create_envelope ~sequence:1L event1)
  in
    printf "Error Count: %d\n" state.error_count;
    ...
    let event2 =
      Bot.Event.System
        (Bot.Event.System_event.Error {venue= None; message= "Fatal error"; is_fatal= true})
    in
    let state =
      Bot.State.apply_event state (Bot.Event.create_envelope ~sequence:2L event2)
    in
...
let%expect_test "state apply_event - order lifecycle" =
  ...
  in
  let state =
    Bot.State.apply_event state (Bot.Event.create_envelope ~sequence:1L submit)
  in
    ...
    in
    let state =
      Bot.State.apply_event state (Bot.Event.create_envelope ~sequence:2L accept)
    in
    let order = Map.find_exn state.active_orders "ord-1" in
      printf
        "After accept - Exchange ID: %s\n"
      ...
      in
      let state =
        Bot.State.apply_event state (Bot.Event.create_envelope ~sequence:3L partial)
      in
      let order = Map.find_exn state.active_orders "ord-1" in
        printf
          "After partial - Filled: %.1f, Remaining: %.1f\n"
          order.filled_qty
          order.remaining_qty;
        (* Full fill *)
        let fill =
        ...
        in
        let state =
          Bot.State.apply_event state (Bot.Event.create_envelope ~sequence:4L fill)
        in
...
let%expect_test "state apply_event - order cancelled" =
  ...
  in
  let state =
    Bot.State.apply_event state (Bot.Event.create_envelope ~sequence:2L cancel)
  in
...
let%expect_test "state apply_event - order rejected" =
  ...
  in
  let state =
    Bot.State.apply_event state (Bot.Event.create_envelope ~sequence:2L reject)
  in
...
let%expect_test "state helper functions" =
  ...
  in
  let state =
    Bot.State.apply_event state (Bot.Event.create_envelope ~sequence:1L submit1)
  in
  let state =
    Bot.State.apply_event state (Bot.Event.create_envelope ~sequence:2L submit2)
  in
  let state =
    Bot.State.apply_event state (Bot.Event.create_envelope ~sequence:3L submit3)
  in
...
let%expect_test "state summary" =
  ...
  let summary = Bot.State.summary state in
    printf
      "Summary contains bot_id: %b\n"
      (String.is_substring summary ~substring:"test-bot");
    printf
      "Summary contains running: %b\n"
...
let%expect_test "strategy signal - buy_market" =
  let signal =
    Bot.Strategy_intf.Signal.buy_market
      ~symbol:"BTCUSD"
      ~venue:Bot.Event.Venue.Gemini
      ~qty:0.1
  in
...
let%expect_test "strategy signal - sell_market" =
  let signal =
    Bot.Strategy_intf.Signal.sell_market
      ~symbol:"BTCUSD"
      ~venue:Bot.Event.Venue.Gemini
      ~qty:0.1
  in
...
let%expect_test "strategy signal - buy_limit" =
  let signal =
    Bot.Strategy_intf.Signal.buy_limit
      ~symbol:"BTCUSD"
      ~venue:Bot.Event.Venue.Gemini
      ~qty:0.1
      ~price:50000.0
...
let%expect_test "strategy signal - sell_limit" =
  let signal =
    Bot.Strategy_intf.Signal.sell_limit
      ~symbol:"BTCUSD"
      ~venue:Bot.Event.Venue.Gemini
      ~qty:0.1
      ~price:51000.0
...
let%expect_test "strategy signal - cancel" =
  let signal = Bot.Strategy_intf.Signal.cancel ~order_id:"ord-123" in
    (match signal with
     | Bot.Strategy_intf.Signal.Cancel
         (Bot.Strategy_intf.Cancel_request.Cancel_order {order_id}) ->
...
let%expect_test "strategy signal - cancel_all" =
  let signal = Bot.Strategy_intf.Signal.cancel_all ~symbol:"BTCUSD" () in
    (match signal with
     | Bot.Strategy_intf.Signal.Cancel
         (Bot.Strategy_intf.Cancel_request.Cancel_all {symbol; venue}) ->
       printf
         "Cancel all - Symbol: %s, Venue: %s\n"
...
let%expect_test "book snapshot - empty book" =
  ...
  in
    printf
      "Mid price: %s\n"
      (Option.value_map
         (Bot.Strategy_intf.Book_snapshot.mid_price book)
         ~default:"none"
         ~f:(sprintf "%.2f"));
    printf
      "Spread: %s\n"
      (Option.value_map
         (Bot.Strategy_intf.Book_snapshot.spread book)
         ~default:"none"
         ~f:(sprintf "%.2f"));
    printf
      "Best bid: %s\n"
      (Option.value_map
         (Bot.Strategy_intf.Book_snapshot.best_bid book)
         ~default:"none"
         ~f:(fun l -> sprintf "%.2f" l.price));
    printf
      "Best ask: %s\n"
      (Option.value_map
         (Bot.Strategy_intf.Book_snapshot.best_ask book)
         ~default:"none"
         ~f:(fun l -> sprintf "%.2f" l.price));
    [%expect
      {|
...
let%expect_test "noop strategy" =
  ...
  let ctx : Bot.Strategy_intf.Context.t =
    ...
    ; unrealized_pnl= 0.0 }
  in
  let signals, _new_state =
    Bot.Strategy_intf.Noop.on_book_update state ~book ~context:ctx
  in
...
let%expect_test "ledger entry apply_fill - single buy" =
  let entry = Bot.Entry.empty ~symbol:"BTCUSD" ~venue:Bot.Entry.Venue.Gemini in
  let entry =
    Bot.Entry.apply_fill entry ~price:50000.0 ~qty:0.1 ~side:Bot.Entry.Side.Buy ~fee:5.0
  in
    printf "Position: %.4f\n" entry.position;
    printf "Avg Cost: %.2f\n" entry.avg_cost;
...
let%expect_test "ledger entry apply_fill - multiple buys average cost" =
  let entry = Bot.Entry.empty ~symbol:"BTCUSD" ~venue:Bot.Entry.Venue.Gemini in
  let entry =
    Bot.Entry.apply_fill entry ~price:50000.0 ~qty:1.0 ~side:Bot.Entry.Side.Buy ~fee:5.0
  in
  let entry =
    Bot.Entry.apply_fill entry ~price:52000.0 ~qty:1.0 ~side:Bot.Entry.Side.Buy ~fee:5.0
  in
    printf "Position: %.4f\n" entry.position;
    printf "Avg Cost: %.2f\n" entry.avg_cost;
    printf "Trade Count: %d\n" entry.trade_count;
...
let%expect_test "ledger entry apply_fill - buy then sell (realize profit)" =
  ...
  in
  let entry =
    Bot.Entry.apply_fill entry ~price:51000.0 ~qty:0.5 ~side:Bot.Entry.Side.Sell ~fee:2.5
  in
    printf "Position: %.4f\n" entry.position;
...
let%expect_test "ledger entry apply_fill - buy then sell (realize loss)" =
  ...
  in
  let entry =
    Bot.Entry.apply_fill entry ~price:49000.0 ~qty:0.5 ~side:Bot.Entry.Side.Sell ~fee:2.5
  in
    printf "Position: %.4f\n" entry.position;
    printf "Realized P&L: %.2f\n" entry.realized_pnl;
    printf "Winning Trades: %d\n" entry.winning_trades;
...
let%expect_test "ledger entry apply_fill - short position" =
  let entry = Bot.Entry.empty ~symbol:"BTCUSD" ~venue:Bot.Entry.Venue.Gemini in
  let entry =
    Bot.Entry.apply_fill entry ~price:50000.0 ~qty:1.0 ~side:Bot.Entry.Side.Sell ~fee:5.0
  in
    printf "Position: %.4f\n" entry.position;
    printf "Avg Cost: %.2f\n" entry.avg_cost;
...
let%expect_test "ledger entry summary" =
  ...
  let summary = Bot.Entry.summary entry in
    printf
      "Summary contains BTCUSD: %b\n"
      (String.is_substring summary ~substring:"BTCUSD");
    printf
      "Summary contains Gemini: %b\n"
...
let%expect_test "ledger entries_for_venue" =
  ...
  in
  let gemini_entries =
    Bot.Ledger.entries_for_venue ledger ~venue:Bot.Entry.Venue.Gemini
  in
  let kraken_entries =
    Bot.Ledger.entries_for_venue ledger ~venue:Bot.Entry.Venue.Kraken
  in
...
let%expect_test "ledger summary" =
  ...
  let summary = Bot.Ledger.summary ledger in
    printf
      "Summary contains Ledger: %b\n"
      (String.is_substring summary ~substring:"Ledger");
    printf
      "Summary contains entries: %b\n"
...
let%expect_test "dashboard render_once basic" =
  ...
  let output = Bot.Dashboard.render_once dashboard state in
    printf
      "Output contains render-test: %b\n"
      (String.is_substring output ~substring:"render-test");
    printf
      "Output contains escape codes: %b\n"
