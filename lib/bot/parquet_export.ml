...
module Row = struct
  ...
  let to_json t =
    let opt_field name value =
      Option.value_map value ~default:"" ~f:(fun v -> sprintf ", \"%s\": \"%s\"" name v)
    in
    let opt_float_field name value =
      Option.value_map value ~default:"" ~f:(fun v -> sprintf ", \"%s\": %.8f" name v)
    in
      sprintf
        "{\"event_id\": \"%s\", \"timestamp\": \"%s\", \"sequence\": %Ld, \"category\": \
         \"%s\", \"event_type\": \"%s\"%s%s%s%s%s%s%s%s}"
        t.event_id
        t.timestamp
        t.sequence
        t.category
  ...
  let of_envelope (env : Event.envelope) =
    ...
    in
      ...
      | Event.Order (Order_filled {order_id; venue; fill_qty; fill_price; fee; _}) ->
        ...
        ; fee= Some fee }
      | Event.Order
          (Order_partially_filled {order_id; venue; fill_qty; fill_price; fee; _}) ->
        { base with
      ...
      | Event.System (Bot_started {bot_id; _}) ->
        {base with event_type= "bot_started"; message= Some bot_id}
      | Event.System (Bot_stopped {reason; _}) ->
        {base with event_type= "bot_stopped"; message= Some reason}
      ...
      | Event.System (Heartbeat {sequence}) ->
        {base with event_type= "heartbeat"; message= Some (sprintf "%Ld" sequence)}
...
let export_csv ~events ~output_path ~include_header =
  ...
  let%bind () =
    ...
    | true ->
      Writer.write_line writer Row.csv_header;
      return ()
    | false -> return ()
  in
...
let export ~events ~config =
  let output_path = config.Config.output_path in
    ...
    | Csv -> export_csv ~events ~output_path ~include_header:config.include_header
    | Json_lines -> export_jsonl ~events ~output_path
    | Arrow_ipc ->
...
module Stats = struct
  ...
  let of_events events =
    ...
    let total_fees = ref 0. in
      ...
      ; time_range=
          (match (!first_time, !last_time) with
