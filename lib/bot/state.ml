...
type t =
  ...
  ; connections: Connection.t Map.M(Event.Venue).t (* Trading state *)
  ; ledger: Unified_ledger.Ledger.t
  ; active_orders: Active_order.t Map.M(String).t (** order_id -> order *)
  ; balances: Balance.t list Map.M(Event.Venue).t (* Event tracking *)
  ; last_sequence: int64
  ; event_count: int
  ; last_event_time: Time.t option (* Error tracking *)
  ; error_count: int
...
let apply_event t (envelope : Event.envelope) =
  ...
  in
    ...
    | System (Bot_started {bot_id; version; _}) ->
      {t with bot_id; started_at= envelope.timestamp; is_running= true; version}
    | System (Bot_stopped _) -> {t with is_running= false}
    | System (Connection_state_changed {venue; new_state; _}) ->
      let conn =
        Map.find t.connections venue |> Option.value ~default:(Connection.create venue)
      in
      let status =
    ...
    | System (Error {message; is_fatal; _}) ->
      ...
      ; is_running=
          (match is_fatal with
           | true -> false
           | false -> t.is_running) }
    | System (Heartbeat _) -> t (* Just updates sequence/timestamp, already done above *)
    (* Order events *)
    ...
    | Order (Order_accepted {order_id; exchange_id; _}) ->
      let active_orders =
        Map.change t.active_orders order_id ~f:(function
          ...
          | Some order ->
            Some
              {order with exchange_id= Some exchange_id; last_update= envelope.timestamp})
      in
    ...
    | Market (Book_update {symbol; bids; asks; _}) ->
      ...
      in
        (match Float.(mid_price > 0.) with
         ...
         | true ->
           let ledger =
             Unified_ledger.Ledger.mark_to_market t.ledger ~prices:[(symbol, mid_price)]
           in
             {t with ledger})
    | Market (Trade {symbol; price; _}) ->
      let ledger =
        Unified_ledger.Ledger.mark_to_market t.ledger ~prices:[(symbol, price)]
      in
        {t with ledger}
    | Market (Ticker {symbol; last; _}) ->
      let ledger =
        Unified_ledger.Ledger.mark_to_market t.ledger ~prices:[(symbol, last)]
      in
        {t with ledger}
...
let summary t =
  ...
  in
    sprintf
      "Bot[%s] running=%b orders=%d pnl=%s%.2f events=%d conn=[%s]"
      t.bot_id
      t.is_running
...
let connected_venues t = Map.filter t.connections ~f:Connection.is_connected |> Map.keys

(** Get all ready venues *)
let ready_venues t = Map.filter t.connections ~f:Connection.is_ready |> Map.keys
