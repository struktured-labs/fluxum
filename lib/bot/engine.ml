...
module Config = struct
  ...
  let to_json t = Sexp.to_string (sexp_of_t t)
end
...
let build_context t =
  ...
  let unrealized_pnl = Unified_ledger.Ledger.total_unrealized t.state.ledger in
    { Strategy_intf.Context.timestamp= Event.Time.now ()
    ; positions
    ; balances
    ; active_orders
    ; total_pnl
...
let execute_signal t signal =
  ...
  | Strategy_intf.Signal.Place_order req ->
    ...
    let%bind () =
      match t.order_submitter with
      | None ->
        (* No order submitter - just log the signal *)
        let signal_event =
          Event.Strategy
            (Event.Strategy_event.Signal_generated
               { signal_type= Event.Strategy_event.Place_order
               ; symbol= Some req.symbol
               ; reason=
                   sprintf
                     "Strategy requested %s %.8f @ %s (no submitter configured)"
        ...
        in
          (match result with
           | Ok exchange_id ->
             let accepted_event =
               Event.Order
                 (Event.Order_event.Order_accepted
           ...
           | Error reason ->
             let rejected_event =
               Event.Order
                 (Event.Order_event.Order_rejected {order_id; venue= req.venue; reason})
             in
...
let on_book_update t ~symbol ~venue ~bids ~asks ~is_snapshot =
  (* Emit market event *)
  let event =
    Event.Market (Event.Market_event.Book_update {symbol; venue; bids; asks; is_snapshot})
  in
  let%bind () = emit t event in
...
let on_trade t ~symbol ~venue ~price ~qty ~side =
  let event =
    Event.Market
      (Event.Market_event.Trade {symbol; venue; price; qty; side; trade_id= None})
  in
  let%bind () = emit t event in
    match (t.strategy, t.is_running) with
    | Some strategy, true ->
...
let on_heartbeat t =
  ...
  let event =
    Event.System (Event.System_event.Heartbeat {sequence= t.heartbeat_sequence})
  in
    emit t event
...
let create config =
  let%bind event_writer =
    Event_store.Writer.create
...
(** Set the strategy *)
let set_strategy t strategy = t.strategy <- Some strategy
...
let set_order_submitter t submitter = t.order_submitter <- Some submitter
...
let start t =
  ...
  | false ->
    ...
    let event =
      Event.System
        (Event.System_event.Bot_started
...
let stop t ~reason =
  ...
  | true ->
    ...
    in
      ...
      (* Emit stop event *)
      let event =
        Event.System (Event.System_event.Bot_stopped {reason; exit_code= Some 0})
      in
      let%bind () = emit t event in
...
let update_connection t ~venue ~old_state ~new_state =
  let event =
    Event.System
      (Event.System_event.Connection_state_changed {venue; old_state; new_state})
  in
    emit t event

(** Report error *)
let report_error t ~message ~is_fatal ?venue () =
  let event = Event.System (Event.System_event.Error {venue; message; is_fatal}) in
    emit t event
...
let update_balance t ~venue ~currency ~available ~locked ~total =
  let event =
    Event.Balance
      (Event.Balance_event.Balance_update {venue; currency; available; locked; total})
  in
    emit t event
...
let summary t =
  sprintf
    "Engine[%s] running=%b strategy=%s state: %s"
    t.config.bot_id
    t.is_running
    (match t.strategy with
     | Some s -> s.name
...
let replay ~base_path ~bot_id =
  let init_state = State.empty ~bot_id in
    Event_store.replay_all ~base_path ~bot_id ~init:init_state ~f:(fun envelope state ->
