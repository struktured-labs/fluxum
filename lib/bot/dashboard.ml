...
let create config = {config; recent_events= []; last_render= Time_ns.epoch}
...
let render_line width = String.make width '-'
...
let render_header ~bot_id ~is_running ~uptime ~width =
  ...
  let header =
    sprintf
      "Bot: %s  │  Status: %s  │  Uptime: %s"
      (Color.bold_text bot_id)
      status
      uptime_str
  in
...
let render_connections state ~width =
  let conns =
    Map.fold state.State.connections ~init:[] ~f:(fun ~key:venue ~data:conn acc ->
      let status =
        match conn.State.Connection.status with
  ...
  let conn_str =
    ...
    | _ ->
      List.map conns ~f:(fun (venue, status) -> sprintf "%s: %s" venue status)
      |> String.concat ~sep:"  │  "
  in
  ...
  let lines = Buffer.create 256 in
    ...
    | _ ->
      List.iter active_entries ~f:(fun entry ->
        let pos_str =
          match Float.(entry.position >= 0.) with
...
let render_pnl state ~width =
  ...
  in
    sprintf
      "│ P&L: Total: %s  │  Realized: %s  │  Unrealized: %s  │  Fees: %.2f%*s │"
      (format_pnl total)
      (format_pnl realized)
  ...
  let count = List.length orders in
    ...
    | _ ->
      List.iter (List.take orders 5) ~f:(fun order ->
        let side_str =
          match order.State.Active_order.side with
  ...
  let lines = Buffer.create 512 in
    ...
    | events ->
      List.iter (List.take events t.config.max_events) ~f:(fun env ->
        let time =
          Time_ns.to_string_abs_parts env.Event.timestamp ~zone:Time_float.Zone.utc
          |> List.last_exn
          |> fun s -> String.prefix s 12
        in
        let desc = Event.describe env.event in
        let desc =
          match String.length desc > width - 20 with
...
let render t state =
  ...
  let buf = Buffer.create 2048 in
    ...
    let uptime = State.uptime state in
      ...
      (* Footer *)
      let now =
        Time_ns.to_string_abs_parts (Time_ns.now ()) ~zone:Time_float.Zone.utc
        |> String.concat ~sep:" "
      in
        Buffer.add_string buf (sprintf "└%s┘\n" (render_line width));
        Buffer.add_string
          buf
          (sprintf
             "%sLast update: %s  │  Events: %d  │  Press Ctrl+C to stop%s\n"
             Color.dim
             now
             state.event_count
             Color.reset);
...
let render_once t state = render t state
