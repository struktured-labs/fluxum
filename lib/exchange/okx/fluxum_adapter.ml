...
module Adapter = struct
  ...
  let cancel_order t ~symbol ~order_id =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      let req : V5.Cancel_order.request =
        {instId= symbol; ordId= Some order_id; clOrdId= None}
      in
  ...
  let get_order_history t ~symbol =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      let req : V5.Orders_history.request =
        {instType= t.inst_type; instId= Some symbol; limit= Some 100}
      in
  ...
  let get_ticker t ~symbol =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      ...
      let req : V5.Market_tickers.request =
        {instType= t.inst_type; uly= None; instFamily= None}
      in
      let%bind result = V5.Market_tickers.request t.cfg req in
        match result with
        | `Ok resp ->
          (match
             List.find resp.data ~f:(fun ticker -> String.equal ticker.instId symbol)
           with
  ...
  let get_order_book t ~symbol =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      let req : V5.Orderbook.request = {instId= symbol; sz= Some 100} in
  ...
  let get_recent_trades t ~symbol =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      let req : V5.Recent_trades.request = {instId= symbol; limit= Some 100} in
  ...
  let get_symbols t =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      let req : V5.Instruments.request = {instType= t.inst_type; instId= None} in
  ...
  let get_balances t =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      ...
      let%bind result = V5.Account_balance.request t.cfg req in
        match result with
        | `Ok resp ->
          let balances = List.concat_map resp.data ~f:(fun account -> account.details) in
            return (Ok balances)
        | #Rest.Error.t as e -> return (Error e))
          ...
          in
            (match matching with
             ...
             | None ->
               return
                 (Error
                    (`Bad_request "No deposit address found for this currency/network")))
        | #Rest.Error.t as e -> return (Error e))
  ...
  let withdraw t ~currency ~amount ~address ?tag ?network () =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      ...
      let fee = "0" in
      (* OKX calculates the fee automatically when 0 is passed *)
      let dest = "4" in
      (* On-chain withdrawal *)
      let to_addr =
        match tag with
      ...
      let req : V5.Withdraw.request =
        {ccy= currency; amt= Float.to_string amount; dest; toAddr= to_addr; fee; chain}
      in
  ...
  let get_withdrawals t ?currency ?limit () =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      ...
      let%bind result = V5.Withdrawal_history.request t.cfg req in
        match result with
        | `Ok resp ->
          return (Ok (List.map resp.data ~f:(fun w -> Native.Withdrawal.History w)))
        | #Rest.Error.t as e -> return (Error e))
  ...
  module Normalize = struct
      ...
      | "0" ->
        Ok
          ({ venue= Venue.t
           ...
           ; updated_at= None }
           : Types.Order.t)
      | code -> Error (sprintf "Order placement failed: code=%s msg=%s" code resp.sMsg)
    ...
    let order_from_status (status : Native.Order.status)
      ...
      let%bind filled =
        Fluxum.Normalize_common.Float_conv.qty_of_string status.accFillSz
      in
        Ok
          ({ venue= Venue.t
           ; id= status.ordId
           ...
           ; created_at=
               (match Fluxum.Normalize_common.Float_conv.of_string status.cTime with
                | Ok ms ->
                  Some
                    (Time_float_unix.of_span_since_epoch (Time_float_unix.Span.of_ms ms))
                | Error _ -> None)
    ...
    let balance (b : Native.Balance.t) : (Types.Balance.t, string) Result.t =
      ...
      let total = cash in
        (* cashBal represents total balance *)
        Ok
          ({venue= Venue.t; currency= b.ccy; total; available; locked= frozen}
           : Types.Balance.t)
    ...
    let deposit (d : Native.Deposit.t) : (Types.Deposit.t, string) Result.t =
      ...
      in
      let address =
        match String.is_empty d.to_ with
        | true -> None
        | false -> Some d.to_
      in
        Ok
