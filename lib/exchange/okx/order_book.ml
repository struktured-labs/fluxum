...
type metadata = {checksum: int option (* OKX provides checksums for validation *)}
[@@deriving sexp]
...
module Book = struct
  ...
  let pipe ~symbol ?(depth = 10) () : (t, string) Result.t Pipe.Reader.t Deferred.t =
    ...
    let%bind ws_result = Ws.connect ~streams:[stream] () in
      ...
      | Error err ->
        Log.Global.error
          "OKX WS connection failed for %s: %s"
          symbol
        ...
        let reader, writer = Pipe.create () in
          don't_wait_for
            (Pipe.write
               writer
               (Error
                  (sprintf "WebSocket connection failed: %s" (Error.to_string_hum err))));
          Pipe.close writer;
          return reader
      | Ok ws ->
        let reader, writer = Pipe.create () in
...
module Books = struct
  ...
  let set_book ?timestamp:_ t (book : book) = set_book t book
end
