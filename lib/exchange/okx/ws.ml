...
module Stream = struct
  ...
  let to_arg t =
    `Assoc
      [("channel", `String (channel_to_string t.channel)); ("instId", `String t.instId)]

  let to_subscribe_message streams =
    `Assoc [("op", `String "subscribe"); ("args", `List (List.map streams ~f:to_arg))]

  let to_unsubscribe_message streams =
    `Assoc [("op", `String "unsubscribe"); ("args", `List (List.map streams ~f:to_arg))]
end
  ...
  *)
  ...
  let candle_of_yojson = function
    | `List
        ...
        ; `String volCcy
        ; `String volCcyQuote
        ; `String confirm ] -> Ok (ts, o, h, l, c, vol, volCcy, volCcyQuote, confirm)
  ...
  let subscription_event_of_yojson json =
    ...
    let%bind event =
      Yojson.Safe.Util.member "event" json |> Yojson.Safe.Util.to_string |> Result.return
    in
    let arg =
      try Some (Yojson.Safe.Util.member "arg" json |> Yojson.Safe.to_string) with
      | _ -> None
    in
    let connId =
      try Some (Yojson.Safe.Util.member "connId" json |> Yojson.Safe.Util.to_string) with
      | _ -> None
    in
      Ok {event; arg; connId}
...
let parse_message (msg : string) : Message.t =
  try
    ...
    let event_opt =
      try Some (Yojson.Safe.Util.member "event" json |> Yojson.Safe.Util.to_string) with
      | _ -> None
    in
      ...
      | None ->
        ...
        (match json with
         | `Assoc _ ->
           ...
           let channel =
             try Yojson.Safe.Util.member "channel" arg |> Yojson.Safe.Util.to_string with
             | _ -> ""
           in
             (* Determine message type based on channel *)
             if String.equal channel "books"
             then (
               match data with
               | `List items ->
                 let parsed =
             ...
             else if String.equal channel "trades"
             then (
               match data with
               | `List items ->
                 let parsed =
               ...
               | _ -> Message.Unknown msg)
             else if String.equal channel "tickers"
             then (
               match data with
               | `List items ->
                 let parsed =
               ...
               | _ -> Message.Unknown msg)
             else if String.is_prefix channel ~prefix:"candle"
             then (
               match data with
               | `List items ->
                 let parsed =
               ...
               | _ -> Message.Unknown msg)
             else Message.Unknown msg
         | _ -> Message.Unknown msg)
      | Some _ -> Message.Unknown msg
  with
  | e -> Message.Error (sprintf "Parse error: %s" (Exn.to_string e))
...
module Auth = struct
  ...
  *)
  let hmac_sha256 ~secret ~message =
    ...
    in
    let ipad =
      String.init block_size ~f:(fun i ->
    ...
    in
    let inner_hash =
      Digestif.SHA256.digest_string (ipad ^ message) |> Digestif.SHA256.to_raw_string
    in
...
module Private = struct
  ...
  let subscribe_orders ?(inst_type = "SPOT") () : string =
    Yojson.Safe.to_string
      (`Assoc
          ...
          ; ( "args"
            , `List
                [`Assoc [("channel", `String "orders"); ("instType", `String inst_type)]]
            ) ])
  ...
  let subscribe_account () : string =
    Yojson.Safe.to_string
      (`Assoc
          [ ("op", `String "subscribe")
          ; ("args", `List [`Assoc [("channel", `String "account")]]) ])
  ...
  let parse_message (msg : string) : message =
    try
      ...
      in
        ...
        | Some "login" ->
          let code =
            try Yojson.Safe.Util.member "code" json |> Yojson.Safe.Util.to_string with
            | _ -> ""
          in
            (match String.equal code "0" with
             ...
             | false ->
               let err_msg =
                 try Yojson.Safe.Util.member "msg" json |> Yojson.Safe.Util.to_string with
                 | _ -> "unknown error"
               in
                 LoginError err_msg)
        | Some "error" ->
        ...
        | _ ->
          ...
          in
            (match channel with
                ...
                | `List items ->
                  let accounts =
                    List.filter_map items ~f:(fun item ->
                      ...
                      let details =
                        try
                          Yojson.Safe.Util.member "details" item
                          |> function
                          | `List detail_items ->
             ...
             | _ -> Unknown msg)
    with
    | e -> Unknown (sprintf "Parse error: %s" (Exn.to_string e))
end
...
let connect ?(url = Endpoint.public_production) ?(streams = []) () : t Deferred.Or_error.t
  ...
  let%bind ws_result = Websocket_curl.connect ~url () in
    ...
    | Error err -> Deferred.Or_error.error_string (Error.to_string_hum err)
    | Ok ws ->
...
let unsubscribe t streams =
  t.streams
  := List.filter !(t.streams) ~f:(fun s -> not (List.mem streams s ~equal:Poly.equal));
  match t.active with
  | false -> Deferred.Or_error.error_string "WebSocket connection is closed"
  | true ->
