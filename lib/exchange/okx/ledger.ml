      ...
      | false ->
        ...
        let total_buy_qty, buy_notional, avg_buy_price =
          ...
          | Buy ->
            ...
            let new_avg =
              if Float.(new_total > 0.0) then new_notional /. new_total else 0.0
            in
        ...
        let pnl =
          ...
          | Sell ->
            ...
            let cost_basis_reduction =
              if Float.(t.position > 0.0) then t.cost_basis /. t.position *. qty else 0.0
            in
...
let from_balances (balances : Fluxum.Types.Balance.t list) : t =
  List.fold balances ~init:empty ~f:(fun acc balance ->
    let symbol = balance.currency in
    (* In OKX, currency is the symbol *)
    let entry =
      Entry.create
        ~symbol
...
(** Market data updates *)
let on_market_data ?update_source ?timestamp ?mid_price t ~symbol ~book =
  match Map.find t symbol with
  | None ->
    (* No entry yet, create one and apply book *)
    let entry = Entry.create ~symbol () in
    let updated = Entry.on_market_data ?update_source ?timestamp ?mid_price entry ~book in
      Map.set t ~key:symbol ~data:updated
  | Some entry ->
    (* Update existing entry *)
    let updated = Entry.on_market_data ?update_source ?timestamp ?mid_price entry ~book in
...
let command : Command.t =
  Command.group
    ~summary:"OKX Ledger Commands"
    [ ( "placeholder"
      , Command.basic
          ~summary:"OKX ledger - use `from_balances` or `pipe` in code"
          (Command.Param.return (fun () ->
             printf "OKX ledger: Real-time P&L tracking available via API\n")) ) ]
