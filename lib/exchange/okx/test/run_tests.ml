...
let test_ledger_entry_creation () =
  ...
  let _ =
    assert_float_equal 0.0 entry.avg_sell_price "Empty entry has 0 avg_sell_price"
  in
  ...
  let _ =
    assert_float_equal 0.0 entry.total_sell_qty "Empty entry has 0 total_sell_qty"
  in
  ...
  let _ =
    assert_float_equal 0.0 entry.total_original "Empty entry has 0 total_original"
  in
  let _ =
    assert_float_equal 0.0 entry.total_executed "Empty entry has 0 total_executed"
  in
  let _ =
    assert_float_equal 0.0 entry.total_remaining "Empty entry has 0 total_remaining"
  in
  ...
  let _ =
    assert_equal
      ~equal:[%equal: Types.Price.Option.t]
      ~sexp_of_t:[%sexp_of: Types.Price.Option.t]
      None
      entry.price
      "Empty entry has None price"
  in
  let _ =
    assert_equal
      ~equal:[%equal: Types.Side.Option.t]
      ~sexp_of_t:[%sexp_of: Types.Side.Option.t]
      None
      entry.side
      "Empty entry has None side"
  in
  let _ =
    assert_equal
  ...
  let _ =
    assert_equal
      ...
      None
      entry.package_price
      "Empty entry has None package_price"
  in
...
let test_ledger_simple_trades () =
  ...
  let _ =
    assert_float_equal 50000.0 entry.pnl_spot "Buy 1 BTC: pnl_spot = price * position"
  in
...
let test_ledger_fees () =
  ...
  let _ =
    assert_float_equal
      (-50.0)
      entry.pnl
      "Fee creates total loss: pnl = -$50 (fee on buy + cost basis drag)"
  in
  (* Buy with fee, sell at higher price to cover fee *)
  let entry2 = Ledger.Entry.create ~symbol:"ETH-USDT" () in
  let entry2 =
    Ledger.Entry.on_trade entry2 ~price:3000.0 ~side:Buy ~qty:1.0 ~fee_usd:10.0
  in
  let _ = assert_float_equal 3010.0 entry2.cost_basis "ETH fee in cost basis: $3,010" in
  (* Sell with fee *)
  let entry2 =
    Ledger.Entry.on_trade entry2 ~price:3100.0 ~side:Sell ~qty:1.0 ~fee_usd:10.0
  in
...
let test_ledger_spot_updates () =
  ...
  let _ =
    assert_float_equal 0.5 entry_sold.position "After partial sell: position = 0.5"
  in
  ...
  let _ =
    assert_float_equal 55050.0 entry_updated.pnl_spot "Market data: pnl_spot = $55,050"
  in
  (* Lower spot *)
  ...
  let _ =
    assert_float_equal 45050.0 entry_lower.pnl_spot "Lower spot: pnl_spot = $45,050"
  in
    ()
...
let test_ledger_cost_basis_accounting () =
  ...
  let _ =
    assert_float_equal 98000.0 entry.cost_basis "After buy 1 more: cost_basis = $98k"
  in
  let _ = assert_float_equal 2.0 entry.position "Position = 2.0" in
  (* Sell all at $51k *)
...
let test_ledger_multi_symbol () =
  ...
  (* Trade BTC *)
  let ledger =
    Ledger.on_trade ledger ~symbol:"BTC-USDT" ~price:50000.0 ~side:Buy ~qty:1.0
  in
  (* Trade ETH *)
  let ledger =
    Ledger.on_trade ledger ~symbol:"ETH-USDT" ~price:3000.0 ~side:Buy ~qty:10.0
  in
  (* Check BTC *)
  ...
  let ledger =
    Ledger.on_trade ledger ~symbol:"BTC-USDT" ~price:52000.0 ~side:Buy ~qty:0.5
  in
  let ledger =
    Ledger.on_trade ledger ~symbol:"ETH-USDT" ~price:3200.0 ~side:Sell ~qty:5.0
  in
  let btc2 = Map.find_exn ledger "BTC-USDT" in
  let _ = assert_float_equal 1.5 btc2.position "BTC position after 2nd buy = 1.5" in
  let _ =
    assert_float_equal 76000.0 btc2.cost_basis "BTC cost_basis = $76k (50k + 26k)"
  in
...
let test_order_book_creation () =
  ...
  let _ =
    assert_float_equal 0.0 (Order_book.Book.mid_price book) "Empty book: mid_price = 0"
  in
...
let test_order_book_bid_sorting () =
  ...
  let _ =
    assert_equal
      ...
      3
      (List.length bids)
      "bids_alist has 3 levels"
  in
  let _ =
    assert_float_equal 51000.0 (fst (List.nth_exn bids 0)) "bids_alist[0] price = $51k"
  in
  let _ =
    assert_float_equal 50000.0 (fst (List.nth_exn bids 1)) "bids_alist[1] price = $50k"
  in
  let _ =
    assert_float_equal 49000.0 (fst (List.nth_exn bids 2)) "bids_alist[2] price = $49k"
  in
  (* Verify volumes via Price_level *)
  let _ =
    assert_float_equal
      1.5
      (Order_book.Price_level.volume (snd (List.nth_exn bids 0)))
      "Best bid volume = 1.5"
  in
  let _ =
    assert_float_equal
      1.0
      (Order_book.Price_level.volume (snd (List.nth_exn bids 1)))
      "2nd bid volume = 1.0"
  in
  let _ =
    assert_float_equal
      2.0
      (Order_book.Price_level.volume (snd (List.nth_exn bids 2)))
      "3rd bid volume = 2.0"
  in
    ()
...
let test_order_book_ask_sorting () =
  ...
  let _ =
    assert_equal
      ...
      3
      (List.length asks)
      "asks_alist has 3 levels"
  in
  let _ =
    assert_float_equal 51000.0 (fst (List.nth_exn asks 0)) "asks_alist[0] price = $51k"
  in
  let _ =
    assert_float_equal 52000.0 (fst (List.nth_exn asks 1)) "asks_alist[1] price = $52k"
  in
  let _ =
    assert_float_equal 53000.0 (fst (List.nth_exn asks 2)) "asks_alist[2] price = $53k"
  in
  (* Verify volumes via Price_level *)
  let _ =
    assert_float_equal
      1.5
      (Order_book.Price_level.volume (snd (List.nth_exn asks 0)))
      "Best ask volume = 1.5"
  in
  let _ =
    assert_float_equal
      1.0
      (Order_book.Price_level.volume (snd (List.nth_exn asks 1)))
      "2nd ask volume = 1.0"
  in
  let _ =
    assert_float_equal
      2.0
      (Order_book.Price_level.volume (snd (List.nth_exn asks 2)))
      "3rd ask volume = 2.0"
  in
    ()
...
let test_order_book_operations () =
  ...
  let _ =
    assert_float_equal
      1.5
      (Order_book.Price_level.volume best_bid)
      "Set creates level with size 1.5"
  in
  let _ =
    assert_float_equal
      50000.0
      (Order_book.Price_level.price best_bid)
      "Set creates level at $50k"
  in
  (* Set with size=0 removes level *)
  let book = Order_book.Book.set book ~side:`Bid ~price:50000.0 ~size:0.0 in
  let best = Order_book.Book.best_bid book in
  let _ =
    assert_float_equal 0.0 (Order_book.Price_level.price best) "Size=0 removes level"
  in
  ...
  let best_bid = Order_book.Book.best_bid book in
  let _ =
    assert_float_equal 2.5 (Order_book.Price_level.volume best_bid) "Set replaces: 2.5"
  in
  (* Set on ask side *)
  let book = Order_book.Book.set book ~side:`Ask ~price:51000.0 ~size:3.0 in
  let best_ask = Order_book.Book.best_ask book in
  let _ =
    assert_float_equal 3.0 (Order_book.Price_level.volume best_ask) "Ask set: size 3.0"
  in
  let _ =
    assert_float_equal 51000.0 (Order_book.Price_level.price best_ask) "Ask set at $51k"
  in
  (* Replace ask *)
  let book = Order_book.Book.set book ~side:`Ask ~price:51000.0 ~size:0.5 in
  let best_ask = Order_book.Book.best_ask book in
  let _ =
    assert_float_equal 0.5 (Order_book.Price_level.volume best_ask) "Ask replaced: 0.5"
  in
  (* Remove ask with 0 *)
  ...
  let _ =
    assert_float_equal
      0.0
      (Order_book.Price_level.price best_ask)
      "Ask removed: price = 0"
  in
    ()
...
let test_normalize_float_conversion_errors () =
  ...
  let _ =
    assert_error
      ...
      "Rejected malformed float string"
  in
  (* Test empty string *)
  let _ =
    assert_error (Fluxum.Normalize_common.Float_conv.of_string "") "Rejected empty string"
  in
  (* Test infinity *)
  let _ =
    assert_error
      (Fluxum.Normalize_common.Float_conv.of_string "inf")
      "Rejected non-finite float (infinity)"
  in
  (* Test negative infinity *)
  let _ =
    assert_error
      (Fluxum.Normalize_common.Float_conv.of_string "-inf")
      "Rejected negative infinity"
  in
  (* Test NaN *)
  let _ =
    assert_error (Fluxum.Normalize_common.Float_conv.of_string "nan") "Rejected NaN"
  in
...
let test_normalize_edge_cases () =
  ...
  (match Fluxum.Normalize_common.Float_conv.price_of_string "0.00000001" with
   | Ok p ->
     let _ =
       assert_float_equal 0.00000001 p ~tolerance:0.000000001 "Very small positive price"
     in
       ()
   | Error msg ->
  ...
  let _ =
    assert_ok (Fluxum.Normalize_common.Side.of_string "buy") "Side 'buy' is valid"
  in
  let _ =
    assert_ok (Fluxum.Normalize_common.Side.of_string "sell") "Side 'sell' is valid"
  in
  let _ =
    assert_ok
      (Fluxum.Normalize_common.Side.of_string "BUY")
      "Side 'BUY' is valid (case insensitive)"
  in
  let _ =
    assert_ok
      (Fluxum.Normalize_common.Side.of_string "SELL")
      "Side 'SELL' is valid (case insensitive)"
  in
  let _ = assert_ok (Fluxum.Normalize_common.Side.of_string "b") "Side 'b' is valid" in
  let _ = assert_ok (Fluxum.Normalize_common.Side.of_string "s") "Side 's' is valid" in
  let _ =
    assert_ok (Fluxum.Normalize_common.Side.of_string "bid") "Side 'bid' is valid"
  in
  let _ =
    assert_ok (Fluxum.Normalize_common.Side.of_string "ask") "Side 'ask' is valid"
  in
  (* Test Side.of_string invalid *)
  let _ =
    assert_error
      (Fluxum.Normalize_common.Side.of_string "invalid_side")
      "Side 'invalid_side' rejected"
  in
  let _ = assert_error (Fluxum.Normalize_common.Side.of_string "") "Side '' rejected" in
