...
module Signature = struct
  ...
  let hmac_sha256 ~secret ~message =
    ...
    let inner_hash =
      Digestif.SHA256.digest_string inner_msg |> Digestif.SHA256.to_raw_string
    in
      ...
      in
        ...
        | `OK | `Created ->
          ...
          (try
           ...
           with
           | exn -> `Json_parse_error Error.{message= Exn.to_string exn; body= s})
        | `Bad_request ->
          Cohttp_async.Body.to_string resp_body >>| fun s -> `Bad_request s
        | `Not_found -> return `Not_found
        | `Unauthorized ->
          Cohttp_async.Body.to_string resp_body >>| fun s -> `Unauthorized s
        | `Forbidden -> Cohttp_async.Body.to_string resp_body >>| fun s -> `Forbidden s
        | `Too_many_requests ->
          Cohttp_async.Body.to_string resp_body >>| fun s -> `Too_many_requests s
        | `Service_unavailable | `Internal_server_error | `Gateway_timeout ->
          Cohttp_async.Body.to_string resp_body >>| fun s -> `Service_unavailable s
        | status ->
          Cohttp_async.Body.to_string resp_body
          >>| fun s ->
          `Bad_request
            (sprintf "Unexpected status %s: %s" (Cohttp.Code.string_of_status status) s)
end
  ...
  let command =
    Command.async
      ...
      (let%map_open.Command cfg = Cfg.param
       and req_sexp = anon ("REQUEST" %: sexp_conv [%of_sexp: Operation.request]) in
         fun () ->
