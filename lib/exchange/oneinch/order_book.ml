(** 1inch Order Book - Uses common Order_book_base *)
...
(** 1inch-specific metadata - none beyond base *)
...
module Book = struct
  ...
  let empty ?timestamp:_ ?epoch:_ symbol = create ~symbol

  (** Update (add or remove) price level *)
  let update t ~side ~price ~size =
    let levels =
      match Poly.(side = `Bid) with
      | true -> bids_alist t
      | false -> asks_alist t
    in
    let current =
  ...
  let remove t ~side ~price ~size = update t ~side ~price ~size:(-.size)

  (** Apply book update from 1inch WebSocket

      Note: This is a placeholder implementation until 1inch WebSocket client is implemented.
      When the WS client is ready, this should parse 1inch's specific update format. *)
  let apply_book_update t (update : Yojson.Safe.t) : t =
    try
      let open Yojson.Safe.Util in
      (* 1inch format: {"b": [[price, size], ...], "a": [[price, size], ...]} *)
      ...
      let safe_to_list json =
        ...
        | other ->
          Log.Global.debug
            "1inch WS: Expected list for bids/asks, got: %s"
      ...
      let t_with_bids =
        List.fold bids ~init:t ~f:(fun acc level ->
              ...
              | [price_json; size_json] ->
                let%bind price =
                  Fluxum.Normalize_common.Float_conv.price_of_string
                    (to_string price_json)
                in
                let%bind volume =
                  Fluxum.Normalize_common.Float_conv.qty_of_string (to_string size_json)
                in
                  Ok (price, volume)
          ...
          | Error err ->
            Log.Global.error "1inch WS: Failed to parse bid level: %s" err;
            acc)
      in
      (* Process ask updates *)
      let t_with_asks =
        List.fold asks ~init:t_with_bids ~f:(fun acc level ->
          ...
          | Error err ->
            Log.Global.error "1inch WS: Failed to parse ask level: %s" err;
            acc)
      in
        t_with_asks
    with
    | exn ->
      Log.Global.error "1inch WS: Exception in apply_book_update: %s" (Exn.to_string exn);
      t

  (** Create live order book pipe from 1inch WebSocket

      Note: This is a placeholder implementation that returns an empty pipe.
      When 1inch WebSocket client is implemented, this should:
      1. Connect to 1inch WebSocket
  ...
  let pipe ~symbol ?(depth = 10) () : (t, string) Result.t Pipe.Reader.t Deferred.t =
    ignore depth;
    Log.Global.info
      "1inch order book pipe for %s (placeholder - WebSocket not yet implemented)"
      symbol;
