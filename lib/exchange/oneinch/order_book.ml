...
module Book = struct
  ...
  let apply_book_update t (update : Yojson.Safe.t) : t =
    try
      ...
      let safe_to_list json = match json with
        ...
        | `List l -> l
        | other ->
          Log.Global.debug "1inch WS: Expected list for bids/asks, got: %s"
  ...
  *)
  let pipe ~symbol ?(depth = 10) () : (t, string) Result.t Pipe.Reader.t Deferred.t =
    ignore depth;
    Log.Global.info "1inch order book pipe for %s (placeholder - WebSocket not yet implemented)" symbol;
