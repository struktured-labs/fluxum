...
module Types = struct
  ...
  (** Token list response *)
  type tokens_response = {tokens: (string * token_info) list} [@@deriving sexp]

  let tokens_response_of_yojson json =
    match json with
    | `Assoc [("tokens", `Assoc tokens)] ->
      let parsed =
        List.filter_map tokens ~f:(fun (addr, token_json) ->
  ...
  type liquidity_sources_response = {protocols: liquidity_source list}
  [@@deriving yojson {strict= false}, sexp]
end
  ...
  | Some api_key ->
    ...
    let headers =
      Cohttp.Header.of_list
        [("Authorization", sprintf "Bearer %s" api_key); ("Accept", "application/json")]
    in
...
let liquidity_sources cfg
  : (Types.liquidity_sources_response, [> Error.t]) result Deferred.t
  =
  get cfg ~path:"/liquidity-sources" ()
  >>| function
  | Error _ as err -> err
  | Ok json ->
...
let order_book cfg ~base_token ~quote_token ~amount
  ...
  let%bind sell_quote = quote cfg ~src:base_token ~dst:quote_token ~amount () in
    ...
    | Ok sq ->
      ...
      let%bind buy_quote =
        quote cfg ~src:quote_token ~dst:base_token ~amount:sq.toAmount ()
      in
