...
let test_cfg () =
  ...
  test_assert
    "Optimism URL"
    (String.is_substring Velodrome.Cfg.optimism.subgraph_url ~substring:"velodrome");
  test_assert
    "Production is Optimism"
    (String.equal
       Velodrome.Cfg.production.subgraph_url
       Velodrome.Cfg.optimism.subgraph_url)

let test_types () =
  printf "\n[Types]\n";
  let token_json =
    `Assoc [("id", `String "0x1"); ("symbol", `String "OP"); ("decimals", `String "18")]
  in
    (match Velodrome.Types.token_of_yojson token_json with
     | Ok t ->
       test_assert "Token parses" true;
       test_assert "Token symbol" (String.equal t.symbol "OP")
    ...
    let pair_json =
      `Assoc
        ...
        ; ( "token0"
          , `Assoc
              [("id", `String "0x1"); ("symbol", `String "OP"); ("decimals", `String "18")]
          )
        ; ( "token1"
          , `Assoc
              [ ("id", `String "0x3")
    ...
    in
      (match Velodrome.Types.pair_of_yojson pair_json with
       | Ok p ->
         ...
         test_assert "Pair stable" (Bool.equal p.stable false)
      ...
      in
        ...
        | Ok s ->
          ...
          test_assert "Swap to" (String.equal s.to_ "0x5")
...
let test_errors () =
  ...
  let http_err : Velodrome.Rest.error = `Http (404, "Not found") in
    test_assert
      "HTTP error"
      (Sexplib.Sexp.to_string_hum (Velodrome.Rest.sexp_of_error http_err)
       |> String.is_substring ~substring:"404")
