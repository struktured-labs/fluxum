...
let () =
  ...
  test_assert
    "Arbitrum URL"
    (String.is_substring Camelot.Cfg.arbitrum.subgraph_url ~substring:"camelot");
  test_assert
    "Production is Arbitrum"
    (String.equal Camelot.Cfg.production.subgraph_url Camelot.Cfg.arbitrum.subgraph_url);
  printf "\n[Types]\n";
  let t =
    `Assoc [("id", `String "0x1"); ("symbol", `String "ARB"); ("decimals", `String "18")]
  in
    (match Camelot.Types.token_of_yojson t with
     | Ok tok ->
       test_assert "Token parses" true;
       test_assert "Token symbol" (String.equal tok.symbol "ARB")
    ...
    in
      (match Camelot.Types.pair_of_yojson p with
       | Ok pair ->
         ...
         test_assert "Pair token0 symbol" (String.equal pair.token0.symbol "ARB")
