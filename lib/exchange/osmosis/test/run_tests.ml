...
let test_cfg () =
  ...
  test_assert
    "Mainnet URL"
    (String.is_substring Osmosis.Cfg.mainnet.api_url ~substring:"osmosis");
  test_assert
    "Production is Mainnet"
    (String.equal Osmosis.Cfg.production.api_url Osmosis.Cfg.mainnet.api_url)

let test_types () =
  printf "\n[Types]\n";
  let token_info_json =
    `Assoc [("denom", `String "uosmo"); ("amount", `String "1000000000")]
  in
    ...
    let pool_asset_json =
      `Assoc
        [ ("token", `Assoc [("denom", `String "uosmo"); ("amount", `String "1000000000")])
