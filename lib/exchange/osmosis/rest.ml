...
let make_request ~cfg ~endpoint =
  ...
  let%bind result =
    Deferred.Or_error.try_with (fun () -> Cohttp_async.Client.get ~headers uri)
    |> Deferred.map
...
let get_pools ~cfg =
  let%bind json_str = make_request ~cfg ~endpoint:"/osmosis/gamm/v1beta1/pools" in
    match
      Result.bind json_str ~f:(fun s ->
        try Ok (Yojson.Safe.from_string s) with
        | ex -> Error (`Json_parse (Exn.to_string ex)))
    with
    | Ok json ->
      (match json with
       | `Assoc fields ->
...
let get_spot_price ~cfg ~pool_id ~base_denom ~quote_denom =
  let endpoint =
    sprintf
      "/osmosis/gamm/v1beta1/pools/%s/prices?base_asset_denom=%s&quote_asset_denom=%s"
      pool_id
      base_denom
      quote_denom
  in
...
let get_token_prices ~cfg =
  let%bind json_str =
    make_request ~cfg ~endpoint:"/osmosis/gamm/v1beta1/estimated_prices"
  in
    match json_str with
    | Ok _ -> return (Ok ())
    | Error e -> return (Error e)
