...
let test_cfg () =
  ...
  test_assert
    "Arbitrum URL"
    (String.is_substring Gmx.Cfg.arbitrum.subgraph_url ~substring:"gmx");
  test_assert
    "Avalanche URL"
    (String.is_substring Gmx.Cfg.avalanche.subgraph_url ~substring:"avalanche");
  test_assert
    "Production is Arbitrum"
    (String.equal Gmx.Cfg.production.subgraph_url Gmx.Cfg.arbitrum.subgraph_url)
    ...
    in
      ...
      in
        ...
        | Ok position ->
          ...
          test_assert
            "Position size"
            (String.equal position.size "1000000000000000000000000000000")
