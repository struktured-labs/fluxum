...
let normalize (pool : Native.pool) : (Pool_intf.Pool.t, string) Result.t =
  ...
  let spot_price =
    match Float.(pool.token0_price > 0.0) with
    | true -> pool.token1_price /. pool.token0_price
    | false -> 0.0
  in
  let spot_price_inv =
    match Float.(pool.token1_price > 0.0) with
  ...
  let tvl_usd =
    (pool.token0_pool_amount *. pool.token0_price)
    +. (pool.token1_pool_amount *. pool.token1_price)
  in
    Ok
      { Pool_intf.Pool.id= pool.id
      ; venue
      ; pool_type= Pool_intf.Pool_type.Weighted
      ; (* GLP is similar to weighted pool *)
        token0=
