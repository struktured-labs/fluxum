      ...
      let best_ask = Binance.Order_book.Book.best_ask book in
        ...
        ignore
          (assert_float_equal
             1.0
             (Exchange_common.Order_book_base.Price_level.volume best_ask)
             "Best ask volume is 1.0");
        ignore
          (assert_int64_equal
             2L
             (Binance.Order_book.Book.last_update_id book)
             "Last update ID is 2")
    | Error msg ->
      ...
      let best_ask = Binance.Order_book.Book.best_ask book in
        ...
        ignore
          (assert_float_equal
             1.5
             (Exchange_common.Order_book_base.Price_level.volume best_ask)
             "Best ask volume is 1.5");
        ignore
          (assert_int64_equal
             100L
             (Binance.Order_book.Book.last_update_id book)
             "Last update ID is 100")
    | Error msg ->
...
let test_stream_names () =
  ...
  let ticker = Binance.Ws.Stream.Ticker "ADAUSDT" in
    ignore
      (assert_string_equal
         "btcusdt@trade"
         (Binance.Ws.Stream.to_stream_name trade)
         "Trade stream name");
    ignore
      (assert_string_equal
         "ethusdt@depth20"
         (Binance.Ws.Stream.to_stream_name depth_20)
         "Depth 20 stream name");
    ignore
      (assert_string_equal
         "bnbusdt@depth"
         (Binance.Ws.Stream.to_stream_name depth_full)
         "Depth full stream name");
    ignore
      (assert_string_equal
         "adausdt@ticker"
         (Binance.Ws.Stream.to_stream_name ticker)
         "Ticker stream name")
...
let test_books_multiple_symbols () =
  ...
  let symbols = Binance.Order_book.Books.symbols books in
    ...
    ignore
      (assert_true (List.mem symbols "BTCUSDT" ~equal:String.equal) "Contains BTCUSDT");
    ignore
      (assert_true (List.mem symbols "ETHUSDT" ~equal:String.equal) "Contains ETHUSDT")
...
let test_signature_query_string () =
  ...
  let query = Binance.Signature.build_query_string params in
    ignore
      (assert_string_equal "symbol=LTCBTC&side=BUY&type=LIMIT" query "Basic query string");
    (* Test URL encoding *)
    ...
    let query_encoded = Binance.Signature.build_query_string params_encoded in
      ignore
        (assert_true
           (String.is_substring query_encoded ~substring:"hello%20world")
    ...
    ignore
      (assert_true
         (String.for_all signature ~f:(fun c ->
            Char.is_alphanum c && not (Char.is_uppercase c)))
  ...
  let expected = "c8db56825ae71d6d79447849e617115f4a920fa2acdcab2b053c4b2838bd6b71" in
    ignore
      (assert_string_equal
         expected
         signature
         "Official Binance example signature matches")
...
let test_common_side () =
  ...
  ignore
    (assert_string_equal "SELL" (Binance.Common.Side.to_string `SELL) "SELL to string");
  ignore
    (assert_true (Option.is_some (Binance.Common.Side.of_string_opt "BUY")) "Parse BUY");
  ignore
    (assert_true (Option.is_some (Binance.Common.Side.of_string_opt "SELL")) "Parse SELL");
  ignore
    (assert_true
       (Option.is_none (Binance.Common.Side.of_string_opt "INVALID"))
...
let test_common_order_type () =
  ...
  ignore
    (assert_string_equal
       "LIMIT"
       (Binance.Common.Order_type.to_string `LIMIT)
       "LIMIT to string");
  ignore
    (assert_string_equal
       "MARKET"
       (Binance.Common.Order_type.to_string `MARKET)
       "MARKET to string");
  ignore
    (assert_string_equal
       "STOP_LOSS"
       (Binance.Common.Order_type.to_string `STOP_LOSS)
       "STOP_LOSS to string");
  ignore
    (assert_true
       (Option.is_some (Binance.Common.Order_type.of_string_opt "LIMIT"))
       "Parse LIMIT");
  ignore
    (assert_true
       (Option.is_some (Binance.Common.Order_type.of_string_opt "MARKET"))
       "Parse MARKET")
...
let test_common_time_in_force () =
  ...
  ignore
    (assert_string_equal
       "GTC"
       (Binance.Common.Time_in_force.to_string `GTC)
       "GTC to string");
  ignore
    (assert_string_equal
       "IOC"
       (Binance.Common.Time_in_force.to_string `IOC)
       "IOC to string");
  ignore
    (assert_string_equal
       "FOK"
       (Binance.Common.Time_in_force.to_string `FOK)
       "FOK to string");
  ignore
    (assert_true
       (Option.is_some (Binance.Common.Time_in_force.of_string_opt "GTC"))
       "Parse GTC")
...
let test_common_order_status () =
  ...
  ignore
    (assert_string_equal
       "NEW"
       (Binance.Common.Order_status.to_string `NEW)
       "NEW to string");
  ignore
    (assert_string_equal
       "FILLED"
       (Binance.Common.Order_status.to_string `FILLED)
       "FILLED to string");
  ignore
    (assert_string_equal
       "CANCELED"
       (Binance.Common.Order_status.to_string `CANCELED)
       "CANCELED to string");
  ignore
    (assert_true
       (Option.is_some (Binance.Common.Order_status.of_string_opt "NEW"))
       "Parse NEW");
  ignore
    (assert_true
       (Option.is_some (Binance.Common.Order_status.of_string_opt "FILLED"))
       "Parse FILLED")
