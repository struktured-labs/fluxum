      ...
      in
        ...
        | `OK | `Created ->
          ...
          (try
           ...
           with
           | e -> `Json_parse_error Rest.Error.{message= Exn.to_string e; body= s})
        ...
        | `Forbidden -> Cohttp_async.Body.to_string body >>| fun b -> `Forbidden b
        | `Too_many_requests ->
...
module Deposit_history = struct
  module T = struct
    ...
    let request_to_params
          {coin; status; startTime; endTime; offset; limit; recvWindow; txId}
      =
      let add_opt key = function
        | None -> Fun.id
        | Some v -> List.cons (key, v)
  ...
  module T = struct
    ...
    let request_to_params
      ...
      in
        ...
        |> add_opt
             "transactionFeeFlag"
             (Option.map transactionFeeFlag ~f:(function
                | true -> "true"
                | false -> "false"))
        |> add_opt "name" name
...
module Withdrawal_history = struct
  module T = struct
    ...
    let request_to_params
          {coin; withdrawOrderId; status; startTime; endTime; offset; limit; recvWindow}
      =
      let add_opt key = function
        | None -> Fun.id
        | Some v -> List.cons (key, v)
...
module All_coins_info = struct
  module T = struct
    ...
    type request = {recvWindow: int option [@default None]} [@@deriving sexp]
