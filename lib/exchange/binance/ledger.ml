...
let from_trades
  ...
  (* Sort trades by time *)
  let sorted_trades = List.sort trades ~compare:(fun a b ->
...
let command : string * Command.t =
  ("ledger", Command.basic ~summary:"Binance ledger (P&L tracking)"
    (Command.Param.return (fun () -> printf "Binance ledger - use `from_trades` or `from_balances` in code\n")))
