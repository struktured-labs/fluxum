(** Binance Ledger - Implements unified Ledger_intf for P&L tracking *)
...
module Entry = struct
      ...
      | false ->
        ...
        let avg_buy_price, avg_sell_price =
          ...
          | Sell ->
            let new_avg_sell =
              ...
              | true ->
                ((t.avg_sell_price *. t.total_sell_qty) +. (price *. qty))
                /. total_sell_qty
              | false -> 0.
            in
        ...
        let avg_price =
          let total_qty = total_buy_qty +. total_sell_qty in
            ...
            | true ->
              ((avg_buy_price *. total_buy_qty) +. (avg_sell_price *. total_sell_qty))
              /. total_qty
            | false -> 0.
        in
        ...
        in
        let running_price =
          match Float.(running_qty > 0.) with
          | true -> cost_basis /. running_qty
          | false -> 0.
        in
          { symbol= t.symbol
          ; spot= price
  ...
  let pipe
        ...
        (trade_events : V3.My_trades.trade Pipe.Reader.t)
    =
    ...
    let combined_pipe =
      match (num_values, behavior) with
             ...
             (match
                ...
                let%bind price =
                  Fluxum.Normalize_common.Float_conv.price_of_string
                    trade.V3.My_trades.price
                in
                let%bind qty =
                  Fluxum.Normalize_common.Float_conv.qty_of_string trade.V3.My_trades.qty
                in
                  Ok (price, qty)
              with
              | Error err ->
              ...
              | Ok (price, qty) ->
                let side =
                  match trade.V3.My_trades.isBuyer with
                ...
                let fee_usd =
                  ...
                  | Ok commission ->
                    ...
                    (match
                       String.is_suffix trade.V3.My_trades.commissionAsset ~suffix:"USD"
                       || String.is_suffix
                            trade.V3.My_trades.commissionAsset
                            ~suffix:"USDT"
                     with
                     | true -> commission
...
let from_balances ?(notional_currency = "USDT") (balances : V3.Account.balance list) : t =
  List.fold balances ~init:Fluxum.Types.Symbol.Map.empty ~f:(fun acc balance ->
    ...
    | Error err ->
      Log.Global.error
        "Binance ledger: Failed to parse balance for %s: %s"
        balance.V3.Account.asset
        err;
      acc
    | Ok position ->
      ...
      (match Float.(position > 0.) with
       ...
       | true ->
         ...
         let symbol = asset_upper ^ notional_str in
...
let from_trades
      ...
      ?avg_trade_prices
      (trades : V3.My_trades.trade list)
  : t * Entry.t Pipe.Reader.t Fluxum.Types.Symbol.Map.t
  =
  (* Sort trades by time *)
  let sorted_trades =
    List.sort trades ~compare:(fun a b ->
      Int64.compare a.V3.My_trades.time b.V3.My_trades.time)
  in
  (* Fold over trades to build ledger and pipes *)
  let symbol_to_entry_and_pipe =
    List.fold sorted_trades ~init:Fluxum.Types.Symbol.Map.empty ~f:(fun acc trade ->
      let symbol = trade.V3.My_trades.symbol in
        (* Parse price and qty *)
        match
          let open Result.Let_syntax in
          let%bind price =
            Fluxum.Normalize_common.Float_conv.price_of_string trade.V3.My_trades.price
          in
          let%bind qty =
            Fluxum.Normalize_common.Float_conv.qty_of_string trade.V3.My_trades.qty
          in
        ...
        | Ok (price, qty) ->
          ...
          let fee_usd =
            ...
            | Ok commission ->
              (match
                 String.is_suffix trade.V3.My_trades.commissionAsset ~suffix:"USD"
                 || String.is_suffix trade.V3.My_trades.commissionAsset ~suffix:"USDT"
               with
               | true -> commission
               | false -> commission *. price)
          in
          let avg_trade_price =
            match avg_trade_prices with
            | Some prices -> Map.find prices symbol
            | None -> None
          in
          (* Get or create entry with pipe *)
          let entry, reader, writer =
            match Map.find acc symbol with
            | Some (e, r, w) -> (e, r, w)
            | None ->
              let init_entry =
                match Map.find init symbol with
                | Some e -> e
          ...
          let updated_entry =
            Entry.on_trade entry ~timestamp ~price ~side ~qty ~fee_usd ?avg_trade_price
          in
            (* Write to pipe *)
...
let update_from_book t ~(book : Order_book.Book.t) =
  ...
  (* For now, update all entries with same book (not ideal) *)
...
let on_order_events t (fills : V3.New_order.fill list) ~symbol:_ =
  List.fold fills ~init:t ~f:(fun acc fill ->
    match
      let open Result.Let_syntax in
      let%bind price =
        Fluxum.Normalize_common.Float_conv.price_of_string fill.V3.New_order.price
      in
      let%bind qty =
        Fluxum.Normalize_common.Float_conv.qty_of_string fill.V3.New_order.qty
      in
      let%bind commission =
        Fluxum.Normalize_common.Float_conv.qty_of_string fill.V3.New_order.commission
      in
    ...
    | Ok (price, qty, commission) ->
      ...
      let fee_usd =
        match
          String.is_suffix fill.V3.New_order.commissionAsset ~suffix:"USD"
          || String.is_suffix fill.V3.New_order.commissionAsset ~suffix:"USDT"
        with
        | true -> commission
        | false -> commission *. price
      in
...
let on_order_event_response t (_response : V3.New_order.response) =
  (* Parse fills from order response and update ledger *)
  (* Need to extract symbol and side from response *)
  on_order_events t _response.V3.New_order.fills ~symbol:_response.V3.New_order.symbol
...
let pipe
      ...
      (trades : V3.My_trades.trade Pipe.Reader.t)
  ...
  Map.fold
    init
    ~init:(return Fluxum.Types.Symbol.Map.empty)
    ~f:(fun ~key:symbol ~data:entry acc_deferred ->
      let%bind acc = acc_deferred in
        ...
        | Some book_pipe ->
          ...
          let symbol_trades =
            Pipe.filter_map trades ~f:(fun trade ->
              match String.equal trade.V3.My_trades.symbol symbol with
              | true -> Some trade
              | false -> None)
          in
          (* Create entry pipe for this symbol *)
          let%bind entry_pipe =
            Entry.pipe ~init:entry ?num_values ?behavior book_pipe symbol_trades
          in
...
let command : string * Command.t =
  ( "ledger"
  , Command.basic
      ~summary:"Binance ledger (P&L tracking)"
      (Command.Param.return (fun () ->
         printf "Binance ledger - use `from_trades` or `from_balances` in code\n")) )
