...
module Stream = struct
  type t =
    ...
    | Depth of
        { symbol: string
        ; levels: int option }
      (* symbol@depth or symbol@depth5/10/20 *)
...
module Message = struct
  ...
  type execution_report =
    ...
    ; execution_type: string
          [@key "x"] (* NEW, CANCELED, REPLACED, REJECTED, TRADE, EXPIRED *)
    ...
    ; cumulative_quote_qty: string
          [@key "Z"] (* Cumulative quote asset transacted quantity *)
  ...
  let sexp_of_t = function
    ...
    | ExecutionReport r ->
      Sexp.List [Sexp.Atom "ExecutionReport"; sexp_of_execution_report r]
    | AccountUpdate u -> Sexp.List [Sexp.Atom "AccountUpdate"; sexp_of_account_update u]
    | BalanceUpdate b ->
      Sexp.List [Sexp.Atom "BalanceUpdate"; sexp_of_balance_update_event b]
