...
module Stream = struct
  ...
  let to_subscription_json = function
    ...
    | L2Book {coin; n_sig_figs; mantissa} ->
      ...
      in
        `Assoc with_mantissa
    | Trades coin -> `Assoc [("type", `String "trades"); ("coin", `String coin)]
    | Candle {coin; interval} ->
      `Assoc
        [ ("type", `String "candle")
        ; ("coin", `String coin)
        ; ("interval", `String (interval_to_string interval)) ]
    | Bbo coin -> `Assoc [("type", `String "bbo"); ("coin", `String coin)]
    | OrderUpdates user ->
      `Assoc [("type", `String "orderUpdates"); ("user", `String user)]
    | UserFills user -> `Assoc [("type", `String "userFills"); ("user", `String user)]
    | UserFundings user ->
      `Assoc [("type", `String "userFundings"); ("user", `String user)]
    | OpenOrders user -> `Assoc [("type", `String "openOrders"); ("user", `String user)]
  ...
  let to_subscribe_message t =
    `Assoc [("method", `String "subscribe"); ("subscription", to_subscription_json t)]

  let to_unsubscribe_message t =
    `Assoc [("method", `String "unsubscribe"); ("subscription", to_subscription_json t)]
end
...
module Message = struct
  ...
  let sexp_of_subscription_response {method_; subscription} =
    Sexp.List
      ...
      ; Sexp.List
          [Sexp.Atom "subscription"; Sexp.Atom (Yojson.Safe.to_string subscription)] ]
  ...
  let sexp_of_t = function
    | SubscriptionResponse resp ->
      Sexp.List [Sexp.Atom "SubscriptionResponse"; sexp_of_subscription_response resp]
    ...
    | Trades trades ->
      Sexp.List [Sexp.Atom "Trades"; Sexp.List (List.map trades ~f:sexp_of_trade)]
    | Bbo bbo -> Sexp.List [Sexp.Atom "Bbo"; sexp_of_bbo bbo]
    | Candle c -> Sexp.List [Sexp.Atom "Candle"; sexp_of_candle c]
    | OrderUpdates updates ->
      Sexp.List
        [Sexp.Atom "OrderUpdates"; Sexp.List (List.map updates ~f:sexp_of_order_update)]
    | UserFills fills ->
      Sexp.List [Sexp.Atom "UserFills"; Sexp.List (List.map fills ~f:sexp_of_user_fill)]
    | Pong -> Sexp.Atom "Pong"
...
let parse_message (msg : string) : Message.t =
  try
    ...
    let channel =
      Yojson.Safe.Util.member "channel" json |> Yojson.Safe.Util.to_string_option
    in
      match channel with
      ...
      | Some "trades" ->
        let data = Yojson.Safe.Util.member "data" json in
          (match data with
           | `List trades ->
             let parsed =
               List.filter_map trades ~f:(fun t ->
      ...
      | Some "orderUpdates" ->
        let data = Yojson.Safe.Util.member "data" json in
          (match data with
           | `List updates ->
             let parsed =
               List.filter_map updates ~f:(fun u ->
