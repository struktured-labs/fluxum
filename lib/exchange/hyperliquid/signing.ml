...
module Domain = struct
  ...
  let domain_separator (domain : t) : string =
    ...
    let combined =
      type_hash ^ name_hash ^ version_hash ^ chain_id_bytes ^ contract_bytes
    in
...
module Address = struct
  ...
  let from_public_key
        (ctx : Secp256k1.Context.t)
        (pubkey : Secp256k1.Key.public Secp256k1.Key.t)
    : string
    =
  ...
  let normalize (addr : string) : string =
    let addr = String.lowercase addr in
      if String.is_prefix addr ~prefix:"0x" then addr else "0x" ^ addr
end
...
module Msgpack = struct
  ...
  let serialize_order_action ~(orders : order_request list) ~grouping : bytes =
    ...
    let order_items =
      List.map orders ~f:(fun (order : order_request) ->
        ...
        let base_fields =
          ...
          ; ( String "t"
            , Map [(String "limit", Map [(String "tif", String order.time_in_force)])] )
          ]
        in
        (* Add cloid if present *)
        let fields_with_cloid =
          match order.cloid with
    ...
    (* Top-level action *)
    let action =
      Map [(String "orders", List order_items); (String "grouping", String grouping)]
    in
  ...
  let serialize_cancel_action ~(cancels : cancel_request list) : bytes =
    let cancel_items =
      List.map cancels ~f:(fun (cancel : cancel_request) ->
        Map [(String "a", Int cancel.asset); (String "o", Int64 cancel.oid)])
    in
    let action = Map [(String "cancels", List cancel_items)] in
      Msgpck.String.to_string action
...
module Signature = struct
  ...
  let sign ~private_key_hex ~digest_hex : (string, string) Result.t =
    ...
    with
    | e -> Error (sprintf "Signing failed: %s" (Exn.to_string e))
end

(** High-level signing interface *)

(** Sign L1 order placement action *)
let sign_place_order ~private_key ~orders ~grouping ~nonce:_ : (string, string) Result.t =
  try
    ...
    let digest =
      Signature.create_digest ~domain_separator:domain_sep ~struct_hash:action_hash
    in
      (* 5. Sign the digest *)
      Signature.sign ~private_key_hex:private_key ~digest_hex:digest
  with
  | e -> Error (sprintf "sign_place_order failed: %s" (Exn.to_string e))

(** Sign L1 cancel action *)
let sign_cancel_order ~private_key ~cancels ~nonce:_ : (string, string) Result.t =
  try
    (* 1. Serialize action to msgpack *)
    ...
    in
      ...
      Signature.sign ~private_key_hex:private_key ~digest_hex:digest
  with
  | e -> Error (sprintf "sign_cancel_order failed: %s" (Exn.to_string e))
...
let sign_withdraw ~private_key ~(req : withdraw_request) : (string, string) Result.t =
  try
    (* 1. Serialize action to msgpack *)
    ...
    in
      ...
      Signature.sign ~private_key_hex:private_key ~digest_hex:digest
  with
  | e -> Error (sprintf "sign_withdraw failed: %s" (Exn.to_string e))
