...
let test_all_mids () =
  ...
  let cfg = Hyperliquid.Cfg.production in
    ...
    | Error err ->
      fail
        (sprintf "Error: %s" (Sexp.to_string_hum (Hyperliquid.Rest.Error.sexp_of_t err)))

let test_l2_book () =
  printf "\n[REST] L2 Order Book\n";
  let cfg = Hyperliquid.Cfg.production in
    Hyperliquid.Rest.l2_book cfg ~coin:"BTC"
    >>| function
    | Ok book ->
      ...
      (match book.levels with
       | [bids; asks] ->
         pass
           (sprintf
              "Bids: %d levels, Asks: %d levels"
              (List.length bids)
              (List.length asks));
         (match List.hd bids with
          | Some best_bid -> pass (sprintf "Best bid: %s @ %s" best_bid.sz best_bid.px)
          | None -> ());
         (match List.hd asks with
          | Some best_ask -> pass (sprintf "Best ask: %s @ %s" best_ask.sz best_ask.px)
          | None -> ())
...
let test_meta_and_asset_ctxs () =
  ...
  let cfg = Hyperliquid.Cfg.production in
    Hyperliquid.Rest.meta_and_asset_ctxs cfg
    >>| function
    | Ok (meta, ctxs) ->
      pass
        (sprintf
           "Universe: %d assets, Contexts: %d"
           (List.length meta.universe)
           (List.length ctxs));
...
let test_recent_trades () =
  printf "\n[REST] Recent Trades\n";
  let cfg = Hyperliquid.Cfg.production in
    Hyperliquid.Rest.recent_trades cfg ~coin:"ETH"
    >>| function
    | Ok trades ->
...
let test_websocket_connection () =
  ...
  let streams = [Hyperliquid.Ws.Stream.AllMids; Hyperliquid.Ws.Stream.Trades "BTC"] in
    Hyperliquid.Ws.connect ~streams ()
    >>= function
    | Error err ->
             ...
             | `Result (`Ok msg) ->
               ...
               let parsed = Hyperliquid.Ws.parse_message msg in
                 (match parsed with
                  ...
                  | Hyperliquid.Ws.Message.Bbo bbo -> pass (sprintf "BBO for %s" bbo.coin)
                  | Hyperliquid.Ws.Message.Pong -> pass "Received pong"
                  | Hyperliquid.Ws.Message.Error err -> fail (sprintf "Error: %s" err)
                  | Hyperliquid.Ws.Message.Unknown _ ->
                    pass "Received message (unknown type)"
                  | _ -> pass "Received other message type");
        ...
        | false ->
          (match%bind
           ...
           | `Timeout ->
             ...
             Hyperliquid.Ws.close ws
           | `Result `Eof -> Hyperliquid.Ws.close ws
           | `Result (`Ok msg) ->
             let parsed = Hyperliquid.Ws.parse_message msg in
               (match parsed with
                | Hyperliquid.Ws.Message.L2Book book ->
...
let test_websocket_invalid_url () =
  ...
  let streams = [Hyperliquid.Ws.Stream.AllMids] in
  ...
  let parsed1 = Hyperliquid.Ws.parse_message malformed_json in
    ...
    let parsed2 = Hyperliquid.Ws.parse_message empty in
      ...
      let parsed3 = Hyperliquid.Ws.parse_message invalid_structure in
        (match parsed3 with
         | Hyperliquid.Ws.Message.Unknown _ ->
           pass "Unknown message for invalid structure"
    ...
    >>= (function
     ...
     | `Result (`Ok msg) ->
       let parsed = Hyperliquid.Ws.parse_message msg in
         (match parsed with
          | Hyperliquid.Ws.Message.SubscriptionResponse _ ->
            pass "Got subscription response"
...
let test_websocket_invalid_coin () =
  ...
  let streams = [Hyperliquid.Ws.Stream.Trades "INVALIDSYMBOL12345"] in
    Hyperliquid.Ws.connect ~streams ()
    >>= function
    | Error err ->
       ...
       | `Result (`Ok msg) ->
         let parsed = Hyperliquid.Ws.parse_message msg in
           (match parsed with
            ...
            | _ -> pass "Exchange accepted invalid coin");
           Hyperliquid.Ws.close ws)
...
let run_tests () =
  ...
  >>= fun () ->
  test_websocket_connection_empty_streams ()
  >>= fun () ->
  test_websocket_invalid_coin ()
  >>= fun () ->
  (* Summary *)
  printf "\n===========================================\n";
