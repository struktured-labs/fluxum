...
let connect ~(streams : Ws.Stream.t list) ?(url = Ws.Endpoint.mainnet) ()
                   ...
                   (match msg_opt with
                    ...
                    | Some payload ->
                      ...
                      (match !receive_count mod 50 = 0 with
                       | true ->
                         Log.Global.info
                           "Hyperliquid: Received %d messages so far"
                           !receive_count
                       | false -> ());
          ...
          let ping_count = ref 0 in
            don't_wait_for
              (let rec ping_loop () =
                 ...
                 | false ->
                   Log.Global.info
                     "Hyperliquid: Ping loop ending (inactive after %d pings)"
                     !ping_count;
                   return ()
                 | true ->
                     ...
                     (match send_result with
                      | Ok () ->
                        (match Float.(duration_ms > 100.0) with
                         | true ->
                           eprintf
                             "Hyperliquid: Ping #%d took %.2f ms%s\n%!"
                             !ping_count
                             duration_ms
                             (match Float.(duration_ms > 1000.0) with
                              | true -> " (WARNING: >1s)"
                              | false -> "")
                         | false -> ())
                      | Error exn ->
                        eprintf
                          "Hyperliquid: Ping #%d failed after %.2f ms: %s\n%!"
                          !ping_count
                          duration_ms
                          (Exn.to_string exn));
