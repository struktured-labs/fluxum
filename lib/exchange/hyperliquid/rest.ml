...
module Types = struct
  ...
  type all_mids = (string * string) list [@@deriving sexp]

  let all_mids_of_yojson json =
    match json with
    | `Assoc pairs ->
      Ok
        (List.map pairs ~f:(fun (coin, price) -> (coin, Yojson.Safe.Util.to_string price)))
  ...
  type meta = {universe: universe_item list} [@@deriving yojson {strict= false}, sexp]

  (** Combined meta and asset contexts *)
  type meta_and_asset_ctxs = meta * asset_ctx list [@@deriving sexp]
  ...
  type candles = candle list [@@deriving sexp]
  ...
  type open_orders = open_order list [@@deriving sexp]
  ...
  type user_fills = user_fill list [@@deriving sexp]
...
let post ~cfg ~body : (Yojson.Safe.t, [> Error.t]) result Deferred.t =
  ...
  let body_str = Yojson.Safe.to_string body in
    Monitor.try_with (fun () ->
      Cohttp_async.Client.post ~headers ~body:(Cohttp_async.Body.of_string body_str) uri
      >>= fun (response, body) ->
      Cohttp_async.Body.to_string body >>| fun body_str -> (response, body_str))
    >>| function
...
let l2_book cfg ~coin : (Types.l2_book, [> Error.t]) result Deferred.t =
  let body = `Assoc [("type", `String "l2Book"); ("coin", `String coin)] in
    post ~cfg ~body
    >>| function
    | Error _ as err -> err
    | Ok json ->
...
let recent_trades cfg ~coin : (Types.recent_trades, [> Error.t]) result Deferred.t =
  let body = `Assoc [("type", `String "recentTrades"); ("coin", `String coin)] in
    post ~cfg ~body
    >>| function
    | Error _ as err -> err
    | Ok json ->
...
let clearinghouse_state cfg ~user
  : (Types.clearinghouse_state, [> Error.t]) result Deferred.t
  =
  let body = `Assoc [("type", `String "clearinghouseState"); ("user", `String user)] in
    post ~cfg ~body
    >>| function
    | Error _ as err -> err
    | Ok json ->
...
let open_orders cfg ~user : (Types.open_orders, [> Error.t]) result Deferred.t =
  let body = `Assoc [("type", `String "openOrders"); ("user", `String user)] in
    post ~cfg ~body
    >>| function
    | Error _ as err -> err
    | Ok json ->
...
let user_fills cfg ~user : (Types.user_fills, [> Error.t]) result Deferred.t =
  let body = `Assoc [("type", `String "userFills"); ("user", `String user)] in
    post ~cfg ~body
    >>| function
    | Error _ as err -> err
    | Ok json ->
            ...
            , `List
                (List.map orders ~f:(fun o ->
                   `Assoc
                     ...
                     ; ("t", `Assoc [("limit", `Assoc [("tif", `String o.time_in_force)])])
                     ; ( "c"
                       , match o.cloid with
                         | Some c -> `String c
                         | None -> `Null ) ])) )
    ...
    | Ok signature ->
      ...
      let action_json =
        `Assoc
          ...
          ; ( "cancels"
            , `List
                (List.map cancels ~f:(fun c ->
                   `Assoc [("a", `Int c.asset); ("o", `Int (Int64.to_int_exn c.oid))])) )
          ]
      in
        post_exchange ~cfg ~action:action_json ~signature ~nonce ()
