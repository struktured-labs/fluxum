(** Hyperliquid Ledger - Implements unified Ledger_intf for P&L tracking *)
...
module Entry = struct
  ...
  let pipe
    ...
    let combined_pipe =
      ...
      | _ -> Pipe.interleave [book_events; trade_events]
    in
    ...
    let entry_ref = ref init in
      don't_wait_for
        (Pipe.iter combined_pipe ~f:(fun event ->
           ...
           | `Trade _trade -> Pipe.write writer !entry_ref)
  ...
  Map.fold
    ...
    ~f:(fun ~key:symbol ~data:entry acc_deferred ->
      let%bind acc = acc_deferred in
        ...
        | Some book_pipe ->
          let%bind entry_pipe =
            Entry.pipe ~init:entry ?num_values ?behavior book_pipe order_events
          in
...
let command : string * Command.t =
  ( "ledger"
  , Command.basic
      ~summary:"Hyperliquid ledger"
      (Command.Param.return (fun () -> printf "Hyperliquid ledger command\n")) )
