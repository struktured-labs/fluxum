...
type metadata = {last_update_time: int64} [@@deriving sexp]
...
module Book = struct
  ...
  let empty ?timestamp:_ ?last_update_time:_ symbol = create ~symbol
  ...
  let apply_l2_book t (book : Ws.Message.l2_book) : (t, string) Result.t =
    ...
    let%bind bid_levels =
      List.fold bids ~init:(Ok []) ~f:(fun acc_result level ->
        ...
        let%bind price =
          Fluxum.Normalize_common.Float_conv.price_of_string level.Ws.Message.px
        in
  ...
  let apply_rest_l2_book t (book : Rest.Types.l2_book) : (t, string) Result.t =
    ...
    let%bind bid_levels =
      List.fold bids ~init:(Ok []) ~f:(fun acc_result level ->
        let%bind acc = acc_result in
        let%bind price =
          Fluxum.Normalize_common.Float_conv.price_of_string level.Rest.Types.px
        in
        let%bind volume = Fluxum.Normalize_common.Float_conv.qty_of_string level.sz in
          match Float.(volume > 0.) with
          | true -> Ok ((`Bid, price, volume) :: acc)
