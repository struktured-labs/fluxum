  ...
  let%bind reserve1 = Fluxum.Normalize_common.Float_conv.qty_of_string pool.reserve1 in
    ...
    | true
      when String.equal token_out pool.token1.id
           || String.equal token_out pool.token1.symbol -> Ok (reserve1 /. reserve0)
    | false
      when (String.equal token_in pool.token1.id
            || String.equal token_in pool.token1.symbol)
           && (String.equal token_out pool.token0.id
               || String.equal token_out pool.token0.symbol) -> Ok (reserve0 /. reserve1)
...
let quote
  ...
  in
    ...
    | true ->
      let params = Amm_pricing.Constant_product.{reserve0; reserve1; fee_bps} in
        Amm_pricing.Constant_product.quote params ~amount_in ~pool_id:pool.id
    | false ->
      (* Swap direction: token1 -> token0 *)
      let params =
        Amm_pricing.Constant_product.{reserve0= reserve1; reserve1= reserve0; fee_bps}
      in
        Amm_pricing.Constant_product.quote params ~amount_in ~pool_id:pool.id
