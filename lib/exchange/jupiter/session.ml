(** Jupiter Session - Implements unified Session_intf with auto-reconnecting streams *)
...
module Events = struct
  ...
  let create ?(symbols : Fluxum.Types.Symbol.t list = []) ?order_ids:_ () : t Deferred.t =
    Log.Global.info "Jupiter Events.create: start symbols=%d" (List.length symbols);
...
let create ?(symbols : Fluxum.Types.Symbol.t list = []) ?order_ids () : t Deferred.t =
  Log.Global.info "Jupiter Session.create: symbols=%d" (List.length symbols);
...
let close t =
  Log.Global.info "Jupiter Session.close: closing session";
