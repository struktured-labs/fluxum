...
module Error = struct
  ...
  let sexp_of_t = function
    ...
    | `Unauthorized -> Sexp.List [Sexp.Atom "Unauthorized"; Sexp.Atom "API key required"]
end
...
module Types = struct
  ...
  let prices_response_of_yojson json =
    ...
    | `Assoc [("data", `Assoc prices); _] | `Assoc [_; ("data", `Assoc prices)] ->
      let parsed =
        List.filter_map prices ~f:(fun (id, price_json) ->
...
let get (module Cfg : Cfg.S) ~path ?params ()
  ...
  | Some api_key ->
    ...
    in
    let headers =
      Cohttp.Header.of_list [("x-api-key", api_key); ("Accept", "application/json")]
    in
...
let prices cfg ~mints : (Types.prices_response, [> Error.t]) result Deferred.t =
  let ids = String.concat ~sep:"," mints in
  let params = [("ids", ids)] in
  (* Price API uses different endpoint *)
  let cfg_with_price_url =
    (module struct
      let api_url = "https://api.jup.ag/price/v2"

      let api_key =
        let (module C : Cfg.S) = cfg in
          C.api_key
    end : Cfg.S)
  in
    get cfg_with_price_url ~path:"" ~params ()
    >>| function
    | Error _ as err -> err
    | Ok json ->
...
let order_book cfg ~base_mint ~quote_mint ~amount
  ...
  let%bind sell_quote =
    quote cfg ~input_mint:base_mint ~output_mint:quote_mint ~amount ()
  in
    ...
    | Ok sq ->
      ...
      in
      let%bind buy_quote =
        quote cfg ~input_mint:quote_mint ~output_mint:base_mint ~amount:quote_amount ()
      in
