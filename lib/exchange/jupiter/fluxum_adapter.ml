...
module Adapter = struct
  ...
  let cancel_order (_ : t) (_ : Native.Order.id) =
    Deferred.return
      (Error (`Api_error "Jupiter trades are atomic swaps, no cancellation"))
  ...
  let get_open_orders (_ : t) ?symbol:_ () =
    Deferred.return
      (Error (`Api_error "Jupiter is a DEX aggregator, no persistent orders"))
  ...
  let get_deposit_address (_ : t) ~currency:_ ?network:_ () =
    Deferred.return
      (Error (`Api_error "Jupiter is a Solana DEX - use wallet address for deposits"))

  let withdraw (_ : t) ~currency:_ ~amount:_ ~address:_ ?tag:_ ?network:_ () =
    Deferred.return
      (Error (`Api_error "Jupiter is a Solana DEX - use Solana wallet for transfers"))

  let get_deposits (_ : t) ?currency:_ ?limit:_ () =
    Deferred.return
      (Error (`Api_error "Jupiter is a Solana DEX - use blockchain explorer"))
  ...
  let get_symbols (_ : t) () =
    ...
    Deferred.return
      (Error (`Api_error "Jupiter supports all SPL tokens, use token list API"))
  ...
  module Normalize = struct
    let order_response (_ : Native.Order.response) : (Types.Order.t, string) Result.t =
      Error
        "Jupiter is a Solana DEX aggregator - use swap/quote APIs instead of order \
         tracking"
    ...
    let trade (_ : Native.Trade.t) : (Types.Trade.t, string) Result.t =
      Error
        "Jupiter is a Solana DEX aggregator - use swap/quote APIs instead of trade \
         tracking"

    let balance (_ : Native.Balance.t) : (Types.Balance.t, string) Result.t =
      Error
        "Jupiter is a Solana DEX aggregator - balances are on-chain, use Solana wallet \
         APIs"
    ...
    let ticker (quote : Native.Ticker.t) : (Types.Ticker.t, string) Result.t =
      ...
      let%bind in_amount =
        Fluxum.Normalize_common.Float_conv.qty_of_string quote.inAmount
      in
      let%bind out_amount =
    ...
    let order_book ((sell_quote, buy_quote) : Native.Book.snapshot)
      ...
      let open Result.Let_syntax in
      let%bind sell_out =
        Fluxum.Normalize_common.Float_conv.qty_of_string sell_quote.outAmount
      in
      let%bind sell_in =
        Fluxum.Normalize_common.Float_conv.qty_of_string sell_quote.inAmount
      in
      let%bind buy_in =
        Fluxum.Normalize_common.Float_conv.qty_of_string buy_quote.inAmount
      in
      let%bind buy_out =
