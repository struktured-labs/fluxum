...
module Book = struct
  ...
  let apply_book_update t (update : Yojson.Safe.t) : t =
    try
      ...
      let safe_to_list json =
        ...
        | other ->
          Log.Global.debug
            "Jupiter WS: Expected list for bids/asks, got: %s"
    ...
    | exn ->
      Log.Global.error
        "Jupiter WS: Exception in apply_book_update: %s"
        (Exn.to_string exn);
      t
  ...
  let pipe ~symbol ?(depth = 10) () : (t, string) Result.t Pipe.Reader.t Deferred.t =
    ignore depth;
    Log.Global.info
      "Jupiter order book pipe for %s (placeholder - WebSocket not yet implemented)"
      symbol;
