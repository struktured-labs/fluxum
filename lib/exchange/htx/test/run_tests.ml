...
let test_cfg () =
  ...
  test_assert "Production has no API key" (Option.is_none Htx.Cfg.production.api_key);
  let cfg =
    Htx.Cfg.with_auth ~api_key:"test_key" ~api_secret:"test_secret" Htx.Cfg.production
  in
    test_assert
