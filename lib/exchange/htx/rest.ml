...
let make_auth_request ~cfg ~method_ ~endpoint ~params =
  ...
  let sorted_params =
    List.sort params ~compare:(fun (k1, _) (k2, _) -> String.compare k1 k2)
  in
  let all_params =
    ("AccessKeyId", api_key)
  ...
  let signature_payload =
    String.concat ~sep:"\n" [method_; "api.htx.com"; endpoint; query_string]
  in
...
let ticker ~cfg ~symbol =
  let open Deferred.Result.Let_syntax in
  let%bind json_str =
    make_public_request ~cfg ~endpoint:(sprintf "/market/detail/merged?symbol=%s" symbol)
  in
    match Yojson.Safe.from_string json_str with
    | `Assoc [("status", `String "ok"); ("tick", tick)] ->
      let tick_with_symbol =
        match tick with
...
let order_book ~cfg ~symbol =
  ...
  let%bind json_str =
    make_public_request
      ~cfg
      ~endpoint:(sprintf "/market/depth?symbol=%s&type=step0" symbol)
  in
    match Yojson.Safe.from_string json_str with
    | `Assoc [("status", `String "ok"); ("tick", tick)] ->
...
let balances ~cfg =
  let open Deferred.Result.Let_syntax in
  let%bind json_str =
    make_auth_request ~cfg ~method_:"GET" ~endpoint:"/v1/account/accounts" ~params:[]
  in
    match Yojson.Safe.from_string json_str with
