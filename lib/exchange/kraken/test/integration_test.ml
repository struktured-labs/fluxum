...
let test_ticker () =
  ...
  let cfg = Kraken.Cfg.of_string "production" in
    Kraken.V1.Ticker.get cfg {pair= "XETHZUSD"}
    >>| function
    | `Ok tickers ->
...
let test_depth () =
  printf "\n[V1] Depth (Order Book)\n";
  let cfg = Kraken.Cfg.of_string "production" in
    Kraken.V1.Depth.get cfg {pair= "XETHZUSD"; count= Some 5}
    >>| function
    | `Ok depth_list ->
...
let test_recent_trades () =
  printf "\n[V1] Recent Trades\n";
  let cfg = Kraken.Cfg.of_string "production" in
    Kraken.V1.Recent_trades.get cfg {pair= "XETHZUSD"; since= None; count= Some 5}
    >>| function
    | `Ok (trades_list, last) ->
...
let test_asset_pairs () =
  ...
  let cfg = Kraken.Cfg.of_string "production" in
    ...
    Kraken.V1.Asset_pairs.post cfg {pair= Some "XETHZUSD"; info= None}
    >>| function
    | `Ok pairs ->
...
let test_adapter_ticker () =
  ...
  let adapter = Kraken.Fluxum_adapter.Adapter.create ~cfg ~symbols:[] () in
    Kraken.Fluxum_adapter.Adapter.get_ticker adapter ~symbol:"XETHZUSD" ()
    >>| function
    | Ok ticker_native ->
...
let test_adapter_order_book () =
  ...
  let cfg = Kraken.Cfg.of_string "production" in
  let adapter = Kraken.Fluxum_adapter.Adapter.create ~cfg ~symbols:[] () in
    Kraken.Fluxum_adapter.Adapter.get_order_book adapter ~symbol:"XETHZUSD" ~limit:5 ()
    >>| function
    | Ok book_native ->
...
let test_adapter_recent_trades () =
  ...
  let cfg = Kraken.Cfg.of_string "production" in
  let adapter = Kraken.Fluxum_adapter.Adapter.create ~cfg ~symbols:[] () in
    Kraken.Fluxum_adapter.Adapter.get_recent_trades adapter ~symbol:"XETHZUSD" ~limit:5 ()
    >>| function
    | Ok trades_native ->
      ...
      in
      let trades =
        List.filter_map trades_results ~f:(fun r ->
          match r with
          | Ok trade -> Some trade
          | Error msg ->
      ...
      in
        ...
        let eth =
          List.find symbols ~f:(fun s ->
            ...
            && String.is_substring s.symbol ~substring:"USD")
        in
          (match eth with
           | Some info ->
             pass
    ...
    | Error err ->
      ...
      let e = Kraken.Fluxum_adapter.Adapter.Normalize.error err in
        (match e with
         ...
         | _ ->
           fail
             (sprintf "Error: %s" (Sexp.to_string_hum (Fluxum.Types.Error.sexp_of_t e))))
