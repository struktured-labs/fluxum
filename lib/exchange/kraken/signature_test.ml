...
let test_hmac_sha512 () =
  ...
  let signature = Kraken.Signature.hmac_sha512 ~secret ~message in
    ...
    | false ->
      printf "HMAC-SHA512 test passed (output length: %d)\n" (String.length signature)

let test_kraken_signature () =
  (* Test with synthetic data *)
  let api_secret_b64 = "YmFzZTY0ZW5jb2RlZHNlY3JldGtleQ==" in
  (* "base64encodedsecretkey" in base64 *)
  ...
  let signature =
    match
      Kraken.Signature.kraken_signature ~api_secret_b64 ~api_path ~nonce ~post_data
    with
    | Ok s -> s
  ...
  in
    ...
    | false ->
      (match
       ...
       | true ->
         printf
           "Kraken signature test passed (signature length: %d)\n"
           (String.length signature)
       | false ->
