...
module Book = struct
        ...
        match
          let%bind price =
            Fluxum.Normalize_common.Float_conv.price_of_string
              level.Ws.Public.Price_level.price
          in
          let%bind volume =
            Fluxum.Normalize_common.Float_conv.qty_of_string
              level.Ws.Public.Price_level.volume
          in
            Ok (price, volume)
        with
  ...
  let pipe ~symbol ?(depth = 10) () : (t, string) Result.t Pipe.Reader.t Deferred.t =
    ...
    let%bind market_data_result =
      Market_data.connect
        ~subscriptions:
          [{channel= "book"; pairs= [symbol]; interval= None; depth= Some depth}]
        ()
    in
            ...
            (Pipe.iter messages ~f:(fun msg_str ->
               try
                 ...
                 | Ok (Ws.Public (Ws.Public.Book book_data))
                   when String.equal book_data.pair symbol ->
                 ...
                 | Error err ->
                   Pipe.write
                     book_writer
                     (Error (Sexp.to_string (Ws.Error.sexp_of_t err)))
               with
               | exn -> Pipe.write book_writer (Error (Exn.to_string exn)))
