  ...
  let auth_message ~signature : string =
    Yojson.Safe.to_string
      (`Assoc
          ...
          ; ( "subscription"
            , `Assoc [("name", `String "own-orders"); ("token", `String signature)] ) ])
  ...
  let auth_trades_message ~signature : string =
    Yojson.Safe.to_string
      (`Assoc
          [ ("event", `String "subscribe")
          ; ( "subscription"
            , `Assoc [("name", `String "ownTrades"); ("token", `String signature)] ) ])
  ...
  let subscribe_ticker pairs : string =
    Yojson.Safe.to_string
      (`Assoc
          ...
          ; ("subscription", `Assoc [("name", `String "ticker")]) ])
  ...
  let subscribe_spread pairs : string =
    Yojson.Safe.to_string
      (`Assoc
          [ ("event", `String "subscribe")
          ; ("pair", `List (List.map pairs ~f:(fun p -> `String p)))
          ; ("subscription", `Assoc [("name", `String "spread")]) ])
  ...
  let subscribe_trade pairs : string =
    Yojson.Safe.to_string
      (`Assoc
          [ ("event", `String "subscribe")
          ; ("pair", `List (List.map pairs ~f:(fun p -> `String p)))
          ; ("subscription", `Assoc [("name", `String "trade")]) ])
  ...
  let subscribe_ohlc ?(interval = 60) pairs : string =
    Yojson.Safe.to_string
      (`Assoc
          ...
          ; ("pair", `List (List.map pairs ~f:(fun p -> `String p)))
          ; ( "subscription"
            , `Assoc [("name", `String "ohlc"); ("interval", `Int interval)] ) ])
  ...
  let subscribe_book ?(depth = 10) pairs : string =
    Yojson.Safe.to_string
      (`Assoc
          [ ("event", `String "subscribe")
          ; ("pair", `List (List.map pairs ~f:(fun p -> `String p)))
          ; ("subscription", `Assoc [("name", `String "book"); ("depth", `Int depth)]) ])
  ...
  let unsubscribe channel pairs : string =
    Yojson.Safe.to_string
      (`Assoc
          [ ("event", `String "unsubscribe")
          ; ("pair", `List (List.map pairs ~f:(fun p -> `String p)))
          ; ("subscription", `Assoc [("name", `String channel)]) ])
  ...
  module V2 = struct
    ...
    let normalize_symbol s =
      ...
      | false ->
        ...
        let s =
          match String.is_prefix s ~prefix:"X" && String.length s > 4 with
      ...
      in
        ...
        (match data_json with
         | `Assoc fields ->
           ...
           let fm =
             List.fold fields ~init:String.Map.empty ~f:(fun acc (k, v) ->
               Map.set acc ~key:k ~data:v)
           in
             (* Check for book update fields *)
             (match
                (Map.find fm "a", Map.find fm "b", Map.find fm "as", Map.find fm "bs")
              with
              | Some asks_json, _, _, _ | _, _, Some asks_json, _ ->
                ...
                let bids_json =
                  Map.find fm "b"
                  |> Option.value
                       ~default:(Map.find fm "bs" |> Option.value ~default:(`List []))
                in
                let checksum =
                  Map.find fm "c"
         ...
         | _ -> Error (`Channel_parse_error "Unable to determine message type"))
    | _ ->
      Error
        (`Json_parse_error
            "Expected array message with [channelID, data, channelName, pair]")
end
        ...
        (match Private.parse_message json with
         ...
         | Error _ ->
           ...
           (match Public.parse_message json with
            ...
            | Error _ -> Ok (Raw json)))
  with
  | e -> Error (`Json_parse_error (Exn.to_string e))
...
module Client = struct
  ...
  let create_public () : t =
    {endpoint= Endpoint.public_url; authenticated= false; active= true; subscriptions= []}

  (** Create a new private client *)
  let create_private () : t =
    {endpoint= Endpoint.private_url; authenticated= true; active= true; subscriptions= []}
  ...
  let ping () : string = Yojson.Safe.to_string (`Assoc [("event", `String "ping")])

  (** Generate pong message *)
  let pong () : string = Yojson.Safe.to_string (`Assoc [("event", `String "pong")])

  (** Close the connection *)
  let close t : string =
    t.active <- false;
    Yojson.Safe.to_string (`Assoc [("event", `String "closeConnection")])
end
...
let create ~endpoint : t =
  ...
  in
    {client; handlers= []; message_count= 0}

(** Register a message handler *)
let on_message ws (handler : handler) : unit = ws.handlers <- handler :: ws.handlers
...
let process_message ws (msg : string) : unit Deferred.t =
  ...
  | Ok parsed ->
    Deferred.all_unit (List.map ws.handlers ~f:(fun handler -> handler parsed))
  | Error _ ->
...
let connect endpoint : (t, Error.t) Result.t Deferred.t =
  let _ws = create ~endpoint in
    ...
    Deferred.return
      (Error
         (`Connection_error
             "WebSocket client requires async_websocket library. Use message functions \
              to build client."))
