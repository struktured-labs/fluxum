...
module New = struct
  ...
  let command : string * Command.t =
    ...
    , Command.async
        ...
        Command.Param.(
          let pair =
            flag "--pair" (required string) ~doc:"STRING trading pair (e.g., XETHZUSD)"
          and side = flag "--side" (required string) ~doc:"STRING buy or sell"
          and ordertype =
            flag "--type" (required string) ~doc:"STRING market, limit, etc."
          and volume = flag "--volume" (required float) ~doc:"FLOAT order volume"
          and price =
            flag "--price" (optional string) ~doc:"STRING order price (for limit orders)"
          and cfg =
        ...
        Command.Param.(
          ...
          in
            return (fun txid cfg () ->
              ...
              let cfg = Cfg.of_string cfg_env in
              let module Cfg = (val cfg : Cfg.S) in
              V1.cancel_order (module Cfg) ~txid ()
              >>= function
              | `Ok json ->
        ...
        Command.Param.(
          ...
          in
            return (fun trades cfg () ->
              ...
              let cfg = Cfg.of_string cfg_env in
              let module Cfg = (val cfg : Cfg.S) in
              V1.open_orders ~trades (module Cfg) ()
              >>= function
              | `Ok json ->
                printf "Open Orders:\n%s\n" (Yojson.Safe.pretty_to_string json);
                Deferred.unit
