  ...
  let command : string * Command.t =
    ...
    , Command.async
        ...
        Command.Param.(
          ...
          and depth =
            flag "--depth" (optional int) ~doc:"INT order book depth (default 10)"
          and is_private =
            flag "--private" no_arg ~doc:"Use private channel (for own-orders)"
          in
            return (fun channel pairs interval depth is_private () ->
              (match is_private with
               | true ->
                 printf
                   "Private channel subscription would connect to: %s\n"
                   Ws.Endpoint.private_url
               | false ->
                 printf
                   "Public channel subscription would connect to: %s\n"
              ...
              let subscription_msg =
                ...
                | "own-orders" -> Ws.Private.auth_message ~signature:"<your-signature>"
                | _ -> "unknown channel"
              in
                printf "\nSubscription message:\n%s\n" subscription_msg;
                printf
                  "\n\
                   (WebSocket connection would be established with async_websocket \
                   library)\n";
                Deferred.unit)
            <*> channel
            <*> pairs
                  ...
                  | true ->
                    ...
                    in
                      Yojson.Safe.to_string
                        (`Assoc [("method", `String "subscribe"); ("params", params)])
                  | false ->
                    (match channel with
  ...
  let command : string * Command.t =
    ...
    , Command.async
        ...
        (Command.Param.return (fun () ->
           ...
           printf
             "  ohlc     - Candlestick OHLC data (configurable interval, default 60s)\n";
           ...
           printf
             "  fluxum kraken ws subscribe --channel ticker --pairs XETHZUSD,XDGUSD\n";
           printf "  # Watch your orders in real-time\n";
           printf "  fluxum kraken ws subscribe --channel own-orders --private\n";
           printf "  # 5-minute candles\n";
           printf
             "  fluxum kraken ws subscribe --channel ohlc --pairs XETHZUSD --interval 300\n";
           printf "  # Order book with 25 levels\n";
           printf
             "  fluxum kraken ws subscribe --channel book --pairs XETHZUSD --depth 25\n";
           Deferred.unit)) )
end
