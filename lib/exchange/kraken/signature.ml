...
let base64_encode data = Base64.encode_exn data
let base64_decode data = Base64.decode data
...
let hmac_sha512 ~secret ~message =
  ...
  let inner_hash =
    Digestif.SHA512.digest_string inner_msg |> Digestif.SHA512.to_raw_string
  in
...
let kraken_signature ~api_secret_b64 ~api_path ~nonce ~post_data =
  ...
  let msg_hash =
    Digestif.SHA256.digest_string msg_content |> Digestif.SHA256.to_raw_string
  in
