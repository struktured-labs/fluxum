...
*)
type ticker = float list [@@deriving yojson, sexp]

(** Order book entry [PRICE, COUNT, AMOUNT] *)
type order_book_entry = float * int * float [@@deriving sexp]
...
type order_book = order_book_entry list [@@deriving yojson, sexp]

(** Trade [ID, MTS, AMOUNT, PRICE] *)
type trade = int64 * int64 * float * float [@@deriving sexp]
...
let trade_to_yojson (id, mts, amount, price) =
  `List
    [`Int (Int64.to_int_exn id); `Int (Int64.to_int_exn mts); `Float amount; `Float price]

type trades = trade list [@@deriving yojson, sexp]

(** Wallet [WALLET_TYPE, CURRENCY, BALANCE, UNSETTLED_INTEREST, BALANCE_AVAILABLE] *)
type wallet = string * string * float * float * float option [@@deriving sexp]

let wallet_of_yojson = function
  | `List [`String wtype; `String currency; `Float balance; `Float unsettled; bal_avail]
    ->
    let avail =
      match bal_avail with
      | `Float f -> Some f
      | `Null -> None
      | _ -> None
    in
...
type wallets = wallet list [@@deriving yojson, sexp]
