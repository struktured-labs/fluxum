...
let test_cfg () =
  ...
  test_assert "Production has no API key" (Option.is_none Bitfinex.Cfg.production.api_key);
  let cfg =
    Bitfinex.Cfg.with_auth
      ~api_key:"test_key"
  ...
  in
    ...
    let entry_json = `List [`Float 50000.0; `Int 5; `Float 1.5] in
      ...
      in
        ...
        let trade_json =
          `List [`Int 123456; `Int 1699000000; `Float 0.5; `Float 50000.0]
        in
          ...
          let wallet_json =
            `List [`String "exchange"; `String "BTC"; `Float 10.5; `Float 0.0; `Float 10.5]
          in
...
let test_rest_signature () =
  ...
  let nonce2 = Bitfinex.Rest.generate_nonce () in
    test_assert "Nonce is numeric" (String.for_all nonce1 ~f:Char.is_digit);
    test_assert "Nonces increase" (Int.of_string nonce2 >= Int.of_string nonce1);
    let sig1 =
      Bitfinex.Rest.create_signature
        ~api_secret:"test_secret"
    ...
    in
      ...
      test_assert "Signature is 128 chars (SHA512 fallback)" (String.length sig1 = 128)
