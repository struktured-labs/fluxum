...
module Config = struct
  ...
  let default_metadata () = {message_id= 0; connection_id= ""}
end
...
module Book = struct
  ...
  let update_from_message book (data : Ws.channel_data) (contents : Ws.orderbook_contents)
    ...
    (* Parse bids *)
    let bid_updates =
      List.filter_map contents.bids ~f:(fun level ->
        match
    ...
    (* Parse asks *)
    let ask_updates =
      List.filter_map contents.asks ~f:(fun level ->
        match
          let open Result.Let_syntax in
          let%bind price =
            Fluxum.Normalize_common.Float_conv.price_of_string level.price
    ...
    let metadata : Config.metadata =
      {message_id= data.message_id; connection_id= data.connection_id}
    in
      (* Apply all updates *)
  ...
  let pipe (module Cfg : Cfg.S) ~symbol () =
    ...
    let book_pipe =
      Pipe.folding_map message_pipe ~init:init_book ~f:(fun book json ->
  ...
  let pipe_exn cfg ~symbol () = pipe cfg ~symbol ()
end
