...
let test_normalize_trade_valid () =
  ...
  in
    ...
    | Error msg -> fail (sprintf "Trade normalization failed: %s" msg)
...
let test_normalize_trade_zero_size () =
  ...
  in
    ...
    | Error msg -> fail (sprintf "Should accept zero-size trade: %s" msg)
...
let test_normalize_trade_invalid_side () =
  ...
  in
    ...
    | Ok t ->
      (match t.side with
       | Fluxum.Types.Side.Buy -> pass "Invalid side defaulted to Buy"
       | _ -> fail "Invalid side should default to Buy")
    | Error msg -> fail (sprintf "Should accept with default side: %s" msg)

let test_normalize_order_book_valid () =
  printf "\n[Normalize] Order book with valid data\n";
  let book : Dydx.Rest.Types.orderbook =
    { bids= [{price= "50000.0"; size= "1.5"}; {price= "49999.0"; size= "2.0"}]
    ; asks= [{price= "50001.0"; size= "1.0"}; {price= "50002.0"; size= "3.0"}] }
  in
    match Dydx.Fluxum_adapter.Adapter.Normalize.order_book book with
    | Ok b ->
      ignore
        (assert_string_equal "2 bids" (sprintf "%d bids" (List.length b.bids)) "2 bids");
      ignore
        (assert_string_equal "2 asks" (sprintf "%d asks" (List.length b.asks)) "2 asks");
      pass "Valid order book normalized"
    | Error msg -> fail (sprintf "Order book normalization failed: %s" msg)
...
let test_normalize_order_book_empty () =
  ...
  let book : Dydx.Rest.Types.orderbook = {bids= []; asks= []} in
    match Dydx.Fluxum_adapter.Adapter.Normalize.order_book book with
    | Ok b ->
      ignore
        (assert_string_equal
           "0 bids"
           (sprintf "%d bids" (List.length b.bids))
           "Empty bids");
      ignore
        (assert_string_equal
           "0 asks"
           (sprintf "%d asks" (List.length b.asks))
           "Empty asks");
      pass "Empty order book accepted"
    | Error msg -> fail (sprintf "Empty order book should be valid: %s" msg)
...
let test_normalize_symbol_info_valid () =
  ...
  in
    ...
    | Error msg -> fail (sprintf "Symbol info normalization failed: %s" msg)
...
let test_normalize_ticker_valid () =
  ...
  in
    ...
    | Error msg -> fail (sprintf "Ticker normalization failed: %s" msg)
...
let test_normalize_ticker_missing_optional_fields () =
  ...
  in
    ...
    | Error msg -> fail (sprintf "Should handle missing optional fields: %s" msg)
...
let test_normalize_public_trade_valid () =
  ...
  in
    ...
    | Error msg -> fail (sprintf "Public trade normalization failed: %s" msg)
