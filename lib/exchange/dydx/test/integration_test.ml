...
let test_markets () =
  ...
  Dydx.Rest.markets (module Dydx.Cfg.Production) ()
  >>| function
  | Ok resp ->
         ...
         (match List.hd resp.markets with
          | Some (ticker, market) ->
            ...
            let quote = Dydx.Rest.Types.quote_asset_of_ticker ticker in
              ...
              Option.iter market.volume24H ~f:(fun v -> pass (sprintf "24h volume: %s" v))
          | None -> ())
...
let test_orderbook () =
  ...
  Dydx.Rest.orderbook (module Dydx.Cfg.Production) ~market:"BTC-USD"
  >>| function
  | Ok book ->
...
let test_trades () =
  ...
  Dydx.Rest.trades (module Dydx.Cfg.Production) ~market:"BTC-USD" ~limit:5 ()
  >>| function
  | Ok trades ->
...
let test_candles () =
  ...
  Dydx.Rest.candles
    (module Dydx.Cfg.Production)
    ~market:"ETH-USD"
    ~resolution:Dydx.Common.ONE_HOUR
    ~limit:5
    ()
  >>| function
  | Ok candles ->
...
let test_ticker () =
  printf "\n[REST] Ticker (SOL-USD)\n";
  Dydx.Rest.ticker (module Dydx.Cfg.Production) ~market:"SOL-USD"
  >>| function
  | Ok (Some (ticker, market)) ->
    pass (sprintf "Ticker: %s" ticker);
    pass (sprintf "Status: %s" market.status);
    Option.iter market.oraclePrice ~f:(fun p -> pass (sprintf "Oracle price: %s" p));
    Option.iter market.priceChange24H ~f:(fun c -> pass (sprintf "24h change: %s" c));
    Option.iter market.volume24H ~f:(fun v -> pass (sprintf "24h volume: %s" v))
  | Ok None -> fail "Ticker not found"
  | Error err ->
    fail (sprintf "Error: %s" (Sexp.to_string_hum (Dydx.Rest.Error.sexp_of_t err)))
...
let test_adapter_symbols () =
  printf "\n[Adapter] get_symbols\n";
  let adapter =
    Dydx.Fluxum_adapter.Adapter.create ~cfg:(module Dydx.Cfg.Production) ~symbols:[] ()
  in
    Dydx.Fluxum_adapter.Adapter.get_symbols adapter ()
    >>| function
    | Ok symbols ->
      pass (sprintf "Total symbols: %d" (List.length symbols));
      (* Find BTC-USD *)
      let btc = List.find symbols ~f:(fun (ticker, _) -> String.equal ticker "BTC-USD") in
        (match btc with
         | Some (ticker, market) ->
...
let test_adapter_ticker () =
  ...
  let adapter =
    Dydx.Fluxum_adapter.Adapter.create ~cfg:(module Dydx.Cfg.Production) ~symbols:[] ()
  in
    Dydx.Fluxum_adapter.Adapter.get_ticker adapter ~symbol:"ETH-USD" ()
    >>| function
    | Ok ticker_native ->
...
let test_adapter_order_book () =
  ...
  let adapter =
    Dydx.Fluxum_adapter.Adapter.create ~cfg:(module Dydx.Cfg.Production) ~symbols:[] ()
  in
    Dydx.Fluxum_adapter.Adapter.get_order_book adapter ~symbol:"BTC-USD" ()
    >>| function
    | Ok book_native ->
...
let test_adapter_recent_trades () =
  ...
  let adapter =
    Dydx.Fluxum_adapter.Adapter.create ~cfg:(module Dydx.Cfg.Production) ~symbols:[] ()
  in
    Dydx.Fluxum_adapter.Adapter.get_recent_trades adapter ~symbol:"ETH-USD" ~limit:5 ()
    >>| function
    | Ok trades_native ->
      let trades =
        List.filter_map trades_native ~f:(fun t ->
...
let test_subaccount () =
  ...
  | Error `Not_found -> pass "Subaccount not found (may not exist)"
  | Error err ->
    fail (sprintf "Error: %s" (Sexp.to_string_hum (Dydx.Rest.Error.sexp_of_t err)))
  ...
  | Ok deposits ->
    ...
    (match List.hd deposits with
     | Some tr ->
       ...
       Option.iter tr.transactionHash ~f:(fun h -> pass (sprintf "Tx hash: %s" h))
...
let test_adapter_account_ops () =
  ...
  let adapter =
    Dydx.Fluxum_adapter.Adapter.create
  ...
  in
    ...
    Dydx.Fluxum_adapter.Adapter.get_deposit_address adapter ~currency:"USDC" ()
    >>= fun result ->
    (match result with
     | Ok addr_native ->
     ...
     | Error err ->
       let e = Dydx.Fluxum_adapter.Adapter.Normalize.error err in
         fail (sprintf "Error: %s" (Sexp.to_string_hum (Fluxum.Types.Error.sexp_of_t e))));
    (* Test get deposits *)
    Dydx.Fluxum_adapter.Adapter.get_deposits adapter ~limit:3 ()
    >>= fun result ->
    (match result with
     | Ok deposits_native ->
        ...
        | Some d ->
          (match Dydx.Fluxum_adapter.Adapter.Normalize.deposit d with
           | Ok deposit ->
             pass
               (sprintf
                  "Normalized deposit: %.4f %s (status: %s)"
                  deposit.amount
                  deposit.currency
    ...
    Dydx.Fluxum_adapter.Adapter.get_withdrawals adapter ~limit:3 ()
    >>| fun result ->
    match result with
       ...
       | Some w ->
         (match Dydx.Fluxum_adapter.Adapter.Normalize.withdrawal w with
          | Ok withdrawal ->
            pass
              (sprintf
                 "Normalized withdrawal: %.4f %s (status: %s)"
                 withdrawal.amount
                 withdrawal.currency
