...
module Error = struct
  ...
  let sexp_of_t = function
    ...
    | `Not_found -> Sexp.List [Sexp.Atom "Not_found"]
end
...
module Types = struct
  ...
  (** Trades response *)
  type trades_response = {trades: trade list} [@@deriving yojson {strict= false}, sexp]
  ...
  type markets_response = {markets: (string * perpetual_market) list} [@@deriving sexp]

  let markets_response_of_yojson json =
    match json with
    | `Assoc [("markets", `Assoc markets)] ->
      let parsed =
        List.filter_map markets ~f:(fun (ticker, market_json) ->
...
let get (module Cfg : Cfg.S) ~path ?params ()
...
let markets cfg ?market () : (Types.markets_response, [> Error.t]) result Deferred.t =
  ...
  in
    get cfg ~path:"/v4/perpetualMarkets" ?params ()
    >>| function
    | Error _ as err -> err
    | Ok json ->
...
let candles cfg ~market ~resolution ?limit ()
  : (Types.candle list, [> Error.t]) result Deferred.t
  =
...
let ticker cfg ~market
  : ((string * Types.perpetual_market) option, [> Error.t]) result Deferred.t
  =
  markets cfg ~market ()
  >>| function
  | Error _ as err -> err
...
module Account_types = struct
  ...
  type transfers_response = {transfers: transfer list}
  [@@deriving yojson {strict= false}, sexp]
  ...
  (** Subaccount response *)
  type subaccount_response = {subaccount: subaccount}
  [@@deriving yojson {strict= false}, sexp]

  (** List of subaccounts response *)
  type subaccounts_response = {subaccounts: subaccount list}
  [@@deriving yojson {strict= false}, sexp]
end
...
let transfers cfg ~address ~subaccount_number ?limit ?created_before_or_at_height ()
  ...
  let params =
    [("address", address); ("subaccountNumber", Int.to_string subaccount_number)]
  in
  let params =
    match limit with
    | None -> params
  ...
  in
  let params =
    match created_before_or_at_height with
    | None -> params
...
let deposits cfg ~address ~subaccount_number ?limit ()
  : (Account_types.transfer list, [> Error.t]) result Deferred.t
  =
  transfers cfg ~address ~subaccount_number ?limit ()
  >>| function
  | Error _ as err -> err
  | Ok all_transfers ->
    Ok
      (List.filter all_transfers ~f:(fun t ->
...
let parent_transfers cfg ~address ?limit ()
  ...
  let params = [("address", address); ("parentSubaccountNumber", "0")] in
  let params =
    match limit with
    | None -> params
    | Some l -> ("limit", Int.to_string l) :: params
...
let asset_positions cfg ~address ~subaccount_number
  ...
  =
  subaccount cfg ~address ~subaccount_number
  >>| function
  | Error _ as err -> err
