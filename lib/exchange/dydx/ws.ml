...
type trades_contents = {trades: trade list} [@@deriving sexp, yojson {strict= false}]
...
let subscribe ~conn ~channel ~id ?(batched = true) () =
  let req = {type_= "subscribe"; channel; id= Some id; batched} in
...
let unsubscribe ~conn ~channel ~id () =
  let req = {type_= "unsubscribe"; channel; id= Some id; batched= false} in
  let json = subscription_request_to_yojson req in
  let msg = Yojson.Safe.to_string json in
    Websocket_curl.send conn msg
...
module Trades = struct
  ...
  let unsubscribe ~conn ~market () = unsubscribe ~conn ~channel:"v4_trades" ~id:market ()
...
module Markets = struct
  let subscribe ~conn () = subscribe ~conn ~channel:"v4_markets" ~id:"" ~batched:true ()
  let unsubscribe ~conn () = unsubscribe ~conn ~channel:"v4_markets" ~id:"" ()
end
