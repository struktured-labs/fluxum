...
module Adapter = struct
  ...
  let place_order (_ : t) (_ : Native.Order.request) =
    Deferred.return
      (Error
         (`Api_error "Trading requires wallet signing - use dYdX TypeScript/Python SDK"))
  ...
  let balances (t : t) =
    ...
    | None ->
      Deferred.return
        (Error (`Api_error "Balances require address - create adapter with ~address"))
    | Some address ->
      let cfg = t.cfg in
  ...
  let get_order_book (t : t) ~symbol ?limit:_ () =
    let cfg = t.cfg in
      Rest.orderbook cfg ~market:symbol
      >>| function
      | Error _ as err -> err
      | Ok book -> Ok book
  ...
  let get_recent_trades (t : t) ~symbol ?limit () =
    let cfg = t.cfg in
      Rest.trades cfg ~market:symbol ?limit ()
      >>| function
      | Error _ as err -> err
      | Ok trades -> Ok trades
  ...
  let get_deposit_address (t : t) ~currency:_ ?network:_ () =
    ...
    | None ->
      Deferred.return
        (Error
           (`Api_error "Deposit address requires address - create adapter with ~address"))
    | Some address ->
      Deferred.return
        (Ok {Native.Deposit_address.address; subaccount_number= t.subaccount_number})
  ...
  let get_subaccount (t : t) () =
    ...
    | None ->
      Deferred.return
        (Error
           (`Api_error "Subaccount query requires address - create adapter with ~address"))
    | Some address ->
      let cfg = t.cfg in
        Rest.subaccount cfg ~address ~subaccount_number:t.subaccount_number
  ...
  let get_subaccounts (t : t) () =
    ...
    | None ->
      Deferred.return
        (Error
           (`Api_error "Subaccounts query requires address - create adapter with ~address"))
    | Some address ->
      let cfg = t.cfg in
  ...
  let withdraw (_ : t) ~currency:_ ~amount:_ ~address:_ ?tag:_ ?network:_ () =
    Deferred.return
      (Error
         (`Api_error
             "dYdX withdrawals require Cosmos chain signing - use dYdX TypeScript/Python \
              SDK or Keplr wallet"))
  ...
  let get_deposits (t : t) ?currency:_ ?limit () =
    ...
    | None ->
      Deferred.return
        (Error
           (`Api_error "Deposit history requires address - create adapter with ~address"))
    | Some address ->
      let cfg = t.cfg in
  ...
  let get_withdrawals (t : t) ?currency:_ ?limit () =
    ...
    | None ->
      Deferred.return
        (Error
           (`Api_error
               "Withdrawal history requires address - create adapter with ~address"))
    | Some address ->
      let cfg = t.cfg in
  ...
  let get_transfers (t : t) ?limit () =
    ...
    | None ->
      Deferred.return
        (Error
           (`Api_error "Transfer history requires address - create adapter with ~address"))
    | Some address ->
      let cfg = t.cfg in
  ...
  let get_asset_positions (t : t) () =
    ...
    | None ->
      Deferred.return
        (Error
           (`Api_error "Asset positions require address - create adapter with ~address"))
    | Some address ->
      let cfg = t.cfg in
  ...
  let get_perpetual_positions (t : t) () =
    ...
    | None ->
      Deferred.return
        (Error
           (`Api_error
               "Perpetual positions require address - create adapter with ~address"))
    | Some address ->
      let cfg = t.cfg in
  ...
  module Streams = struct
    let trades (t : t) =
      ...
      let%bind () =
        Deferred.List.iter t.symbols ~how:`Sequential ~f:(fun symbol ->
          Ws.Trades.subscribe ~conn ~market:symbol ())
      in
      (* Filter and transform messages to trades *)
      let trade_pipe =
        Pipe.filter_map message_pipe ~f:(fun json ->
  ...
  module Normalize = struct
    ...
    let trade (tr : Native.Trade.t) : (Types.Trade.t, string) Result.t =
      ...
      let ts =
        try Some (Time_float_unix.of_string tr.createdAt) with
        | _ -> None
      in
        Ok
          ({ venue= Venue.t
    ...
    let book_update (u : Native.Book.update) : (Types.Book_update.t, string) Result.t =
      ...
      in
      let%bind bid_levels =
        u.bids |> List.map ~f:parse_level |> Fluxum.Normalize_common.Result_util.transpose
      in
      let%bind ask_levels =
        u.asks |> List.map ~f:parse_level |> Fluxum.Normalize_common.Result_util.transpose
      in
    ...
    let symbol_info ((ticker, market) : Native.Symbol_info.t)
      : (Types.Symbol_info.t, string) Result.t
      =
      let open Result.Let_syntax in
      let%bind min_order_size =
        Fluxum.Normalize_common.Float_conv.qty_of_string market.stepSize
      in
      let%bind tick_size =
        Fluxum.Normalize_common.Float_conv.price_of_string market.tickSize
      in
        Ok
          ({ venue= Venue.t
           ; symbol= ticker
    ...
    let order_book (book : Native.Book.snapshot) : (Types.Order_book.t, string) Result.t =
      ...
      in
        Ok
          ({venue= Venue.t; symbol= ""; bids; asks; ts= None; epoch= 0}
           : Types.Order_book.t)
    ...
    let public_trade (tr : Native.Public_trade.t)
      ...
      in
        Ok
          ({venue= Venue.t; symbol= ""; price; qty; side; trade_id= Some tr.id; ts}
    ...
    let parse_timestamp s =
      try Some (Time_float_unix.of_string s) with
      | _ -> None
      ...
      let ts = parse_timestamp tr.createdAt in
        Ok
          ({ venue= Venue.t
           ...
           ; status
           ; address=
               Option.map tr.sender ~f:(fun s -> s.Rest.Account_types.sender_address)
    ...
    let asset_position_to_balance (pos : Native.Asset_position.t)
