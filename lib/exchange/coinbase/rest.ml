...
module Error = struct
  ...
  let sexp_of_t = function
    ...
    | `Json_parse msg -> Sexp.List [Sexp.Atom "Json_parse"; Sexp.Atom msg]
    | `Api_error msg -> Sexp.List [Sexp.Atom "Api_error"; Sexp.Atom msg]
end
...
module Types = struct
  ...
  type products_response = {products: product list}
  [@@deriving yojson {strict= false}, sexp]
  ...
  type product_book_response = {pricebook: product_book}
  [@@deriving yojson {strict= false}, sexp]
  ...
  type candles_response = {candles: candle list} [@@deriving yojson {strict= false}, sexp]
end
...
let get ~cfg ~path : (Yojson.Safe.t, [> Error.t]) result Deferred.t =
  ...
  let uri = Uri.of_string (Cfg.rest_url ^ path) in
    Monitor.try_with (fun () ->
      ...
      >>= fun (_response, body) -> Cohttp_async.Body.to_string body)
    >>| function
    | Error exn -> Error (`Http (0, Exn.to_string exn))
    | Ok body_str ->
...
let products cfg : (Types.products_response, [> Error.t]) result Deferred.t =
  get ~cfg ~path:"/api/v3/brokerage/products"
  >>| function
  | Error _ as err -> err
  | Ok json ->
...
let candles cfg ~product_id ~start ~end_ ~granularity
  ...
  let path =
    sprintf
      "/api/v3/brokerage/products/%s/candles?start=%s&end=%s&granularity=%s"
      product_id
      start
      end_
      granularity
...
let get_authenticated ~cfg ~path : (Yojson.Safe.t, [> Error.t]) result Deferred.t =
  ...
  let timestamp = Signature.current_timestamp () in
    match
      Signature.coinbase_rest_signature
...
let accounts cfg : (Account.response, [> Error.t]) result Deferred.t =
  get_authenticated ~cfg ~path:"/api/v3/brokerage/accounts"
  >>| function
  | Error _ as err -> err
  | Ok json ->
...
module Order = struct
  ...
  let order_configuration_to_yojson = function
    | Market_market_ioc m -> `Assoc [("market_market_ioc", market_ioc_to_yojson m)]
    | Limit_limit_gtc l -> `Assoc [("limit_limit_gtc", limit_gtc_to_yojson l)]
  ...
  type order_status_response = {order: order_status}
  [@@deriving yojson {strict= false}, sexp]
  ...
  type cancel_request_item = {order_id: string} [@@deriving yojson {strict= false}, sexp]
  ...
  type cancel_response = {results: cancel_result list}
  [@@deriving yojson {strict= false}, sexp]
end

(** Create an order *)
let create_order cfg (req : Order.create_order_request)
  : (Order.create_order_response, [> Error.t]) result Deferred.t
  =
  let body = Order.create_order_request_to_yojson req in
    post_authenticated ~cfg ~path:"/api/v3/brokerage/orders" ~body
    >>| function
    | Error _ as err -> err
    | Ok json ->
...
let cancel_orders cfg ~order_ids : (Order.cancel_response, [> Error.t]) result Deferred.t =
  let body =
    `Assoc [("order_ids", `List (List.map order_ids ~f:(fun id -> `String id)))]
  in
    post_authenticated ~cfg ~path:"/api/v3/brokerage/orders/batch_cancel" ~body
    >>| function
    | Error _ as err -> err
    | Ok json ->
...
(** List orders *)
let list_orders cfg ?product_id ?status ?limit ()
  : (Order.orders_response, [> Error.t]) result Deferred.t
  =
  let params =
    List.filter_opt
  ...
  let path =
    ...
    | _ ->
      sprintf "/api/v3/brokerage/orders/historical?%s" (String.concat ~sep:"&" params)
  in
    get_authenticated ~cfg ~path
    >>| function
    | Error _ as err -> err
...
module Deposit_address = struct
  ...
  type response = {data: t} [@@deriving yojson {strict= false}, sexp]
  type list_response = {data: t list} [@@deriving yojson {strict= false}, sexp]
end
...
let get_account_by_currency cfg ~currency
  : (Account.account option, [> Error.t]) result Deferred.t
  =
  accounts cfg
  >>| function
  | Error _ as err -> err
  | Ok resp ->
    let currency_upper = String.uppercase currency in
      Ok
        (List.find resp.accounts ~f:(fun a ->
...
let create_deposit_address cfg ~account_id ?name ()
  : (Deposit_address.t, [> Error.t]) result Deferred.t
  =
  ...
  let body =
    ...
    | None -> `Assoc []
  in
    post_authenticated ~cfg ~path ~body
    >>| function
    | Error _ as err -> err
    | Ok json ->
...
let list_deposit_addresses cfg ~account_id
  : (Deposit_address.t list, [> Error.t]) result Deferred.t
  =
  let path = sprintf "/v2/accounts/%s/addresses" account_id in
    get_authenticated ~cfg ~path
    >>| function
...
let get_deposit_address cfg ~currency ?network:_ ()
  : (Deposit_address.t, [> Error.t]) result Deferred.t
  =
  ...
  | Ok None ->
    Deferred.return
      (Error (`Api_error (sprintf "No account found for currency %s" currency)))
  | Ok (Some account) ->
...
let send_money cfg ~account_id ~to_address ~amount ~currency ?description ()
  : (Transaction.t, [> Error.t]) result Deferred.t
  =
  let path = sprintf "/v2/accounts/%s/transactions" account_id in
  let req : Send.request =
    {type_= "send"; to_= to_address; amount; currency; description; idem= None}
  in
...
let withdraw cfg ~currency ~amount ~address ?tag:_ ?network:_ ()
  : (Transaction.t, [> Error.t]) result Deferred.t
  =
  (* First get the account UUID for this currency *)
  get_account_by_currency cfg ~currency
...
let list_transactions cfg ~account_id ?limit ()
  : (Transaction.t list, [> Error.t]) result Deferred.t
  =
  let params = List.filter_opt [Option.map limit ~f:(fun l -> sprintf "limit=%d" l)] in
  let path =
    match params with
    | [] -> sprintf "/v2/accounts/%s/transactions" account_id
    | _ ->
      sprintf "/v2/accounts/%s/transactions?%s" account_id (String.concat ~sep:"&" params)
  in
    get_authenticated ~cfg ~path
    >>| function
    | Error _ as err -> err
...
let get_deposits cfg ?currency ?limit ()
  : (Transaction.t list, [> Error.t]) result Deferred.t
  =
...
let get_withdrawals cfg ?currency ?limit ()
  : (Transaction.t list, [> Error.t]) result Deferred.t
  =
  (* Get all accounts and filter by currency if specified *)
  accounts cfg
...
module Transaction_summary = struct
  ...
  type margin_rate = {value: string option [@default None]}
  [@@deriving yojson {strict= false}, sexp]
...
let transaction_summary cfg
  : (Transaction_summary.response, [> Error.t]) result Deferred.t
  =
  get_authenticated ~cfg ~path:"/api/v3/brokerage/transaction_summary"
  >>| function
  | Error _ as err -> err
  | Ok json ->
