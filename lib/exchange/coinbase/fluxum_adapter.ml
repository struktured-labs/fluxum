...
module Adapter : Exchange_intf.S = struct
  ...
  let place_order (_ : t) (_ : Native.Order.request) =
    Deferred.Result.fail (`Not_implemented "coinbase.place_order")

  let cancel_order (_ : t) (_ : Native.Order.id) =
    Deferred.Result.fail (`Not_implemented "coinbase.cancel_order")

  let balances (_ : t) =
    Deferred.Result.fail (`Not_implemented "coinbase.balances")
