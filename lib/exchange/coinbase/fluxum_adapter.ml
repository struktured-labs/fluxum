...
module Adapter = struct
  ...
  let cancel_order t order_id =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      Rest.cancel_orders t.cfg ~order_ids:[order_id]
      >>| function
      | Ok resp ->
        (match List.find resp.results ~f:(fun r -> r.success) with
         ...
         | None ->
           let msg =
             List.filter_map resp.results ~f:(fun r -> r.failure_reason)
             |> String.concat ~sep:", "
           in
  ...
  let get_open_orders t ?symbol () =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      Rest.list_orders t.cfg ?product_id:symbol ~status:"OPEN" ~limit:100 ()
      >>| function
  ...
  let get_order_history t ?symbol ?limit () =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      Rest.list_orders t.cfg ?product_id:symbol ?limit ()
      >>| function
      | Ok resp -> Ok resp.orders
      | Error e -> Error e)
  ...
  let get_ticker t ~symbol () =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      Rest.best_bid_ask t.cfg ~product_id:symbol
      >>| function
      | Ok ticker -> Ok ticker
      | Error e -> Error e)
  ...
  let get_order_book t ~symbol ?limit:_ () =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      Rest.product_book t.cfg ~product_id:symbol
      >>| function
      | Ok book -> Ok book
      | Error e -> Error e)
    ...
    | Ok orders ->
      let order_ids = List.map orders ~f:(fun o -> o.order_id) in
        (match order_ids with
         ...
         | ids ->
           Rest.cancel_orders t.cfg ~order_ids:ids
           >>| (function
  ...
  module Normalize = struct
    let order_response (resp : Native.Order.response) : (Types.Order.t, string) Result.t =
      ...
      | true ->
        let order_id =
          match resp.success_response with
          | Some sr -> sr.order_id
        ...
        in
          Ok
            ({ venue= Venue.t
             ...
             ; symbol=
                 (match resp.success_response with
                  | Some sr -> Option.value sr.product_id ~default:""
                  | None -> "")
      ...
      | false ->
        let msg =
          match resp.error_response with
          | Some er ->
            String.concat
              ~sep:"; "
              (List.filter_opt
    ...
    let order_from_status (status : Native.Order.status)
      ...
      let qty =
        match status.base_size with
        | Some s ->
          (match Fluxum.Normalize_common.Float_conv.qty_of_string s with
           | Ok q -> q
           | Error _ -> 0.0)
        | None -> 0.0
      in
      let filled =
        match status.filled_size with
        | Some s ->
          (match Fluxum.Normalize_common.Float_conv.qty_of_string s with
           | Ok q -> q
           | Error _ -> 0.0)
    ...
    let balance (b : Native.Balance.t) : (Types.Balance.t, string) Result.t =
      let open Result.Let_syntax in
      let%bind available =
        Fluxum.Normalize_common.Float_conv.of_string b.available_balance.value
      in
      let%bind hold =
        match b.hold with
    ...
    let book_update (book : Native.Book.update) : (Types.Book_update.t, string) Result.t =
      ...
      let%bind levels =
        List.map book.bids ~f:(fun level ->
          let%bind price =
            Fluxum.Normalize_common.Float_conv.price_of_string level.price
          in
    ...
    let symbol_info (s : Native.Symbol_info.t) : (Types.Symbol_info.t, string) Result.t =
      let open Result.Let_syntax in
      let base =
        match s.base_name with
        | Some n -> n
        | None -> ""
      in
      let quote =
        match s.quote_name with
        | Some n -> n
        | None -> ""
      in
      let%bind min_order_size =
        match s.base_min_size with
        | Some str -> Fluxum.Normalize_common.Float_conv.of_string str
        | None -> Ok 0.0
      in
      let%bind tick_size =
        match s.base_increment with
        | Some str ->
          let%map f = Fluxum.Normalize_common.Float_conv.of_string str in
            Some f
        | None -> Ok None
      in
      let%bind quote_increment =
        match s.quote_increment with
        | Some str ->
          let%map f = Fluxum.Normalize_common.Float_conv.of_string str in
            Some f
        | None -> Ok None
    ...
    let ticker (t : Native.Ticker.t) : (Types.Ticker.t, string) Result.t =
      let open Result.Let_syntax in
      let%bind bid_price =
        Fluxum.Normalize_common.Float_conv.price_of_string t.best_bid
      in
      let%bind ask_price =
        Fluxum.Normalize_common.Float_conv.price_of_string t.best_ask
      in
        Ok
          ({ venue= Venue.t
    ...
    let order_book (book : Native.Book.snapshot) : (Types.Order_book.t, string) Result.t =
      ...
      in
        Ok
          ({venue= Venue.t; symbol= book.product_id; bids; asks; ts= None; epoch= 0}
           : Types.Order_book.t)
    ...
    let parse_timestamp (ts : string) : Time_float_unix.t option =
      try Some (Time_float_unix.of_string ts) with
      | _ -> None
    ...
    let deposit (tx : Native.Deposit.t) : (Types.Deposit.t, string) Result.t =
      let open Result.Let_syntax in
      let%bind amount =
        Fluxum.Normalize_common.Float_conv.amount_of_string tx.amount.amount
      in
      let status =
        match String.lowercase tx.status with
    ...
    let withdrawal (tx : Native.Withdrawal.t) : (Types.Withdrawal.t, string) Result.t =
      ...
      let tx_id = Option.bind tx.network ~f:(fun n -> n.hash) in
      let address =
        Option.bind tx.to_ ~f:(fun t -> t.address) |> Option.value ~default:""
      in
      let tag =
        Option.bind tx.to_ ~f:(fun t ->
