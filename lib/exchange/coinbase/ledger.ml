(** Coinbase Ledger - Implements unified Ledger_intf for P&L tracking

    NOTE: Coinbase Advanced Trade API is data-only for this adapter.
    Full trade history and P&L tracking requires additional implementation. *)
...
module Entry = struct
  ...
  let pipe
        ...
        (trade_events : Rest.Types.trade Pipe.Reader.t)
    =
             ...
             (match
                ...
                let%bind price =
                  Fluxum.Normalize_common.Float_conv.price_of_string
                    trade.Rest.Types.price
                in
                let%bind qty =
                  Fluxum.Normalize_common.Float_conv.qty_of_string trade.Rest.Types.size
                in
                  Ok (price, qty)
              with
              | Error err ->
                Log.Global.error
                  "Coinbase ledger: Failed to parse trade price/qty: %s"
                  err;
                Deferred.unit
              | Ok (price, qty) ->
                let side =
                  match trade.Rest.Types.side with
...
let from_balances ?(notional_currency = "USD") (balances : Rest.Account.account list) : t =
  List.fold balances ~init:Fluxum.Types.Symbol.Map.empty ~f:(fun acc account ->
    match
      Fluxum.Normalize_common.Float_conv.amount_of_string
        account.Rest.Account.available_balance.value
    with
    | Error err ->
      Log.Global.error
        "Coinbase ledger: Failed to parse balance for %s: %s"
        account.Rest.Account.currency
        err;
      acc
    | Ok position ->
      ...
      (match Float.(position > 0.) with
       ...
       | true ->
         ...
         let symbol = account.Rest.Account.currency ^ "-" ^ notional_currency in
...
let from_trades
  ...
  let symbol_to_entry_and_pipe =
    List.fold sorted_trades ~init:Fluxum.Types.Symbol.Map.empty ~f:(fun acc trade ->
      let symbol = trade.Rest.Types.product_id in
        (* Parse price and qty *)
        match
          let open Result.Let_syntax in
          let%bind price =
            Fluxum.Normalize_common.Float_conv.price_of_string trade.Rest.Types.price
          in
          let%bind qty =
            Fluxum.Normalize_common.Float_conv.qty_of_string trade.Rest.Types.size
          in
        ...
        with
        | Error err ->
          Log.Global.error "Coinbase ledger: Failed to parse trade for %s: %s" symbol err;
          acc
        | Ok (price, qty) ->
          let side =
            match trade.Rest.Types.side with
...
let pipe
      ...
      (trades : Rest.Types.trade Pipe.Reader.t)
        ...
        | Some book_pipe ->
          ...
          let symbol_trades =
            Pipe.filter_map trades ~f:(fun trade ->
              match String.equal trade.Rest.Types.product_id symbol with
              | true -> Some trade
              | false -> None)
          in
...
let command : string * Command.t =
  ( "ledger"
  , Command.basic
      ~summary:"Coinbase ledger (P&L tracking - limited API support)"
      (Command.Param.return (fun () ->
         printf "Coinbase ledger - use `from_balances` in code\n")) )
