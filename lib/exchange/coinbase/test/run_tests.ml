...
let test_ws_parse_market_trades () =
  ...
  in
    ...
    | Coinbase.Ws.Message.MarketTrades trades ->
      ignore
        (assert_string_equal "market_trades" trades.channel "Channel is market_trades");
...
let test_stream_names () =
  ...
  let trades = Coinbase.Ws.Stream.MarketTrades ["BTC-USD"] in
    ignore
      (assert_string_equal
         "level2"
         (Coinbase.Ws.Stream.channel_name level2)
         "Level2 channel name");
    ignore
      (assert_string_equal
         "market_trades"
         (Coinbase.Ws.Stream.channel_name trades)
         "MarketTrades channel name");
    ignore
      (assert_true
         (List.length (Coinbase.Ws.Stream.product_ids level2) = 2)
         "Level2 has 2 products");
    ignore
      (assert_true
         (List.length (Coinbase.Ws.Stream.product_ids trades) = 1)
         "Trades has 1 product")
...
let test_books_multiple_symbols () =
  ...
  let symbols = Coinbase.Order_book.Books.symbols books in
    ...
    ignore
      (assert_true (List.mem symbols "BTC-USD" ~equal:String.equal) "Contains BTC-USD");
    ignore
      (assert_true (List.mem symbols "ETH-USD" ~equal:String.equal) "Contains ETH-USD")
...
let test_normalize_deposit_address () =
  ...
  let addr : Coinbase.Rest.Deposit_address.t =
    ...
    ; address_info=
        Some {address= "0x1234567890abcdef1234567890abcdef12345678"; destination_tag= None}
  ...
  in
    ...
    | Ok da ->
      ...
      ignore
        (assert_string_equal
           "ethereum"
           (Option.value da.network ~default:"")
           "Network is ethereum");
...
let test_normalize_deposit_address_with_tag () =
  ...
  let addr : Coinbase.Rest.Deposit_address.t =
    ...
    ; address_info=
        Some
          {address= "rN7n3473SaZBCG4dFL83w7a1RXtXtbk2D9"; destination_tag= Some "12345678"}
  ...
  in
    ...
    | Ok da ->
      ...
      ignore
        (assert_string_equal
           "12345678"
           (Option.value da.tag ~default:"")
           "Destination tag extracted");
  ...
  in
    ...
    | Ok dep ->
      ...
      ignore
        (assert_string_equal
           "0xabc123"
           (Option.value dep.tx_id ~default:"")
           "TX hash extracted");
...
let test_normalize_withdrawal () =
  ...
  in
    ...
    | Ok wd ->
      ...
      ignore
        (assert_string_equal "0xdestination" wd.address "Destination address extracted");
        ...
        let status_str = Fluxum.Types.Transfer_status.to_string wd.status in
          ignore
            (assert_string_equal
               expected_str
               status_str
               (sprintf "Status '%s' -> '%s'" input_status expected_str))
      | Error _ ->
        incr tests_run;
        incr tests_failed;
