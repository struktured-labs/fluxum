    ...
    let count = List.length products in
      (match count > 0 with
       | true ->
         ...
         (match
            List.find products ~f:(fun p ->
              String.equal (p |> member "id" |> to_string) "BTC-USD")
          with
          | Some p ->
...
let test_product_book () =
  ...
  get_json "/products/BTC-USD/book?level=2"
  >>| function
  | Ok json ->
    (try
       match json with
     ...
     with
     | e -> fail (sprintf "Parse error: %s" (Exn.to_string e)))
  | Error e -> fail (sprintf "Error: %s" e)

let test_ticker () =
  printf "\n[REST] Ticker (ETH-USD)\n";
  get_json "/products/ETH-USD/ticker"
  >>| function
  | Ok json ->
    let open Yojson.Safe.Util in
    (try
...
let test_trades () =
  ...
  get_json "/products/BTC-USD/trades?limit=5"
  >>| function
  | Ok json ->
    let trades =
      match json with
         ...
         (match List.hd trades with
          | Some trade ->
            (try
               ...
               let trade_id_str =
                 match trade |> member "trade_id" with
                 | `Int n -> Int.to_string n
                 | `Intlit s -> s
                 | _ -> "?"
               in
                 pass
                   (sprintf "Latest: %s %s @ $%s (id: %s)" side size price trade_id_str)
             with
             | e -> fail (sprintf "Parse error: %s" (Exn.to_string e)))
          | None -> ())
