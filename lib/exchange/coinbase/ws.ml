...
module Stream = struct
  ...
  let to_subscribe_message streams =
    let channel_groups =
      List.map streams ~f:(fun stream ->
        `Assoc
          [ ("name", `String (channel_name stream))
          ; ( "product_ids"
            , `List (List.map (product_ids stream) ~f:(fun id -> `String id)) ) ])
    in
      `Assoc [("type", `String "subscribe"); ("channels", `List channel_groups)]
      ...
      List.map streams ~f:(fun stream ->
        `Assoc
          ...
          ; ( "product_ids"
            , `List (List.map (product_ids stream) ~f:(fun id -> `String id)) ) ])
    in
      `Assoc [("type", `String "unsubscribe"); ("channels", `List channel_groups)]
end
...
module Message = struct
  ...
  type ticker_event =
    ...
    ; price_percent_chg_24_h: string option
          [@default None] [@key "price_percent_chg_24_h"] }
  [@@deriving yojson {strict= false}, sexp]
  ...
  let sexp_of_heartbeat {channel; client_id; timestamp; sequence_num; events} =
    Sexp.List
      ...
      ; Sexp.List
          [Sexp.Atom "events"; Sexp.Atom (sprintf "%d events" (List.length events))] ]
  ...
  let sexp_of_t = function
    ...
    | Subscriptions resp ->
      Sexp.List [Sexp.Atom "Subscriptions"; sexp_of_subscribe_response resp]
...
let parse_message (msg : string) : Message.t =
  try
    ...
    in
    let type_ =
      Yojson.Safe.Util.member "type" json |> Yojson.Safe.Util.to_string_option
    in
      match type_ with
      | Some "subscriptions" ->
