...
type metadata = {sequence_num: int64} [@@deriving sexp]
...
module Book = struct
  ...
  let empty ?timestamp:_ ?sequence_num:_ symbol = create ~symbol
      ...
      List.fold msg.events ~init:t ~f:(fun acc event ->
        List.fold event.updates ~init:acc ~f:(fun acc2 update ->
          match
            ...
            let%bind price =
              Fluxum.Normalize_common.Float_conv.price_of_string update.price_level
            in
            let%bind size =
              Fluxum.Normalize_common.Float_conv.qty_of_string update.new_quantity
            in
              Ok (price, size)
          with
          | Error err ->
  ...
  let apply_product_book t (book : Rest.Types.product_book) =
    ...
    let bid_levels =
      List.filter_map book.bids ~f:(fun level ->
        match
          let open Result.Let_syntax in
          let%bind price =
    ...
    in
    let ask_levels =
      List.filter_map book.asks ~f:(fun level ->
        match
          let open Result.Let_syntax in
          let%bind price =
            Fluxum.Normalize_common.Float_conv.price_of_string level.price
