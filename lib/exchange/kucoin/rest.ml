...
let generate_timestamp () =
  ...
  let ms =
    Time_float_unix.to_span_since_epoch now |> Time_float.Span.to_ms |> Float.to_int
  in
    Int.to_string ms
...
let make_auth_request ~cfg ~method_ ~endpoint ~params =
  ...
  let encrypted_passphrase =
    match cfg.Cfg.api_version with
    | 1 -> passphrase
...
let ticker ~cfg ~symbol =
  ...
  let%bind json_str =
    make_public_request
      ~cfg
      ~endpoint:(sprintf "/api/v1/market/stats?symbol=%s" symbol)
      ~params:[]
  in
...
let order_book ~cfg ~symbol =
  ...
  let%bind json_str =
    make_public_request
      ~cfg
      ~endpoint:(sprintf "/api/v1/market/orderbook/level2_20?symbol=%s" symbol)
      ~params:[]
  in
    match Yojson.Safe.from_string json_str with
    | `Assoc [("data", data)] ->
...
let trades ~cfg ~symbol =
  ...
  let%bind json_str =
    make_public_request
      ~cfg
      ~endpoint:(sprintf "/api/v1/market/histories?symbol=%s" symbol)
      ~params:[]
  in
    match Yojson.Safe.from_string json_str with
...
let accounts ~cfg =
  let open Deferred.Result.Let_syntax in
  let%bind json_str =
    make_auth_request ~cfg ~method_:"GET" ~endpoint:"/api/v1/accounts" ~params:[]
  in
    match Yojson.Safe.from_string json_str with
...
let place_limit_order ~cfg ~symbol ~side ~price ~size =
  ...
  in
  let%bind json_str =
    make_auth_request ~cfg ~method_:"POST" ~endpoint:"/api/v1/orders" ~params
  in
    match Yojson.Safe.from_string json_str with
    | `Assoc [("data", data)] ->
...
let cancel_order ~cfg ~order_id =
  ...
  let%bind json_str =
    make_auth_request
      ~cfg
      ~method_:"DELETE"
      ~endpoint:(sprintf "/api/v1/orders/%s" order_id)
      ~params:[]
  in
    match Yojson.Safe.from_string json_str with
