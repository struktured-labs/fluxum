...
type order_book_entry = string * string [@@deriving sexp]
...
let order_book_entry_to_yojson (price, size) = `List [`String price; `String size]
...
(** Order response *)
type order_response = {orderId: string (** Order ID *)}
[@@deriving yojson {strict= false}, sexp]
