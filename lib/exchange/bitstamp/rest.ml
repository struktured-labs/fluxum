...
let generate_nonce () =
  ...
  let micros =
    Time_float_unix.to_span_since_epoch now |> Time_float.Span.to_us |> Float.to_int
  in
...
let make_auth_request ~cfg ~endpoint ~params =
  ...
  let auth_params = [("key", api_key); ("signature", signature); ("nonce", nonce)] in
  ...
  let headers =
    Cohttp.Header.of_list [("Content-Type", "application/x-www-form-urlencoded")]
  in
...
let make_public_request ~cfg ~endpoint ~params =
  ...
  let%bind response, body =
    Deferred.Or_error.try_with (fun () -> Cohttp_async.Client.get (Uri.of_string url))
    |> Deferred.map
         ~f:(Result.map_error ~f:(fun err -> `Network (Core.Error.to_string_hum err)))
  in
  let%bind body_string =
...
let ticker ~cfg ~pair =
  let open Deferred.Result.Let_syntax in
  let%bind json_str =
    make_public_request ~cfg ~endpoint:(sprintf "ticker/%s" pair) ~params:[]
  in
...
let ticker_hour ~cfg ~pair =
  let open Deferred.Result.Let_syntax in
  let%bind json_str =
    make_public_request ~cfg ~endpoint:(sprintf "ticker_hour/%s" pair) ~params:[]
  in
    match Yojson.Safe.from_string json_str |> Types.ticker_of_yojson with
    | Ok ticker -> return ticker
    | Error msg -> Deferred.Result.fail (`Json_parse msg)
...
let order_book ~cfg ~pair ?(group = 1) () =
  ...
  let%bind json_str =
    make_public_request ~cfg ~endpoint:(sprintf "order_book/%s" pair) ~params
  in
...
let transactions ~cfg ~pair ?(time = `Hour) () =
  ...
  let%bind json_str =
    make_public_request ~cfg ~endpoint:(sprintf "transactions/%s" pair) ~params
  in
...
let trading_pairs_info ~cfg =
  let open Deferred.Result.Let_syntax in
  let%bind json_str =
    make_public_request ~cfg ~endpoint:"trading-pairs-info" ~params:[]
  in
    match Yojson.Safe.from_string json_str with
    | `List pairs ->
...
let ohlc ~cfg ~pair ~step ~limit ?(start = 0) ?(end_ = 0) () =
  ...
  in
  let%bind json_str =
    make_public_request ~cfg ~endpoint:(sprintf "ohlc/%s" pair) ~params
  in
    match Yojson.Safe.from_string json_str with
...
let user_transactions ~cfg ~pair ?(offset = 0) ?(limit = 100) ?(sort = `Desc) () =
  ...
  in
  let params =
    [("offset", Int.to_string offset); ("limit", Int.to_string limit); ("sort", sort_str)]
  in
  let%bind json_str =
    make_auth_request ~cfg ~endpoint:(sprintf "user_transactions/%s" pair) ~params
  in
    match Yojson.Safe.from_string json_str with
    | `List txs ->
...
let open_orders ~cfg ~pair =
  let open Deferred.Result.Let_syntax in
  let%bind json_str =
    make_auth_request ~cfg ~endpoint:(sprintf "open_orders/%s" pair) ~params:[]
  in
    match Yojson.Safe.from_string json_str with
    | `List orders ->
...
let buy_limit_order ~cfg ~pair ~amount ~price ?limit_price ?daily_order () =
  ...
  let params =
    List.filter_opt
      ...
      ; Option.map daily_order ~f:(fun d ->
          ("daily_order", if d then "True" else "False")) ]
  in
...
let buy_market_order ~cfg ~pair ~amount =
  ...
  let%bind json_str =
    make_auth_request ~cfg ~endpoint:(sprintf "buy/market/%s" pair) ~params
  in
    match Yojson.Safe.from_string json_str |> Types.order_response_of_yojson with
    | Ok order -> return order
    | Error msg -> Deferred.Result.fail (`Json_parse msg)
...
let sell_market_order ~cfg ~pair ~amount =
  ...
  let params = [("amount", Float.to_string amount)] in
  let%bind json_str =
    make_auth_request ~cfg ~endpoint:(sprintf "sell/market/%s" pair) ~params
  in
    match Yojson.Safe.from_string json_str |> Types.order_response_of_yojson with
    | Ok order -> return order
    | Error msg -> Deferred.Result.fail (`Json_parse msg)
...
let deposit_address ~cfg ~currency =
  ...
  let%bind json_str = make_auth_request ~cfg ~endpoint ~params:[] in
    ...
    | `Assoc fields ->
      let address =
        List.Assoc.find fields ~equal:String.equal "address"
      ...
      in
      let destination_tag =
        List.Assoc.find fields ~equal:String.equal "destination_tag"
        |> Option.bind ~f:(function
          | `String s -> Some s
          | _ -> None)
      in
