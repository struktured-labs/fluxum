...
module Adapter = struct
  ...
  module Native = struct
    module Order = struct
      ...
      type request =
        ...
        * string
        * float
        * float option (* side_type, pair, amount, price option *)
  ...
  let place_order t (req : Native.Order.request) =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      let side_type, pair, amount, price = req in
        ...
        | `Buy_market, _ -> Rest.buy_market_order ~cfg:t.cfg ~pair ~amount
        | `Sell_market, _ -> Rest.sell_market_order ~cfg:t.cfg ~pair ~amount
        | `Buy_limit, None ->
  ...
  let get_open_orders t ?symbol () =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      match symbol with
      | Some pair ->
        Rest.open_orders ~cfg:t.cfg ~pair
        >>| (function
         | Ok orders -> Ok orders
         | Error e -> Error e)
      | None ->
        Rest.open_orders_all ~cfg:t.cfg
        >>| (function
         | Ok orders -> Ok orders
         | Error e -> Error e))
  ...
  let get_order_history _t ?symbol:_ ?limit:_ () =
    Deferred.return
      (Error (`Api_error "Bitstamp: use user_transactions for order history"))

  let get_my_trades t ~symbol ?limit () =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      let limit = Option.value limit ~default:100 in
        Rest.user_transactions ~cfg:t.cfg ~pair:symbol ~limit ()
        >>| function
        | Ok txs ->
  ...
  let cancel_all_orders t ?symbol:_ () =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      Rest.cancel_all_orders ~cfg:t.cfg
      >>| function
      | Ok _ -> Ok 0
      | Error e -> Error e)
  ...
  let get_deposit_history _t ?currency:_ ?status:_ ?start_time:_ ?end_time:_ ?limit:_ () =
    ...
    Deferred.return
      (Error (`Api_error "Bitstamp: use user_transactions for deposit history"))
  ...
  module Normalize = struct
      ...
      let ts =
        Some
          (Time_float_unix.of_span_since_epoch
             (Time_float_unix.Span.of_sec (Float.of_string t.date)))
      in
        Ok
          ({ venue= Venue.t
           ; symbol= ""
    ...
    let balance (b : Native.Balance.t) : (Types.Balance.t, string) Result.t =
      ...
      let extract_currency name available_opt balance_opt reserved_opt =
        let avail =
          Option.value_map available_opt ~default:0.0 ~f:(fun s ->
            Option.value ~default:0.0 (Float.of_string_opt s))
        in
        let total =
          Option.value_map balance_opt ~default:0.0 ~f:(fun s ->
            Option.value ~default:0.0 (Float.of_string_opt s))
        in
        let locked =
          Option.value_map reserved_opt ~default:0.0 ~f:(fun s ->
            Option.value ~default:0.0 (Float.of_string_opt s))
        in
          {Types.Balance.venue= Venue.t; currency= name; total; available= avail; locked}
      in
    ...
    let order_book (book : Native.Book.snapshot) : (Types.Order_book.t, string) Result.t =
      let bids =
        List.map book.bids ~f:(fun (price, volume) ->
          ({Types.Order_book.Price_level.price; volume} : Types.Order_book.Price_level.t))
      in
      let asks =
        List.map book.asks ~f:(fun (price, volume) ->
          ({Types.Order_book.Price_level.price; volume} : Types.Order_book.Price_level.t))
      in
        Ok
      ...
      in
        Ok
          ({ venue= Venue.t
           ...
           ; status= s.trading
           ; min_order_size=
               Option.value ~default:0.0 (Float.of_string_opt s.minimum_order)
           ; tick_size= None
    ...
    let ticker (t : Native.Ticker.t) : (Types.Ticker.t, string) Result.t =
      ...
      let ts =
        Some
          (Time_float_unix.of_span_since_epoch
             (Time_float_unix.Span.of_sec (Float.of_string t.timestamp)))
      in
        Ok
          ({ venue= Venue.t
           ; symbol= ""
    ...
    let deposit_address (d : Native.Deposit_address.t) ~currency
    ...
    let withdrawal (w : Native.Withdrawal.t) : (Types.Withdrawal.t, string) Result.t =
      ...
      let created_at =
        try Some (Time_float_unix.of_string w.datetime) with
        | _ -> None
      in
        Ok
          ({ venue= Venue.t
