...
type metadata = {microtimestamp: string} [@@deriving sexp]
...
module Book = struct
  ...
  let apply_snapshot book (ob : Types.order_book) =
    ...
    let bid_levels =
      List.filter_map ob.bids ~f:(fun (price, amount) ->
        match Float.(amount > 0.) with
        | true -> Some (`Bid, price, amount)
        | false -> None)
    in
    let ask_levels =
      List.filter_map ob.asks ~f:(fun (price, amount) ->
        match Float.(amount > 0.) with
               ...
               | Some msg_str ->
                 (match Ws.parse_message msg_str with
                  ...
                  | Ws.Message.Data json_str ->
                    ...
                    (try
                       match
                         Yojson.Safe.from_string json_str |> Types.order_book_of_yojson
                       with
                       | Ok ob ->
                         book_ref := apply_snapshot !book_ref ob;
                         Pipe.write book_writer (Ok !book_ref) >>= read_loop
