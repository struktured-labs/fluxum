  ...
  let conn_msg = {|{"event":"bts:connection_established","data":{}}|} in
    ...
    in
      (match Bitstamp.Ws.parse_message sub_msg with
       | Bitstamp.Ws.Message.Subscribed "live_trades_btcusd" ->
         pass "Parse subscription succeeded"
...
let test_rest_signature () =
  ...
  let nonce2 = Bitstamp.Rest.generate_nonce () in
    test_assert "Nonce is numeric string" (String.for_all nonce1 ~f:Char.is_digit);
    test_assert "Nonces increase" (Int.of_string nonce2 >= Int.of_string nonce1);
    (* Test signature creation *)
    let signature =
      Bitstamp.Rest.create_signature
        ~api_secret:"test_secret"
    ...
    in
      ...
      test_assert "Signature is 64 characters (SHA256)" (String.length signature = 64);
