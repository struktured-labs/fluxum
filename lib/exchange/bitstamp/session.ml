...
module Events = struct
  ...
  let create ~cfg ?(symbols : Fluxum.Types.Symbol.t list = []) ?order_ids:_ ()
    : t Deferred.t
    =
          ...
          ~create_pipe:(fun () ->
            ...
            let rec poll_loop () =
              ...
              | Ok bal ->
                ...
                >>= fun () -> after (Time_float_unix.Span.of_sec 10.0) >>= poll_loop
              | Error err ->
                Log.Global.error
                  "Bitstamp balance poll failed: %s"
                 ...
                 let rec poll_loop () =
                   ...
                   | Ok ticker ->
                     let json = Types.ticker_to_yojson ticker in
                       Pipe.write writer json
                       >>= fun () -> after (Time_float_unix.Span.of_sec 5.0) >>= poll_loop
                   | Error err ->
                     Log.Global.error
                       "Bitstamp ticker poll failed for %s: %s"
                       pair
                       (Sexp.to_string (Rest.Error.sexp_of_t err));
    ...
    let ledger =
      Fluxum.Types.Symbol.Map.of_alist_exn
        (List.map symbols ~f:(fun pair ->
           ...
           let book_pipe =
             match Map.find order_books pair with
             | Some pipe -> pipe
             | None ->
               let reader, _writer = Pipe.create () in
                 reader
           in
           let trade_pipe =
             match Map.find trades pair with
             | Some pipe -> pipe
             | None ->
               let reader, _writer = Pipe.create () in
                 reader
           ...
           let entry_ref = ref init in
             ...
             don't_wait_for
               (Pipe.iter trade_pipe ~f:(fun (trade : Types.trade) ->
                  ...
                  | Ok price ->
                    let side =
                      match trade.type_ with
                      | 0 -> Fluxum.Types.Side.Buy
                      | _ -> Fluxum.Types.Side.Sell
                    in
                    let qty =
                      match
                        Fluxum.Normalize_common.Float_conv.qty_of_string trade.amount
                      with
                      | Ok v -> v
                      | Error _ -> 0.0
                    in
                      entry_ref
                      := Ledger.Entry.with_market_data !entry_ref ~price ~qty ~side;
                      Pipe.write ledger_writer !entry_ref
                  | Error err ->
