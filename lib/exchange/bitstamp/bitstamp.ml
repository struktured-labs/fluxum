...
let ticker_command : string * Command.t =
  ...
  , Command.async
      ...
      (let%map_open.Command pair =
         flag "--pair" (required string) ~doc:"STRING Trading pair (e.g. btcusd)"
       in
         fun () ->
           Rest.ticker ~cfg ~pair
           >>| function
           | Ok t ->
             printf "Ticker: %s\n" pair;
             printf "  Last:    %s\n" t.last;
           ...
           | Ok book ->
             ...
             let asks = Array.of_list book.asks in
               for i = 0 to max_len - 1 do
                 let bid_str =
                   match i < Array.length bids with
                   | true ->
                     let p, q = bids.(i) in
                       sprintf "%.2f x %.8f" p q
                   | false -> ""
                 in
                 let ask_str =
                   match i < Array.length asks with
                   | true ->
                     let p, q = asks.(i) in
                       sprintf "%.2f x %.8f" p q
                   | false -> ""
           ...
           let print_bal name avail reserved total =
             ...
             | Some a, Some r, Some t ->
               (match Float.(of_string a + of_string r > 0.) with
                | true ->
                  printf "  %-6s  available=%-14s  reserved=%-14s  total=%s\n" name a r t
                | false -> ())
             | _ -> ()
           in
...
let open_orders_command : string * Command.t =
  ...
  , Command.async
      ...
      (let%map_open.Command pair =
         flag "--pair" (optional string) ~doc:"STRING Trading pair (omit for all)"
       in
         fun () ->
           (match pair with
           ...
           | Ok orders ->
             ...
             List.iter orders ~f:(fun o ->
               let side =
                 match o.Types.type_ with
                 | 0 -> "BUY "
                 | _ -> "SELL"
               in
                 printf
                   "  %s  %s  price=%s  amount=%s  pair=%s\n"
                   o.id
                   side
                   o.price
                   o.amount
                 ...
                 let spread = Order_book.Book.spread book in
                   ...
                   let top_asks = Order_book.Book.best_n_asks book ~n:5 () in
                     ...
                     List.iter2_exn
                       (List.take
                          (top_bids
                           @ List.init 5 ~f:(fun _ -> Order_book.Price_level.empty))
                          5)
                       (List.take
                          (top_asks
                           @ List.init 5 ~f:(fun _ -> Order_book.Price_level.empty))
...
let session_command : string * Command.t =
  ...
  , Command.async
      ...
      (let%map_open.Command pairs_str =
         flag
           "--pairs"
           (required string)
         ...
         fun () ->
           let pairs = String.split pairs_str ~on:',' |> List.map ~f:String.strip in
             ...
             printf
               "Session state: %s\n"
               (Sexp.to_string_hum
                  (Fluxum.Session_intf.State.sexp_of_t (Session.state session)));
...
let command : Command.t =
  Command.group
    ~summary:"Bitstamp Exchange Commands"
    [ ticker_command
    ; depth_command
    ; trades_command
    ; pairs_command
