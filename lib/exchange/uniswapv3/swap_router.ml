             ...
             (match gas_price_result with
              ...
              | Ok gas_price_hex ->
                let tx : Ethereum.Tx.eip1559_tx =
                  ...
                  ; nonce
                  ; max_priority_fee_per_gas= "3B9ACA00"
                  ; (* 1 gwei *)
                    max_fee_per_gas= gas_price_hex
                  ; gas_limit= 60000
                  ; (* ERC-20 approve is ~46k gas *)
                    to_= token_in
                  ; value= "0"
                  ; data= approve_data }
                in
                  (match Ethereum.Tx.sign tx ~private_key_hex:private_key with
                   | Error msg ->
                     return
                       (Error (`Swap_error (sprintf "Approve signing failed: %s" msg)))
                   | Ok raw_tx ->
             ...
             (match nonce_result with
              ...
              | Ok nonce ->
                ...
                let gas_limit =
                  ...
                  | Ok gas_hex ->
                    Ethereum.Rpc.hex_to_int gas_hex * 120 / 100 (* 20% buffer *)
                  ...
                  (match gas_price_result with
                   ...
                   | Ok gas_price_hex ->
                     ...
                     let tx : Ethereum.Tx.eip1559_tx =
                       { chain_id= cfg.chain_id
                       ; nonce
                       ; max_priority_fee_per_gas= "59682F00"
                       ; (* 1.5 gwei *)
                         max_fee_per_gas= gas_price_hex
                       ; gas_limit
                       ; to_= cfg.swap_router_address
                       ; value= "0"
                       ; (* No ETH value for token swaps *)
                         data= calldata }
                     in
                       (match Ethereum.Tx.sign tx ~private_key_hex:private_key with
                        | Error msg ->
                          return
                            (Error (`Swap_error (sprintf "Tx signing failed: %s" msg)))
                        | Ok raw_tx ->
