...
let normalize (pool : Native.pool) : (Pool_intf.Pool.t, string) Result.t =
  ...
  in
  let spot_price_inv =
    match Float.(spot_price > 0.0) with
    | true -> 1.0 /. spot_price
    | false -> 0.0
  in
  ...
  let%bind liquidity = Pool_common.Concentrated.parse_liquidity pool.liquidity in
    Ok
      ...
      ; token0=
          { Pool_intf.Token.address= pool.token0.id
          ; symbol= pool.token0.symbol
          ; decimals= decimals0 }
      ; token1=
          { Pool_intf.Token.address= pool.token1.id
      ...
      ; tvl_usd= pool.volumeUSD
      ; (* Using volume as proxy for TVL *)
        fee_bps
      ; spot_price
      ; spot_price_inv }
...
let spot_price (pool : Native.pool) ~(token_in : string) ~(token_out : string)
  ...
  let decimals1 = pool.token1.decimals in
    ...
    in
      match
        String.equal token_in pool.token0.id || String.equal token_in pool.token0.symbol
      with
      | true
        when String.equal token_out pool.token1.id
             || String.equal token_out pool.token1.symbol -> Ok base_price
      | false
        when (String.equal token_in pool.token1.id
              || String.equal token_in pool.token1.symbol)
             && (String.equal token_out pool.token0.id
                 || String.equal token_out pool.token0.symbol) -> Ok (1.0 /. base_price)
...
let quote
  ...
  in
  let is_token1_out =
    String.equal token_out pool.token1.id || String.equal token_out pool.token1.symbol
  in
