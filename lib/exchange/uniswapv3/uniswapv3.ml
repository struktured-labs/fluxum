...
let pools_command : string * Command.t =
  ...
  , Command.async
      ...
      (let%map_open.Command first =
       ...
       in
         fun () ->
           Rest.pools ~cfg ~first ()
           >>| function
           | Ok pools ->
             printf "Top %d Uniswap V3 Pools:\n\n" (List.length pools);
             printf
               "%-44s %-8s %-8s %8s %15s %10s\n"
               "Pool ID"
               "Token0"
               "Token1"
               "Fee"
...
let pool_info_command : string * Command.t =
  ...
  , Command.async
      ...
      (let%map_open.Command pool_id =
       ...
       in
         fun () ->
           Rest.pool_by_id ~cfg ~pool_id
           >>| function
           | Ok p ->
             printf "Pool: %s\n" p.id;
             printf "  Pair: %s / %s\n" p.token0.symbol p.token1.symbol;
             printf
               "  Token0: %s (%s, %d decimals)\n"
               p.token0.name
               p.token0.id
               p.token0.decimals;
             printf
               "  Token1: %s (%s, %d decimals)\n"
               p.token1.name
               p.token1.id
               p.token1.decimals;
             printf
               "  Fee tier: %d (%.2f%%)\n"
               p.feeTier
               (Float.of_int p.feeTier /. 10000.0);
             printf "  Liquidity: %s\n" p.liquidity;
             printf "  sqrtPrice: %s\n" p.sqrtPrice;
...
let price_command : string * Command.t =
  ...
  , Command.async
      ...
      (let%map_open.Command pool_id =
         flag "--pool-id" (required string) ~doc:"STRING Pool address"
       in
         fun () ->
           Rest.pool_by_id ~cfg ~pool_id
           >>| function
...
let quote_command : string * Command.t =
  ...
  , Command.async
      ...
      (let%map_open.Command pool_id =
         flag "--pool-id" (required string) ~doc:"STRING Pool address"
       and amount = flag "--amount" (required float) ~doc:"FLOAT Amount of input token"
       and token_in = flag "--from" (required string) ~doc:"STRING Input token symbol"
       and token_out = flag "--to" (required string) ~doc:"STRING Output token symbol" in
         fun () ->
           Rest.pool_by_id ~cfg ~pool_id
           >>| function
           | Ok pool ->
...
let recent_swaps_command : string * Command.t =
  ...
  , Command.async
      ...
      (let%map_open.Command pool_id =
       ...
       in
         fun () ->
           Rest.recent_swaps ~cfg ~pool_id ~first ()
           >>| function
           | Ok swaps ->
                   ...
                   let spread = Order_book.Book.spread book in
                     ...
                     let bids = Order_book.Book.best_n_bids book ~n:depth () in
                       List.iter
                         ...
                         ~f:
                           (fun
                             (level : Exchange_common.Order_book_base.Price_level.t) ->
                           printf "    %.8f  |  %.6f\n" level.price level.volume);
                       Deferred.unit
             | Error e, _ ->
               eprintf "Error: %s\n" (Sexp.to_string_hum (Rest.sexp_of_error e));
...
let session_command : string * Command.t =
  ...
  , Command.async
      ...
      (let%map_open.Command pool_id =
         flag "--pool-id" (required string) ~doc:"STRING Pool address to monitor"
       in
         fun () ->
                         ...
                         (match result with
                          | Ok book ->
                            ...
                            let ask = Order_book.Book.best_ask book in
                              printf
                                "[book] bid=%.8f ask=%.8f mid=%.8f spread=%.8f\n"
                                bid.price
                                ask.price
                                (Order_book.Book.mid_price book)
                                (Order_book.Book.spread book)
                          | Error err -> printf "[book] error: %s\n" err);
                         Deferred.unit))
                  | None -> ());
                 Deferred.never ()) )
...
let swap_command : string * Command.t =
  ...
  , Command.async
      ...
      (let%map_open.Command pool_id =
       ...
       and amount = flag "--amount" (required float) ~doc:"FLOAT Amount of input token"
       and token_in = flag "--from" (required string) ~doc:"STRING Input token symbol"
       and token_out = flag "--to" (required string) ~doc:"STRING Output token symbol"
       and slippage =
         flag
           "--slippage"
           (optional_with_default 0.5 float)
           ~doc:"FLOAT Max slippage in percent (default: 0.5)"
       and dry_run = flag "--dry-run" no_arg ~doc:" Show quote without executing" in
         fun () ->
                 ...
                 (match pool_result with
                  ...
                  | Ok pool ->
                    ...
                    let token_in_addr, token_out_addr, decimals_in, decimals_out =
                      match
                        String.equal
                          (String.uppercase token_in)
                          (String.uppercase pool.token0.symbol)
                      with
                      | true ->
                        ( pool.token0.id
                        , pool.token1.id
                    ...
                    in
                      ...
                      (match
                         Pool_adapter.quote pool ~amount_in:amount ~token_in ~token_out
                       with
                       | Ok q ->
                         printf "Swap Quote:\n";
                         printf
                         ...
                         let min_out = q.amount_out *. (1.0 -. (slippage /. 100.0)) in
                           ...
                           (match dry_run with
                            ...
                            | false ->
                              ...
                              let amount_in_hex =
                                Ethereum.Abi.uint256_of_float ~decimals:decimals_in amount
                              in
                              let amount_out_min_hex =
                                Ethereum.Abi.uint256_of_float
                              ...
                              let _ = slippage_bps in
                                ...
                                let%bind result =
                                  Swap_router.exact_input_single ~cfg ~params
                                in
                                  (match result with
                                   | Ok tx_hash ->
                                     ...
                                     let%bind receipt =
                                       Swap_router.wait_for_receipt ~cfg ~tx_hash ()
                                     in
                                       (match receipt with
                                        | Ok (Some _receipt) ->
                                        ...
                                        | Error e ->
                                          eprintf
                                            "Error waiting for receipt: %s\n"
                                            (Sexp.to_string_hum
                                               (Swap_router.sexp_of_error e));
                                          Deferred.unit)
                                   | Error e ->
                                     eprintf
                                       "Swap failed: %s\n"
...
let command : Command.t =
  Command.group
    ~summary:"Uniswap V3 DEX Commands"
    [ pools_command
    ; pool_info_command
    ; price_command
    ; quote_command
