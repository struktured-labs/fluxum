...
let test_cfg () =
  ...
  test_assert
    ...
    (String.is_substring
       Uniswapv3.Cfg.ethereum_mainnet.subgraph_url
       ~substring:"uniswap-v3");
  test_assert
    "Production is mainnet"
    (String.equal
       Uniswapv3.Cfg.production.subgraph_url
       Uniswapv3.Cfg.ethereum_mainnet.subgraph_url);
  test_assert
    "Polygon URL"
  ...
  test_assert "Default no API key" (Option.is_none Uniswapv3.Cfg.production.api_key);
  let with_key =
    Uniswapv3.Cfg.with_api_key ~api_key:"test_key" Uniswapv3.Cfg.production
  in
    test_assert
      "Can set API key"
  ...
  let err_str = Sexplib.Sexp.to_string_hum (Uniswapv3.Rest.sexp_of_error http_err) in
    ...
    let net_str = Sexplib.Sexp.to_string_hum (Uniswapv3.Rest.sexp_of_error network_err) in
      ...
      let json_str = Sexplib.Sexp.to_string_hum (Uniswapv3.Rest.sexp_of_error json_err) in
        ...
        let gql_str =
          Sexplib.Sexp.to_string_hum (Uniswapv3.Rest.sexp_of_error graphql_err)
        in
          test_assert
