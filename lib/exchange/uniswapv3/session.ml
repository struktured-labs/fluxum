          ...
          ~create_pipe:(fun () ->
            ...
            let%bind token_infos =
              Deferred.List.filter_map symbols ~how:`Sequential ~f:(fun pool_id ->
                ...
                | Error _ -> None)
              >>| List.concat
              >>| List.dedup_and_sort ~compare:(fun (a, _, _) (b, _, _) ->
                String.compare a b)
            in
            ...
            let rec poll_loop () =
              (* ETH balance *)
              let%bind eth_bal =
                Ethereum.Rpc.eth_get_balance ~rpc_url ~address:owner
                >>| function
                | Ok hex ->
                  let wei =
                    Float.of_string
              ...
              let%bind token_bals =
                Deferred.List.filter_map
                  ...
                  ~f:(fun (token_addr, symbol, decimals) ->
                    Ethereum.Erc20.balance_of ~rpc_url ~token:token_addr ~owner
                    >>| function
                    | Ok hex ->
                      let raw =
                        Float.of_string
                      ...
                      let amount = raw /. divisor in
                        (match Float.(amount > 0.) with
                         | true ->
                           Some
                             { Fluxum.Types.Balance.currency= symbol
                             ; available= amount
                             ; locked= 0.0
                             ; total= amount
                             ; venue= Fluxum.Types.Venue.Uniswap_v3 }
             ...
             Auto_restart.pipe
               ...
               ~create_pipe:(fun () ->
                 ...
                 let rec poll_loop () =
                   ...
                   | Error err ->
                     Log.Global.error
                       "Uniswap V3 market data poll failed for %s: %s"
                       pool_id
                       (Sexp.to_string (Rest.sexp_of_error err));
           ...
           let pipe =
             Auto_restart.pipe
               ...
               ~create_pipe:(fun () ->
                 ...
                 let rec poll_loop () =
                   Rest.recent_swaps ~cfg ~pool_id ~first:20 ()
                   >>= function
                   | Ok swaps ->
                     (* Only emit new swaps since last poll *)
                     let new_swaps =
                       match String.is_empty !last_seen with
                       | true -> swaps
                       | false ->
                   ...
                   | Error err ->
                     Log.Global.error
                       "Uniswap V3 trade poll failed for %s: %s"
                       pool_id
                       (Sexp.to_string (Rest.sexp_of_error err));
                     after (Time_float_unix.Span.of_sec 15.0) >>= poll_loop
                 in
    ...
    let ledger =
      Fluxum.Types.Symbol.Map.of_alist_exn
        (List.map symbols ~f:(fun pool_id ->
           ...
           let book_pipe =
             match Map.find order_books pool_id with
             | Some pipe -> pipe
             | None ->
               let reader, _writer = Pipe.create () in
                 reader
