...
module Adapter = struct
  ...
  let create ~cfg ?(pool_ids = []) () = {cfg; pool_ids}
        ...
        (match pool_result with
         ...
         | Ok pool ->
           let decimals =
             match
               String.equal
                 (String.lowercase req.token_in)
                 (String.lowercase pool.token0.id)
             with
           ...
           let spot_result =
             Pool_common.Concentrated.price_from_sqrt_price_x96
               ~sqrt_price_x96:pool.sqrtPrice
               ~decimals0:pool.token0.decimals
               ~decimals1:pool.token1.decimals
           in
           let amount_out_min_hex =
             match spot_result with
             | Error _ -> "0"
             | Ok spot ->
               let is_token0_in =
                 String.equal
           ...
           let%bind swap_result = Swap_router.exact_input_single ~cfg:t.cfg ~params in
             (match swap_result with
              | Ok tx_hash -> return (Ok {Native.Order.tx_hash; amount_out= 0.0})
  ...
  let get_order_history _t ?symbol:_ ?limit:_ () = Deferred.return (Ok [])
  ...
  let get_my_trades t ~symbol:pool_id ?limit () =
    ...
    | Some sender ->
      let first = Option.value limit ~default:100 in
        Rest.swaps_for_sender ~cfg:t.cfg ~sender ~first ()
        >>| (function
         | Ok swaps ->
           (* Filter by pool_id if needed *)
           let filtered =
             List.filter swaps ~f:(fun (_swap : Uni_types.swap) ->
  ...
  let get_symbols t () =
    Rest.pools ~cfg:t.cfg ~first:100 ()
    >>| function
    | Ok pools -> Ok pools
    | Error e -> Error e
  ...
  let get_order_book t ~symbol:pool_id ?limit:_ () =
    ...
    let%bind tick_result = Rest.pool_ticks ~cfg:t.cfg ~pool_id () in
      ...
      | Ok pool, Ok ticks -> return (Ok (Order_book.Book.of_pool_and_ticks ~pool ~ticks))
  ...
  let cancel_all_orders _t ?symbol:_ () = Deferred.return (Ok 0)
  ...
  let get_deposit_address (_ : t) ~currency:_ ?network:_ () =
    Deferred.return
      (Error (`GraphQL "Uniswap V3 is an EVM DEX - use wallet address for deposits"))

  let withdraw (_ : t) ~currency:_ ~amount:_ ~address:_ ?tag:_ ?network:_ () =
    Deferred.return
      (Error (`GraphQL "Uniswap V3 is an EVM DEX - use wallet for transfers"))

  let get_deposits (_ : t) ?currency:_ ?limit:_ () =
    Deferred.return
      (Error (`GraphQL "Uniswap V3 is an EVM DEX - use blockchain explorer"))
  ...
  module Normalize = struct
    ...
    let order_from_status (status : Native.Order.status)
      ...
      Ok
        ...
        ; time_in_force= Types.Time_in_force.GTC
        ; qty=
            (match Float.of_string_opt status.amount0 with
             | Some f -> Float.abs f
             | None -> 0.0)
        ; filled=
            (match Float.of_string_opt status.amount0 with
    ...
    let trade (swap : Native.Trade.t) : (Types.Trade.t, string) Result.t =
      ...
      let side =
        match Float.(amount0 < 0.) with
        | true -> Types.Side.Sell
        | false -> Types.Side.Buy
      in
      let price =
        match Float.(Float.abs amount0 > 0.) with
        | true -> Float.abs (amount1 /. amount0)
        | false -> 0.0
      in
      let ts =
        match Float.of_string_opt swap.timestamp with
        | Some epoch ->
    ...
    let balance (b : Native.Balance.t) : (Types.Balance.t, string) Result.t = Ok b
    ...
    let book_update (book_result : Native.Book.update) : Types.Book_update.t =
      ...
      | Ok book ->
        ...
        let bid_levels =
          List.map bids ~f:(fun (level : Order_book.Price_level.t) ->
            {Types.Book_update.price= level.price; qty= level.volume})
        in
          { venue= Venue.t
    ...
    let ticker (native : Native.Ticker.t) : (Types.Ticker.t, string) Result.t =
      ...
      let decimals1 = pool.token1.decimals in
        ...
        | Ok spot ->
          let high, low =
            match native.day_data with
            | [] -> (spot, spot)
            | data ->
              List.fold
                data
                ~init:(Float.neg_infinity, Float.infinity)
                ~f:(fun (h, l) d -> (Float.max h d.high, Float.min l d.low))
          in
          let volume =
            match native.day_data with
            | [] -> 0.0
    ...
    let public_trade (swap : Native.Public_trade.t)
    ...
    let error (e : Native.Error.t) : Types.Error.t =
      ...
      | `Network msg -> Types.Error.Transport (Failure msg)
      | `GraphQL msg ->
        Types.Error.Exchange_specific {venue= Venue.t; code= "graphql"; message= msg}
  end
end
