...
let normalize (pool : Native.pool) : (Pool_intf.Pool.t, string) Result.t =
  ...
  let fee_bps = 30 in
    Ok
      { Pool_intf.Pool.id= pool.address
      ; venue
      ; pool_type= Pool_intf.Pool_type.Constant_product
      ; token0=
          { Pool_intf.Token.address= pool.token0.address
          ; symbol= pool.token0.symbol
          ; decimals= pool.token0.decimals }
      ; token1=
          { Pool_intf.Token.address= pool.token1.address
...
let normalize_lb (pool : Native.lb_pool) : (Pool_intf.Pool.t, string) Result.t =
  ...
  let%bind totals =
    List.fold_result pool.bins ~init:(0.0, 0.0) ~f:(fun (x, y) bin ->
      let open Result.Let_syntax in
...
let spot_price (pool : Native.pool) ~(token_in : string) ~(token_out : string)
  ...
  let%bind reserve1 = Fluxum.Normalize_common.Float_conv.qty_of_string pool.reserve1 in
    match
      String.equal token_in pool.token0.address
      || String.equal token_in pool.token0.symbol
    with
    | true
      when String.equal token_out pool.token1.address
...
let quote
  ...
  let%bind reserve1 = Fluxum.Normalize_common.Float_conv.qty_of_string pool.reserve1 in
  let is_token0_in =
    String.equal token_in pool.token0.address || String.equal token_in pool.token0.symbol
  in
  ...
  let _ = token_out in
  let params =
    Amm_pricing.Constant_product.{reserve0= reserve_in; reserve1= reserve_out; fee_bps= 30}
  in
