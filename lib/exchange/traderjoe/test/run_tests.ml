...
let test_cfg () =
  printf "\n[Configuration]\n";
  test_assert
    "Avalanche mainnet URL"
    (String.is_substring
       Traderjoe.Cfg.avalanche_mainnet.api_url
       ~substring:"barn.traderjoexyz.com");
  test_assert
    "Avalanche mainnet network"
    (String.equal Traderjoe.Cfg.avalanche_mainnet.network "avalanche");
  test_assert
    "Production is mainnet"
    (String.equal
       Traderjoe.Cfg.production.network
       Traderjoe.Cfg.avalanche_mainnet.network);
  test_assert "Arbitrum network" (String.equal Traderjoe.Cfg.arbitrum.network "arbitrum");
  test_assert "BSC network" (String.equal Traderjoe.Cfg.bsc.network "bsc")
        ...
        in
          (match Traderjoe.Types.lb_pair_of_yojson lb_pair_json with
           | Ok pair ->
             ...
             test_assert
               "LB pair volumeUSD"
               Float.(abs (pair.volumeUSD -. 987654.32) < 1.0);
          ...
          in
            ...
            | Ok swap ->
              ...
              test_assert
                "Swap amountIn"
                (String.equal swap.amountIn "1000000000000000000");
              test_assert "Swap amountOut" (String.equal swap.amountOut "50000000");
              test_assert
                "Swap tokenIn"
                (String.equal swap.tokenIn "0xb31f66aa3c1e785363f0875a1b74e27b85fd66c7");
              test_assert
                "Swap tokenOut"
                (String.equal swap.tokenOut "0xb97ef9ef8734c71904d8002f8b6bc66dd9c48a6e")
