...
let normalize (pool : Native.pool) : (Pool_intf.Pool.t, string) Result.t =
  ...
  | token0 :: token1 :: _ ->
    ...
    let%bind weight0 =
      match token0.weight with
      | Some w -> Pool_common.Weighted.parse_weight w
      | None -> Ok 0.5
    in
    let%bind weight1 =
      match token1.weight with
      | Some w -> Pool_common.Weighted.parse_weight w
      | None -> Ok 0.5
    in
    ...
    let%bind tvl = Fluxum.Normalize_common.Float_conv.qty_of_string pool.totalLiquidity in
      Ok
        ...
        ; token0=
            { Pool_intf.Token.address= token0.address
            ; symbol= token0.symbol
            ; decimals= token0.decimals }
        ; token1=
            { Pool_intf.Token.address= token1.address
...
let spot_price (pool : Native.pool) ~(token_in : string) ~(token_out : string)
  ...
  let open Result.Let_syntax in
  let find_token addr =
    List.find pool.tokens ~f:(fun t ->
      String.equal t.address addr || String.equal t.symbol addr)
  in
    ...
    | Some tin, Some tout ->
      ...
      let%bind weight_in =
        match tin.weight with
        | Some w -> Pool_common.Weighted.parse_weight w
        | None -> Ok 0.5
      in
      let%bind weight_out =
        match tout.weight with
        | Some w -> Pool_common.Weighted.parse_weight w
        | None -> Ok 0.5
      in
    ...
    | Some tin, Some tout ->
      ...
      let balance_in =
        Amm_pricing.Weighted.
          {balance= balance_in_val; weight= weight_in_val; decimals= tin.decimals}
      in
      let balance_out =
        Amm_pricing.Weighted.
          {balance= balance_out_val; weight= weight_out_val; decimals= tout.decimals}
      in
      let%bind swap_fee = Fluxum.Normalize_common.Float_conv.of_string pool.swapFee in
      let fee_pct = swap_fee *. 100.0 in
      let params =
        Amm_pricing.Weighted.{tokens= [balance_in; balance_out]; swap_fee_pct= fee_pct}
      in
