...
let test_cfg () =
  ...
  test_assert
    ...
    (String.is_substring
       Balancer.Cfg.ethereum_mainnet.subgraph_url
       ~substring:"balancer-v2");
  test_assert
    "Production is mainnet"
    (String.equal
       Balancer.Cfg.production.subgraph_url
       Balancer.Cfg.ethereum_mainnet.subgraph_url);
  test_assert
    "Polygon URL"
...
let test_types () =
  ...
  in
    ...
    let pool_json =
      `Assoc
        [ ( "id"
          , `String "0x5c6ee304399dbdb9c8ef030ab642b10820db8f56000200000000000000000014"
          )
      ...
      in
        (match Balancer.Types.swap_of_yojson swap_json with
         | Ok swap ->
           ...
           test_assert
             "Swap tokenIn"
             (String.equal swap.tokenIn "0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2");
           test_assert
             "Swap tokenOut"
             (String.equal swap.tokenOut "0xba100000625a3754423978a60c9317c58a424e3d");
           test_assert
             "Swap tokenAmountIn"
             (String.equal swap.tokenAmountIn "1000000000000000000");
           test_assert
             "Swap tokenAmountOut"
             (String.equal swap.tokenAmountOut "5000000000000000000");
        ...
        in
          ...
          | Ok snapshot ->
            ...
            test_assert
              "Snapshot timestamp"
              (String.equal snapshot.timestamp "1640000000");
            test_assert
              "Snapshot liquidity"
              (String.equal snapshot.liquidity "123456789.12");
  ...
  let err_str = Sexplib.Sexp.to_string_hum (Balancer.Rest.sexp_of_error http_err) in
    ...
    let net_str = Sexplib.Sexp.to_string_hum (Balancer.Rest.sexp_of_error network_err) in
      ...
      let json_str = Sexplib.Sexp.to_string_hum (Balancer.Rest.sexp_of_error json_err) in
        ...
        let gql_str =
          Sexplib.Sexp.to_string_hum (Balancer.Rest.sexp_of_error graphql_err)
        in
          test_assert
            "GraphQL error to_sexp"
            (String.is_substring gql_str ~substring:"Query")
