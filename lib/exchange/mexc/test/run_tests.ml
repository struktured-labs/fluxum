...
let test_signature_hmac_sha256 () =
  ...
  let signature = Signature.sign ~api_secret:"test_secret" ~params in
    ...
    ignore
      (assert_int_equal 64 (String.length signature) "Signature is 64 hex characters");
...
let test_common_side () =
  printf "\n[Common] Side enum\n";
  ignore (assert_string_equal "BUY" (Common.Side.to_string `BUY) "BUY side to string");
  ignore (assert_string_equal "SELL" (Common.Side.to_string `SELL) "SELL side to string");
  ignore
    (assert_true (Option.is_some (Common.Side.of_string_opt "BUY")) "Parse BUY string");
  ignore
    (assert_true (Option.is_some (Common.Side.of_string_opt "SELL")) "Parse SELL string");
  ignore
    (assert_true
       (Option.is_none (Common.Side.of_string_opt "INVALID"))
...
let test_common_order_type () =
  ...
  ignore
    (assert_string_equal
       "LIMIT"
       (Common.Order_type.to_string `LIMIT)
       "LIMIT order type to string");
  ignore
    (assert_string_equal
       "MARKET"
       (Common.Order_type.to_string `MARKET)
       "MARKET order type to string");
  ignore
    (assert_string_equal
       "LIMIT_MAKER"
       (Common.Order_type.to_string `LIMIT_MAKER)
       "LIMIT_MAKER order type to string");
  ignore
    (assert_true
       (Option.is_some (Common.Order_type.of_string_opt "LIMIT"))
       "Parse LIMIT string");
  ignore
    (assert_true
       (Option.is_some (Common.Order_type.of_string_opt "MARKET"))
       "Parse MARKET string")

let test_common_time_in_force () =
  printf "\n[Common] Time in force enum\n";
  ignore (assert_string_equal "GTC" (Common.Time_in_force.to_string `GTC) "GTC to string");
  ignore (assert_string_equal "IOC" (Common.Time_in_force.to_string `IOC) "IOC to string");
  ignore (assert_string_equal "FOK" (Common.Time_in_force.to_string `FOK) "FOK to string")
...
let test_ws_parse_json_subscription_ack () =
  ...
  let json_msg = {|{"id":1,"msg":"spot@public.aggre.deals.v3.api@100ms@BTCUSDT"}|} in
    ...
    | Ws.Message.SubscriptionAck {id; msg} ->
      ...
      ignore
        (assert_true (String.is_substring msg ~substring:"BTCUSDT") "Ack contains symbol")
    | _ ->
      incr tests_run;
      incr tests_failed;
  ...
  let d = Ws.Protobuf.create_decoder "\x01" in
    (match Ws.Protobuf.read_varint d with
     ...
     | None ->
       incr tests_run;
       incr tests_failed;
       printf "  X FAIL: Failed to read varint\n");
...
let test_ws_protobuf_string () =
  ...
  let d = Ws.Protobuf.create_decoder "\x05hello" in
    ...
    | None ->
      incr tests_run;
      incr tests_failed;
      printf "  X FAIL: Failed to read string\n"
...
let test_ws_protobuf_tag () =
  ...
  let d = Ws.Protobuf.create_decoder "\x0a" in
    ...
    | Some (field, _) ->
      ...
      incr tests_run;
      incr tests_failed;
      printf "  X FAIL: Wrong wire type\n"
    | None ->
      incr tests_run;
      incr tests_failed;
      printf "  X FAIL: Failed to read tag\n"
...
let test_ws_deal_to_trade () =
  ...
  in
    ...
    | Ok trade ->
      ...
      ignore
        (assert_true
           (Fluxum.Types.Side.equal trade.side Fluxum.Types.Side.Buy)
           "Trade side is Buy")
    | Error msg ->
        ...
        in
          ...
          in
            ...
            let zero_qty_trade : Mexc.V1.Recent_trades.trade =
              ...
              ; price= "50000.0"
              ; qty= "0.0"
              ; (* Zero quantity - should be accepted *)
                time= 1234567890000L
              ; isBuyerMaker= true
              ; quoteQty= "0.0"
              ; isBestMatch= true
            ...
            in
              (match
                 Mexc.Fluxum_adapter.Adapter.Normalize.public_trade zero_qty_trade
               with
               | Ok trade ->
              ...
              in
                (match
                   Mexc.Fluxum_adapter.Adapter.Normalize.order_response bad_order_resp
                 with
                 | Error msg ->
