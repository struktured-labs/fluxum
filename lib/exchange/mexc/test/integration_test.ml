...
let test_server_time () =
  ...
  let cfg = Mexc.Cfg.production in
    Mexc.V1.Server_time.request cfg ()
    >>| function
    | `Ok resp ->
...
let test_depth () =
  ...
  let cfg = Mexc.Cfg.production in
    Mexc.V1.Depth.request cfg {symbol= "BTCUSDT"; limit= Some 5}
    >>| function
    | `Ok resp ->
...
let test_recent_trades () =
  printf "\n[REST] Recent Trades\n";
  let cfg = Mexc.Cfg.production in
    Mexc.V1.Recent_trades.request cfg {symbol= "BTCUSDT"; limit= Some 5}
    >>| function
    | `Ok trades ->
...
let test_ticker_24hr () =
  printf "\n[REST] 24hr Ticker\n";
  let cfg = Mexc.Cfg.production in
    Mexc.V1.Ticker_24hr.request cfg {symbol= Some "BTCUSDT"}
    >>| function
    | `Ok ticker ->
...
let test_exchange_info () =
  printf "\n[REST] Exchange Info\n";
  let cfg = Mexc.Cfg.production in
    Mexc.V1.Exchange_info.request cfg {symbol= Some "BTCUSDT"}
    >>| function
    | `Ok info ->
      ...
      let symbol_count = List.length info.symbols in
        ...
        (match symbol_count > 0 with
         | true ->
           let sym =
             match List.hd info.symbols with
             | Some x -> x
...
let test_adapter_ticker () =
  ...
  let adapter = Mexc.Fluxum_adapter.Adapter.create ~cfg ~symbols:["BTCUSDT"] () in
    Mexc.Fluxum_adapter.Adapter.get_ticker adapter ~symbol:"BTCUSDT" ()
    >>| function
    | Ok ticker_native ->
      (match Mexc.Fluxum_adapter.Adapter.Normalize.ticker ticker_native with
       ...
       | Error msg -> fail (sprintf "Failed to normalize ticker: %s" msg))
    | Error err ->
...
let test_adapter_order_book () =
  ...
  let cfg = Mexc.Cfg.production in
  let adapter = Mexc.Fluxum_adapter.Adapter.create ~cfg ~symbols:["BTCUSDT"] () in
    Mexc.Fluxum_adapter.Adapter.get_order_book adapter ~symbol:"BTCUSDT" ~limit:5 ()
    >>| function
    | Ok book_native ->
      (match Mexc.Fluxum_adapter.Adapter.Normalize.order_book book_native with
       | Ok book ->
         pass
           (sprintf "Bids: %d | Asks: %d" (List.length book.bids) (List.length book.asks));
         let spread = Fluxum.Types.Order_book.spread book in
           ...
           let mid = Fluxum.Types.Order_book.mid_price book in
             ...
             (match List.hd book.bids with
              | Some level ->
                pass (sprintf "Best bid: %.8f @ %.2f" level.volume level.price)
              | None -> ());
             (match List.hd book.asks with
              | Some level ->
                pass (sprintf "Best ask: %.8f @ %.2f" level.volume level.price)
              | None -> ())
       | Error msg -> fail (sprintf "Failed to normalize order book: %s" msg))
    | Error err ->
      let e = Mexc.Fluxum_adapter.Adapter.Normalize.error err in
        fail (sprintf "Error: %s" (Sexp.to_string_hum (Fluxum.Types.Error.sexp_of_t e)))
...
let test_adapter_recent_trades () =
  ...
  let cfg = Mexc.Cfg.production in
  let adapter = Mexc.Fluxum_adapter.Adapter.create ~cfg ~symbols:["BTCUSDT"] () in
    Mexc.Fluxum_adapter.Adapter.get_recent_trades adapter ~symbol:"BTCUSDT" ~limit:5 ()
    >>| function
    | Ok trades_native ->
      let trades_results =
        ...
        (match result_transpose trades_results with
         | Ok trades ->
           ...
           (match List.hd trades with
            ...
            | None -> fail "No trades")
         | Error msg -> fail (sprintf "Failed to normalize trades: %s" msg))
    | Error err ->
      let e = Mexc.Fluxum_adapter.Adapter.Normalize.error err in
        fail (sprintf "Error: %s" (Sexp.to_string_hum (Fluxum.Types.Error.sexp_of_t e)))
...
let test_adapter_symbols () =
  ...
  let adapter = Mexc.Fluxum_adapter.Adapter.create ~cfg ~symbols:[] () in
    ...
    | Ok symbols_native ->
      let symbols =
        List.filter_map symbols_native ~f:(fun s ->
...
let test_websocket_connection () =
  ...
  let streams = [Mexc.Ws.Stream.AggreDeals {symbol= "BTCUSDT"; frequency= Ms100}] in
    (* Try secure first, fall back to insecure if SSL fails *)
    Mexc.Ws.connect ~streams ()
    >>= fun result ->
    (match result with
     | Ok _ -> return result
     | Error _ ->
                  ...
                  | Mexc.Ws.Message.Data wrapper ->
                    (match wrapper.body with
                       ...
                       let deal_count = List.length deals.deals in
                         (match deal_count > 0 with
                          | true ->
                            ...
                            let deal =
                              match List.hd deals.deals with
                              | Some x -> x
                     ...
                     | Mexc.Ws.Message.AggreDepth depth ->
                       pass
                         (sprintf
                            "Depth update: %d bids, %d asks"
                            (List.length depth.bids)
                            (List.length depth.asks))
                     | Mexc.Ws.Message.LimitDepth depth ->
                       pass
                         (sprintf
                            "Depth snapshot: %d bids, %d asks"
                            (List.length depth.bids)
                            (List.length depth.asks))
                     | Mexc.Ws.Message.BookTicker ticker ->
                       pass
                         (sprintf
                            "Book ticker: bid=%s ask=%s"
                            ticker.bid_price
                            ticker.ask_price)
                     | Mexc.Ws.Message.Unknown _ -> pass "Received unknown message type")
                  | Mexc.Ws.Message.Pong -> pass "Received pong"
                  | Mexc.Ws.Message.Error err -> fail (sprintf "Parse error: %s" err)
                  | Mexc.Ws.Message.Raw _ -> pass "Received raw message");
...
let test_websocket_connection_recovery () =
  ...
  | Ok ws ->
    ...
    Clock.with_timeout (Time_float.Span.of_sec 2.0) (Pipe.read (Mexc.Ws.messages ws))
    >>| (function
     | `Timeout -> pass "No messages on empty streams (as expected)"
     | `Result `Eof -> pass "Connection closed on empty streams"
     | `Result (`Ok msg) ->
       let parsed = Mexc.Ws.parse_message msg in
         (match parsed with
...
let test_websocket_invalid_symbol () =
  ...
  in
    ...
    | Ok ws ->
      ...
      Clock.with_timeout (Time_float.Span.of_sec 3.0) (Pipe.read (Mexc.Ws.messages ws))
      >>| (function
       | `Timeout -> pass "No messages for invalid symbol (exchange may silently ignore)"
       | `Result `Eof -> pass "Connection closed for invalid symbol"
       | `Result (`Ok msg) ->
         let parsed = Mexc.Ws.parse_message msg in
           (match parsed with
            ...
            | _ -> pass "Exchange accepted invalid symbol"))
...
let run_tests () =
  ...
  test_adapter_ticker ()
  >>= fun () ->
  test_adapter_order_book ()
  >>= fun () ->
  test_adapter_recent_trades ()
  >>= fun () ->
  test_adapter_symbols ()
  >>= fun () ->
  (* WebSocket test *)
  test_websocket_connection ()
  >>= fun () ->
  (* WebSocket error handling tests *)
  test_websocket_invalid_url ()
  >>= fun () ->
  test_websocket_malformed_message ()
  >>= fun () ->
  test_websocket_connection_recovery ()
  >>= fun () ->
  test_websocket_invalid_symbol ()
  >>= fun () ->
  (* Summary *)
  printf "\n===========================================\n";
