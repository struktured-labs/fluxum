...
module Stream = struct
  ...
  let to_channel = function
    | AggreDeals {symbol; frequency} ->
      sprintf
        "spot@public.aggre.deals.v3.api.pb@%s@%s"
        (frequency_to_string frequency)
        (String.uppercase symbol)
    | AggreDepths {symbol; frequency} ->
      sprintf
        "spot@public.aggre.depth.v3.api.pb@%s@%s"
        (frequency_to_string frequency)
        (String.uppercase symbol)
    | LimitDepths {symbol; levels} ->
      sprintf "spot@public.limit.depth.v3.api.pb@%s@%d" (String.uppercase symbol) levels
    | BookTicker symbol ->
      sprintf "spot@public.bookTicker.v3.api@%s" (String.uppercase symbol)
    | MiniTicker symbol ->
      sprintf "spot@public.miniTicker.v3.api@%s" (String.uppercase symbol)
    | Kline {symbol; interval} ->
      sprintf "spot@public.kline.v3.api@%s@%s" interval (String.uppercase symbol)
end
...
module Parse = struct
  ...
  let parse_deal_item data =
    ...
    let time = ref 0L in
    let rec loop () =
      match remaining d <= 0 with
      | true -> ()
      | false ->
        (match read_tag d with
         | None -> ()
         | Some (1, LengthDelim) ->
           (match read_string d with
            | Some s -> price := s
            | None -> ());
           loop ()
         | Some (2, LengthDelim) ->
         ...
         | Some (3, Varint) ->
           (match read_varint d with
            | Some v -> trade_type := Int64.to_int_exn v
            | None -> ());
           loop ()
         | Some (4, Varint) ->
           (match read_varint d with
        ...
        (match read_tag d with
         ...
         | Some (1, LengthDelim) ->
           (match read_bytes d with
            | Some item_data -> deals := parse_deal_item item_data :: !deals
            | None -> ());
           loop ()
         | Some (2, LengthDelim) ->
           (match read_string d with
    ...
    let rec loop () =
      ...
      | false ->
        (match read_tag d with
         ...
         | Some (1, LengthDelim) ->
           (match read_bytes d with
            | Some item_data -> asks := parse_depth_item item_data :: !asks
            | None -> ());
           loop ()
         | Some (2, LengthDelim) ->
           (match read_bytes d with
            | Some item_data -> bids := parse_depth_item item_data :: !bids
            | None -> ());
           loop ()
         | Some (3, LengthDelim) ->
           (match read_string d with
...
let parse_message (data : string) : Message.t =
  ...
  | true ->
    (try
       let json = Yojson.Safe.from_string data in
         ...
         | `String msg ->
           let id =
             Yojson.Safe.Util.member "id" json
  ...
  | false ->
    ...
    (try Message.Data (Parse.parse_wrapper data) with
     | _ ->
       Message.Error
         (sprintf "Failed to parse protobuf message of length %d" (String.length data)))
...
module Subscribe = struct
  ...
  let ping () : string = Yojson.Safe.to_string (`Assoc [("method", `String "PING")])
end
...
let apply_depth_to_book (book : Order_book.Book.t) (depth : Message.aggre_depth)
  : (Order_book.Book.t, string) Result.t
  =
...
let deal_to_trade ~symbol:sym (deal : Message.deal_item)
  : (Fluxum.Types.Trade.t, string) Result.t
  =
  let open Result.Let_syntax in
  let side =
    match deal.trade_type = 1 with
    | true -> Fluxum.Types.Side.Buy
    | false -> Fluxum.Types.Side.Sell
  in
  let ts =
    Time_float_unix.of_span_since_epoch
      (Time_float_unix.Span.of_ms (Int64.to_float deal.time))
  in
  ...
  let%bind qty = Fluxum.Normalize_common.Float_conv.qty_of_string deal.quantity in
    Ok
      { Fluxum.Types.Trade.venue= Fluxum.Types.Venue.Mexc
      ; symbol= sym
      ; side
      ; price
      ; qty
