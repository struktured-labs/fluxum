        ...
        (Pipe.iter combined_pipe ~f:(fun event ->
           ...
           | `Trade trade ->
             ...
             (match
                ...
                let%bind price =
                  Fluxum.Normalize_common.Float_conv.price_of_string trade.price
                in
                let%bind qty =
                  Fluxum.Normalize_common.Float_conv.qty_of_string trade.qty
                in
                  Ok (price, qty)
              with
              | Error err ->
...
let from_balances ?(notional_currency = "USD") (balances : V1.Account.balance list) : t =
  List.fold balances ~init:Fluxum.Types.Symbol.Map.empty ~f:(fun acc account ->
    ...
    | Error err ->
      Log.Global.error "MEXC ledger: Failed to parse balance for %s: %s" account.asset err;
      acc
    | Ok position ->
    ...
    List.fold sorted_trades ~init:Fluxum.Types.Symbol.Map.empty ~f:(fun acc trade ->
      let symbol = trade.symbol in
        ...
        | Ok (price, qty) ->
          ...
          in
          let fee_usd = 0. in
          (* MEXC doesn't provide fee in trade data *)
          let avg_trade_price =
