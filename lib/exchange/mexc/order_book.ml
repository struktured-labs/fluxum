(** MEXC Order Book - Implements unified Order_book_intf *)
...
module Book = struct
  ...
  let empty ?timestamp ?(epoch = 0) symbol =
    let update_time =
      Option.value_or_thunk timestamp ~default:Time_float_unix.now
    in
    { symbol
  ...
  let add ?timestamp t ~side ~price ~size = update ?timestamp t ~side ~price ~size
  ...
  (** Get top N bids *)
  let best_n_bids t ~n () =
    Map.to_alist t.bids |> (fun list -> List.take list n) |> List.map ~f:snd

  (** Get top N asks *)
  let best_n_asks t ~n () =
    Map.to_alist t.asks |> (fun list -> List.take list n) |> List.map ~f:snd
  ...
  let market_price t ~side ~volume =
    let levels =
      ...
      | Fluxum.Types.Side.Buy ->
        Map.to_alist t.asks (* Buying means taking asks *)
      | Fluxum.Types.Side.Sell ->
        Map.to_alist t.bids (* Selling means taking bids *)
    in
    let rec accumulate remaining_vol acc_cost levels =
      ...
      | (_, level) :: rest ->
        ...
        if Float.(remaining_vol <= available)
        then
    ...
    let avg_price =
      if Float.(filled_volume > 0.) then total_cost /. filled_volume else 0.
    in
  ...
  let total_volume_at_price_level t ~side ~price =
    match side with
    | `Bid -> Map.find t.bids price |> Option.value ~default:Price_level.empty
    | `Ask -> Map.find t.asks price |> Option.value ~default:Price_level.empty
  ...
  (** Apply MEXC depth snapshot *)
  let apply_depth_snapshot t (depth : V1.Depth.T.response) =
    let t = { t with bids = Bid_price_map.empty; asks = Ask_price_map.empty } in
    let t =
      List.fold depth.bids ~init:t ~f:(fun acc (price_str, qty_str) ->
        let price = Float.of_string price_str in
        let size = Float.of_string qty_str in
        set acc ~side:`Bid ~price ~size)
    in
    let t =
      List.fold depth.asks ~init:t ~f:(fun acc (price_str, qty_str) ->
        let price = Float.of_string price_str in
        let size = Float.of_string qty_str in
        set acc ~side:`Ask ~price ~size)
    in
    { t with last_update_id = depth.lastUpdateId }
...
module Books = struct
  ...
  let add ?timestamp t ~symbol ~side ~price ~size =
    let book =
      Map.find t symbol |> Option.value ~default:(Book.empty ?timestamp symbol)
    in
