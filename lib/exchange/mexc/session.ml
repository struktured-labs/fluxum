...
module Events = struct
  ...
  let create
    ...
    let balance_pipe =
      Auto_restart.pipe
        ~name:"mexc_balance_poll"
        ~create_pipe:(fun () ->
          let reader, writer = Pipe.create () in
          let rec poll_loop () =
            V1.Account.request (module Cfg) ()
            >>= function
            | `Ok resp ->
             ...
             Auto_restart.pipe
               ...
               ~create_pipe:(fun () ->
                 ...
                 in
                              ...
                              (match wrapper.body with
                               | Ws.Message.LimitDepth depth ->
                                 (match Ws.apply_limit_depth_to_book !book_ref depth with
                                  ...
                                  | Error err -> Pipe.write book_writer (Error err))
                   ...
                   | Error err ->
                     Log.Global.error
                       "Failed to connect order book for %s: %s"
                       symbol
                       (Error.to_string_hum err);
                     let reader, _writer = Pipe.create () in
                       reader)
                            ...
                            | Ws.Message.Data wrapper ->
                              (match wrapper.body with
                               | Ws.Message.AggreDeals deals_msg ->
                                 ...
                                 let trades =
                                   List.filter_map deals_msg.deals ~f:(fun deal ->
                                     let trade : V1.My_trades.trade =
                                       { symbol
                                       ; id= deal.time
                                       ; (* Use timestamp as ID *)
                                         orderId= ""
                                       ; (* Not available from public stream *)
                                         price= deal.price
                                       ; qty= deal.quantity
                                       ; quoteQty= "0"
                                       ; commission= "0"
