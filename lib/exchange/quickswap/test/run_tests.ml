...
let test_cfg () =
  ...
  test_assert
    "Production is Polygon"
    (String.equal
       Quickswap.Cfg.production.subgraph_url
       Quickswap.Cfg.polygon.subgraph_url);
  test_assert
    "Polygon zkEVM URL"
  ...
  let err_str = Sexplib.Sexp.to_string_hum (Quickswap.Rest.sexp_of_error http_err) in
    ...
    let net_str = Sexplib.Sexp.to_string_hum (Quickswap.Rest.sexp_of_error network_err) in
      ...
      let json_str = Sexplib.Sexp.to_string_hum (Quickswap.Rest.sexp_of_error json_err) in
        ...
        let gql_str =
          Sexplib.Sexp.to_string_hum (Quickswap.Rest.sexp_of_error graphql_err)
        in
          test_assert
