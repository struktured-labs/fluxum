...
let test_cfg () =
  ...
  test_assert
    "Production is Fantom"
    (String.equal
       Spookyswap.Cfg.production.subgraph_url
       Spookyswap.Cfg.fantom.subgraph_url)
      ...
      in
        (match Spookyswap.Types.swap_of_yojson swap_json with
         | Ok swap ->
           ...
           test_assert
             "Swap amount0In"
             (String.equal swap.amount0In "1000000000000000000");
        ...
        in
          ...
          | Ok position ->
            ...
            test_assert
              "Position balance"
              (String.equal position.liquidityTokenBalance "7071067811865475244")
...
let test_error_types () =
  ...
  let err_str = Sexplib.Sexp.to_string_hum (Spookyswap.Rest.sexp_of_error http_err) in
    ...
    let net_str =
      Sexplib.Sexp.to_string_hum (Spookyswap.Rest.sexp_of_error network_err)
    in
      test_assert
      ...
      let json_str =
        Sexplib.Sexp.to_string_hum (Spookyswap.Rest.sexp_of_error json_err)
      in
        test_assert
          "JSON error to_sexp"
        ...
        let gql_str =
          Sexplib.Sexp.to_string_hum (Spookyswap.Rest.sexp_of_error graphql_err)
        in
          test_assert
