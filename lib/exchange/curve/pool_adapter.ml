...
let normalize (pool : Native.pool) : (Pool_intf.Pool.t, string) Result.t =
  try
    ...
    | coin0 :: coin1 :: _ ->
      ...
      let virtual_price = Float.of_string pool.virtualPrice in
...
let quote (pool : Native.pool) ~(amount_in : float) ~(token_in : string) ~(token_out : string)
  ...
  try
    ...
    | (Some (i, _), Some (j, _)) when i <> j ->
      ...
      let amp = match pool.amplificationCoefficient with
        | Some a -> Int.of_string a
        | None -> 100  (* Default amplification *)
      in
