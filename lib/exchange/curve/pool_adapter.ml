  ...
  | coin0 :: coin1 :: _ ->
    ...
    let fee_bps = 4 in
      Ok
        ...
        ; token0=
            { Pool_intf.Token.address= coin0.address
            ; symbol= coin0.symbol
            ; decimals= coin0.decimals }
        ; token1=
            { Pool_intf.Token.address= coin1.address
            ; symbol= coin1.symbol
...
let spot_price (pool : Native.pool) ~(token_in : string) ~(token_out : string)
  ...
  (* Find token indices *)
  let token_in_idx =
    List.findi pool.coins ~f:(fun _ c ->
      String.equal c.address token_in || String.equal c.symbol token_in)
  in
  let token_out_idx =
    List.findi pool.coins ~f:(fun _ c ->
      String.equal c.address token_out || String.equal c.symbol token_out)
  in
...
let quote
  ...
  in
    ...
    | Some (i, _), Some (j, _) when i <> j ->
      ...
      let amp =
        Pool_common.Stable.parse_amplification
          ?amp_str:pool.amplificationCoefficient
          ~default:100
          ()
      in
      let params =
