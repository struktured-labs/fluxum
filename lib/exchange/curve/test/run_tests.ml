...
let test_cfg () =
  ...
  test_assert "Polygon network" (String.equal Curve.Cfg.polygon.network "polygon");
  test_assert "Arbitrum network" (String.equal Curve.Cfg.arbitrum.network "arbitrum");
  test_assert "Optimism network" (String.equal Curve.Cfg.optimism.network "optimism");
  test_assert "Avalanche network" (String.equal Curve.Cfg.avalanche.network "avalanche");
  test_assert "Fantom network" (String.equal Curve.Cfg.fantom.network "fantom")
  ...
  in
    ...
    let pool_json =
      `Assoc
        ...
        ; ( "coinAddresses"
          , `List
              [ `String "0x6b175474e89094c44da98b954eedeac495271d0f"
    ...
    in
      (match Curve.Types.pool_data_of_yojson pool_json with
       | Ok pool ->
         ...
         test_assert
           "Pool coin addresses count"
           (Int.equal (List.length pool.coinAddresses) 2);
         test_assert "Pool usdTotal" Float.(abs (pool.usdTotal -. 1234567890.12) < 1.0)
       | Error msg -> fail (sprintf "Pool parsing failed: %s" msg));
      let volume_json =
        `Assoc [("date", `String "2024-01-01"); ("volume", `Float 12345678.90)]
      in
        ...
        in
          ...
          | Ok trade ->
            ...
            test_assert
              "Trade tokenIn"
              (String.equal trade.tokenIn "0x6b175474e89094c44da98b954eedeac495271d0f");
            test_assert
              "Trade tokenOut"
              (String.equal trade.tokenOut "0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48");
            test_assert
              "Trade amountIn"
              (String.equal trade.amountIn "1000000000000000000");
...
let test_error_types () =
  ...
  let err_str = Sexplib.Sexp.to_string_hum (Curve.Rest.sexp_of_error http_err) in
    ...
    let net_str = Sexplib.Sexp.to_string_hum (Curve.Rest.sexp_of_error network_err) in
      test_assert
        "Network error to_sexp"
        (String.is_substring net_str ~substring:"Connection");
      ...
      let json_str = Sexplib.Sexp.to_string_hum (Curve.Rest.sexp_of_error json_err) in
        test_assert
          "JSON error to_sexp"
          (String.is_substring json_str ~substring:"Invalid");
        ...
        let api_str = Sexplib.Sexp.to_string_hum (Curve.Rest.sexp_of_error api_err) in
          test_assert
            "API error to_sexp"
            (String.is_substring api_str ~substring:"failed")
