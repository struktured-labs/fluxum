...
let float_equal ?(tolerance = 0.0001) a b = Float.(abs (a - b) < tolerance)

let%test_module "Price_level" =
  (module struct
...
let%test_module "Book - Set Operations" =
  (module struct
...
let%test_module "Book - Best Prices" =
  (module struct
    ...
    let%test "best prices after removal" =
      ...
      let book = Book.set book ~side:`Bid ~price:50000. ~size:1.0 in
      let book = Book.set book ~side:`Bid ~price:50001. ~size:0.5 in
      let book = Book.set book ~side:`Bid ~price:50001. ~size:0.0 in
      (* Remove top *)
      let best = Book.best_bid book in
        float_equal (Price_level.price best) 50000.
  end)

let%test_module "Book - Spread and Mid Price" =
  (module struct
...
let%test_module "Book - Batch Operations" =
  (module struct
...
let%test_module "Book - Accessors" =
  (module struct
...
let%test_module "Book - Metadata Handling" =
  (module struct
...
let%test_module "Books - Multi-Symbol" =
  (module struct
    ...
    let%test "update_book modifies existing book" =
      ...
      let books = Books.set_book books book in
      let books =
        Books.update_book books "BTC/USD" ~f:(fun book ->
