...
let on_rate_limit_error t =
  ...
  let new_backoff = t.current_backoff *. t.config.backoff_multiplier *. (1.0 +. jitter) in
    ...
    Log.Global.info
      "Rate limit hit, backing off for %.1fs (next: %.1fs)"
      t.current_backoff
      new_backoff
...
let reset_backoff t =
  ...
  | false -> t.current_backoff <- t.config.initial_backoff_sec

(** Get current state for monitoring *)
let status t =
  refill t;
  sprintf
    "tokens=%.1f/%d backoff=%.1fs in_backoff=%b"
    t.tokens
    t.config.burst_size
    t.current_backoff
    t.in_backoff
