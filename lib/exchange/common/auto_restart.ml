...
module Health_monitor = struct
  ...
  let timeout_triggered t = Ivar.read !(t.trigger_ivar)
               ...
               let%bind ping_result = send_ping () in
                 (match ping_result with
                  | Error err ->
                    ...
                    Log.Global.error
                      "health_monitor: ping send failed: %s"
                         ...
                         let ping_time = Option.value_exn t.last_ping_sent in
                           (match Time_float_unix.(t.last_pong >= ping_time) with
                            ...
                            | false ->
                              ...
                              Log.Global.info
                                "health_monitor: pong timeout detected (attempt %d/%d)"
                                t.consecutive_timeouts
                                t.config.max_reconnect_attempts;
                              (match
                               ...
                               | true ->
                                 ...
                                 Log.Global.error
                                   "health_monitor: max timeouts reached, triggering \
                                    reconnect";
                                 t.running <- false;
                                 Ivar.fill_if_empty !(t.trigger_ivar) ();
                                 Deferred.unit
...
let pipe_with_health ~name ~config ~create_pipe () =
  ...
  let rec restart_loop ~attempt () =
    ...
    | true ->
      Log.Global.error
        "auto_restart_pipe[%s]: max reconnect attempts (%d) exceeded, giving up"
        name
    ...
    | false ->
      ...
      let%bind connection_result = create_pipe () in
        (match connection_result with
         | Error err ->
           Log.Global.error
             "auto_restart_pipe[%s]: connection failed: %s"
             name
             (Error.to_string_hum err);
         ...
         | Ok (source_pipe, send_ping, on_pong_callback_setter) ->
           Log.Global.info
             "auto_restart_pipe[%s]: connected, starting health monitor"
             name;
           Health_monitor.reset health_monitor;
           (* Register pong callback *)
           on_pong_callback_setter (fun () ->
             Health_monitor.on_pong_received health_monitor);
           ...
           let%bind reason = Deferred.any [eof_deferred; timeout_deferred] in
             ...
             in
               Log.Global.info
                 "auto_restart_pipe[%s]: %s detected, restarting in %s"
                 name
                 reason_str
                 (Time_float_unix.Span.to_short_string config.reconnect_delay);
               let%bind () = after config.reconnect_delay in
                 restart_loop
                   ~attempt:1
