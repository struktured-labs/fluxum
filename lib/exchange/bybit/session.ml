(** Bybit Session - Implements unified Session_intf with auto-reconnecting streams *)
    ...
    let ledger =
      Fluxum.Types.Symbol.Map.of_alist_exn (
        List.map symbols ~f:(fun symbol ->
          ...
          (* Get order book pipe for this symbol *)
...
let create
    ...
    ()
  : t Deferred.t =
  Log.Global.info "Bybit Session.create: symbols=%d" (List.length symbols);
...
(** Close session *)
let close t =
  Log.Global.info "Bybit Session.close: closing session";
