...
let test_ledger_entry_creation () =
  ...
  let _ =
    assert_equal
      ~equal:[%equal: Types.Side.t option]
      ~sexp_of_t:[%sexp_of: Types.Side.t option]
      None
      entry.side
      "Empty entry has no side"
  in
  let _ =
    assert_equal
  ...
  let _ =
    assert_equal
      ...
      None
      entry.package_price
      "Empty entry has no package_price"
  in
    ()
...
let test_ledger_simple_trades () =
  ...
  let _ =
    assert_float_equal 50000.0 entry.pnl_spot "Buy: pnl_spot = position * price = $50k"
  in
...
let test_ledger_fees () =
  ...
  let entry2 =
    Ledger.Entry.on_trade entry2 ~price:3000.0 ~side:Buy ~qty:10.0 ~fee_usd:100.0
  in
  let _ =
    assert_float_equal (-30100.0) entry2.notional "Large fee: notional = -$30,100"
  in
  let _ =
    assert_float_equal 30100.0 entry2.cost_basis "Large fee: cost_basis = $30,100"
  in
  (* Sell at same price *)
...
let test_ledger_partial_fills () =
  ...
  let _ =
    assert_float_equal 50000.0 entry.cost_basis "Sell 1/2 remaining: cost_basis = $50k"
  in
  (* Sell last BTC *)
...
let test_ledger_cost_basis_accounting () =
  ...
  let _ =
    assert_float_equal 170000.0 entry.cost_basis "After buy 2 more: cost_basis = $170k"
  in
  let _ = assert_float_equal 3.0 entry.running_qty "Running qty = 3.0" in
...
let test_ledger_multi_symbol () =
  ...
  let ledger =
    Ledger.on_trade ledger ~symbol:"BTC/USD" ~price:55000.0 ~side:Sell ~qty:1.0
  in
  let btc = Map.find_exn ledger "BTC/USD" in
...
let test_order_book_creation () =
  ...
  let _ =
    assert_float_equal
      0.0
      (Order_book.Price_level.volume best_ask)
      "Empty book: best_ask volume = 0"
  in
...
let test_order_book_bid_sorting () =
  ...
  let _ =
    assert_float_equal
      1.5
      (Order_book.Price_level.volume (List.nth_exn bid_levels 0))
      "1st bid volume = 1.5"
  in
  let _ =
    assert_float_equal
      1.0
      (Order_book.Price_level.volume (List.nth_exn bid_levels 1))
      "2nd bid volume = 1.0"
  in
  let _ =
    assert_float_equal
      2.0
      (Order_book.Price_level.volume (List.nth_exn bid_levels 2))
      "3rd bid volume = 2.0"
  in
  (* best_n_bids should return same ordering *)
  let top_3 = Order_book.Book.best_n_bids book ~n:3 () in
  let _ =
    assert_float_equal 51000.0 (List.nth_exn top_3 0).price "best_n_bids 1st = $51k"
  in
  let _ =
    assert_float_equal 50000.0 (List.nth_exn top_3 1).price "best_n_bids 2nd = $50k"
  in
  let _ =
    assert_float_equal 49000.0 (List.nth_exn top_3 2).price "best_n_bids 3rd = $49k"
  in
    ()
...
let test_order_book_ask_sorting () =
  ...
  let _ =
    assert_float_equal
      1.5
      (Order_book.Price_level.volume (List.nth_exn ask_levels 0))
      "1st ask volume = 1.5"
  in
  let _ =
    assert_float_equal
      1.0
      (Order_book.Price_level.volume (List.nth_exn ask_levels 1))
      "2nd ask volume = 1.0"
  in
  let _ =
    assert_float_equal
      2.0
      (Order_book.Price_level.volume (List.nth_exn ask_levels 2))
      "3rd ask volume = 2.0"
  in
  (* best_n_asks should return same ordering *)
  let top_3 = Order_book.Book.best_n_asks book ~n:3 () in
  let _ =
    assert_float_equal 51000.0 (List.nth_exn top_3 0).price "best_n_asks 1st = $51k"
  in
  let _ =
    assert_float_equal 52000.0 (List.nth_exn top_3 1).price "best_n_asks 2nd = $52k"
  in
  let _ =
    assert_float_equal 53000.0 (List.nth_exn top_3 2).price "best_n_asks 3rd = $53k"
  in
    ()
...
let test_order_book_operations () =
  ...
  let _ =
    assert_float_equal
      0.0
      (Order_book.Price_level.price best)
      "Size=0 removes level (price=0)"
  in
  let _ =
    assert_float_equal
      0.0
      (Order_book.Price_level.volume best)
      "Size=0 removes level (volume=0)"
  in
...
let test_fluxum_adapter_venue () =
  ...
  let _ =
    assert_false (Types.Venue.equal Types.Venue.Kraken venue) "Venue is not Kraken"
  in
  let _ =
    assert_false (Types.Venue.equal Types.Venue.Gemini venue) "Venue is not Gemini"
  in
  let _ =
    assert_false (Types.Venue.equal Types.Venue.Binance venue) "Venue is not Binance"
  in
    ()
   ...
   | Ok side ->
     let _ =
       assert_equal
         ...
         Types.Side.Sell
         side
         "Side 'SELL' = Sell (case insensitive)"
     in
       ()
   | Error msg ->
     printf "  FAIL: Should parse 'SELL': %s\n" msg;
   ...
   | Ok s ->
     let _ =
       assert_equal
         ~equal:order_status_equal
         ~sexp_of_t:Types.Order_status.sexp_of_t
         Types.Order_status.Partially_filled
         s
