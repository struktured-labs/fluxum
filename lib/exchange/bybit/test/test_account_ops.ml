...
module Mock_v5 = struct
  ...
  module Withdraw = struct
    type withdraw_response = {id: string (** Withdrawal ID *)} [@@deriving sexp]
  end
end
  ...
  in
    ...
    let _ =
      assert_error
        ...
        "Unknown deposit status returns Error"
    in
      ()
  ...
  in
    ...
    let _ =
      assert_error
        ...
        "Unknown withdrawal status returns Error"
    in
      ()
...
let normalize_deposit_address ~currency (chain_info : Mock_v5.Deposit_address.chain_info)
  ...
  =
  let tag =
    match String.is_empty chain_info.tagDeposit with
    | true -> None
...
let normalize_deposit (dep : Mock_v5.Deposit_history.deposit_record)
  ...
  in
  let address =
    match String.is_empty dep.toAddress with
    | true -> None
    | false -> Some dep.toAddress
  in
  let tx_id =
    match String.is_empty dep.txID with
    | true -> None
    | false -> Some dep.txID
  in
    Ok
...
let normalize_withdrawal (wd : Mock_v5.Withdrawal_history.withdrawal_record)
  ...
  in
  let tx_id =
    match String.is_empty wd.txID with
    | true -> None
    | false -> Some wd.txID
  in
  let tag =
    match String.is_empty wd.tag with
    | true -> None
    | false -> Some wd.tag
  in
    Ok
...
let test_malformed_amount () =
  ...
  let native_dep : Mock_v5.Deposit_history.deposit_record =
    ...
    ; depositId= "bad_dep" }
  in
  let _ = assert_error (normalize_deposit native_dep) "Malformed amount returns Error" in
    ()
