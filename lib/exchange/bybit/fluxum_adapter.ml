...
module Adapter = struct
  ...
  let place_order t (req : Native.Order.request) =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      V5.Create_order.request t.cfg req
      >>| function
      | `Ok resp -> Ok resp
  ...
  let cancel_order t ~symbol (order_id : Native.Order.id) =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      V5.Cancel_order.request
        t.cfg
        {category= t.category; symbol; orderId= Some order_id; orderLinkId= None}
      >>| function
      | `Ok _ -> Ok ()
      | #Rest.Error.t as e -> Error e)
  ...
  let balances t =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      V5.Wallet_balance.request
        t.cfg
        {accountType= "UNIFIED" (* Unified trading account *); coin= None}
      >>| function
      | `Ok resp ->
  ...
  let get_open_orders t ?symbol () =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      V5.Order_realtime.request
        t.cfg
        {category= t.category; symbol; orderId= None; orderLinkId= None}
      >>| function
  ...
  let get_my_trades t ~symbol ?limit () =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      V5.Execution_list.request
        t.cfg
        {category= t.category; symbol= Some symbol; orderId= None; limit}
      >>| function
      | `Ok resp -> Ok resp.list
      | #Rest.Error.t as e -> Error e)
  ...
  let get_symbols t () =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      V5.Instruments_info.request t.cfg {category= t.category; symbol= None}
      >>| function
  ...
  let get_ticker t ~symbol () =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      V5.Market_tickers.request t.cfg {category= t.category; symbol= Some symbol}
      >>| function
      | `Ok resp ->
        (match resp.list with
         | ticker :: _ -> Ok ticker
  ...
  let get_order_book t ~symbol ?limit () =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      V5.Orderbook.request t.cfg {category= t.category; symbol; limit}
      >>| function
      | `Ok book -> Ok book
      | #Rest.Error.t as e -> Error e)
  ...
  let get_recent_trades t ~symbol ?limit () =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      V5.Recent_trade.request t.cfg {category= t.category; symbol; limit}
      >>| function
      | `Ok resp -> Ok resp.list
      | #Rest.Error.t as e -> Error e)
  ...
  let get_candles (_ : t) ~symbol:_ ~timeframe:_ ?since:_ ?until:_ ?limit:_ () =
    ...
    Deferred.return
      (Error
         (`Api_error Rest.Error.{retCode= -1; retMsg= "Bybit candles not yet implemented"}))
  ...
  let get_deposit_address t ~currency ?network () =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      let%bind result =
        V5.Deposit_address.request t.cfg {coin= currency; chainType= network}
      in
        match result with
        | `Ok resp ->
          ...
          let matching =
            match network with
            | Some n -> List.find chains ~f:(fun c -> String.equal c.chainType n)
            | None -> List.hd chains
          in
            (match matching with
             | Some chain_info ->
             ...
             | None ->
               return
                 (Error (`Bad_request "No deposit address found for this coin/chain")))
  ...
  let withdraw t ~currency ~amount ~address ?tag ?network () =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      ...
      in
      let timestamp =
        Int63.to_int64 (Int63.of_float (Core_unix.gettimeofday () *. 1000.0))
      in
          ...
          return
            (Ok
               (Native.Withdrawal.Response
                  {id= resp.id; coin= currency; amount= Float.to_string amount}))
        | #Rest.Error.t as e -> return (Error e))

  let get_deposits t ?currency ?limit () =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      let req : V5.Deposit_record.request =
        {coin= currency; startTime= None; endTime= None; limit; cursor= None}
      in
  ...
  let get_withdrawals t ?currency ?limit () =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      ...
      let%bind result = V5.Withdrawal_record.request t.cfg req in
        match result with
        | `Ok resp ->
          return (Ok (List.map resp.rows ~f:(fun w -> Native.Withdrawal.History w)))
        | #Rest.Error.t as e -> return (Error e))
  ...
  module Normalize = struct
    ...
    let order_from_status (status : Native.Order.status)
      ...
      let%bind filled =
        Fluxum.Normalize_common.Float_conv.qty_of_string status.cumExecQty
      in
        Ok
          ({ venue= Venue.t
           ; id= status.orderId
    ...
    let balance (b : Native.Balance.t) : (Types.Balance.t, string) Result.t =
      let open Result.Let_syntax in
      let%bind wallet_balance =
        Fluxum.Normalize_common.Float_conv.of_string b.walletBalance
      in
      let%bind available =
        Fluxum.Normalize_common.Float_conv.of_string b.availableToWithdraw
      in
      let%bind locked = Fluxum.Normalize_common.Float_conv.of_string b.locked in
        Ok
          ({venue= Venue.t; currency= b.coin; total= wallet_balance; available; locked}
           : Types.Balance.t)

    let book_update ~symbol (book : Native.Book.update)
      : (Types.Book_update.t, string) Result.t
      =
      let open Result.Let_syntax in
      let%bind bid_levels =
    ...
    let ticker (t : Native.Ticker.t) : (Types.Ticker.t, string) Result.t =
      ...
      in
      let%bind bid_price =
        Fluxum.Normalize_common.Float_conv.price_of_string t.bid1Price
      in
      let%bind ask_price =
        Fluxum.Normalize_common.Float_conv.price_of_string t.ask1Price
      in
      let%bind volume_24h =
        Fluxum.Normalize_common.Float_conv.qty_of_string t.volume24h
      in
      let%bind quote_volume =
        Fluxum.Normalize_common.Float_conv.of_string t.turnover24h
      in
        Ok
          ({ venue= Venue.t
           ; symbol= t.symbol
    ...
    let order_book ~symbol (book : Native.Book.snapshot)
    ...
    let public_trade (trade : Native.Public_trade.t)
               ...
               Some
                 (Time_float_unix.of_span_since_epoch
                    (Time_float_unix.Span.of_sec
                       (Int64.to_float (Int64.of_string trade.time) /. 1000.))) }
    ...
    let transfer_status_of_bybit_withdrawal_status (status : string)
      : Types.Transfer_status.t
      =
    ...
    let deposit_address (addr : Native.Deposit_address.t)
      ...
      =
      let tag =
        match String.is_empty addr.chain_info.tagDeposit with
        | true -> None
    ...
    let withdrawal (w : Native.Withdrawal.t) : (Types.Withdrawal.t, string) Result.t =
      ...
      | Native.Withdrawal.History wh ->
        ...
        let%bind fee =
          Fluxum.Normalize_common.Float_conv.amount_of_string wh.withdrawFee
        in
        ...
        in
        let tx_id =
          match String.is_empty wh.txID with
          | true -> None
          | false -> Some wh.txID
        in
        let tag =
          match String.is_empty wh.tag with
          | true -> None
          | false -> Some wh.tag
        in
          Ok
    ...
    let error (e : Native.Error.t) : Types.Error.t =
      ...
      | `Not_found ->
        Types.Error.Exchange_specific
          {venue= Venue.t; code= "not_found"; message= "Resource not found"}
      | `Unauthorized _ -> Types.Error.Auth_failed
      | `Forbidden _ -> Types.Error.Auth_failed
      | `Too_many_requests _ -> Types.Error.Rate_limited
      | `Service_unavailable msg ->
        Types.Error.Exchange_specific
          {venue= Venue.t; code= "service_unavailable"; message= msg}
      | `Json_parse_error {message; body} ->
        Types.Error.Normalization_error
          (sprintf "JSON parse error: %s (body: %s)" message body)
      | `Api_error {retCode; retMsg} ->
        Types.Error.Exchange_specific
          {venue= Venue.t; code= Int.to_string retCode; message= retMsg}
  end
