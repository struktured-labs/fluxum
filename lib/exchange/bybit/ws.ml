...
module Stream = struct
  ...
  let to_topic = function
    | Orderbook {depth; symbol} -> sprintf "orderbook.%d.%s" (depth_to_int depth) symbol
    | PublicTrade symbol -> sprintf "publicTrade.%s" symbol
    | Tickers symbol -> sprintf "tickers.%s" symbol
...
let parse_message (msg : string) : Message.t =
  try
    ...
    let success_opt =
      try Some (Yojson.Safe.Util.member "success" json |> Yojson.Safe.Util.to_bool) with
      | _ -> None
    in
    (* Check for pong response (has "ret_msg" and "op": "pong") *)
    let op_opt =
      try Some (Yojson.Safe.Util.member "op" json |> Yojson.Safe.Util.to_string) with
      | _ -> None
    in
      ...
      | _, Some "pong" ->
        (* Pong response *)
        let ret_msg =
          try Yojson.Safe.Util.member "ret_msg" json |> Yojson.Safe.Util.to_string with
          | _ -> "pong"
        in
        let conn_id =
          try Yojson.Safe.Util.member "conn_id" json |> Yojson.Safe.Util.to_string with
          | _ -> ""
        in
      ...
      | None, _ ->
        ...
        let data = Yojson.Safe.Util.member "data" json in
          ...
          else if String.is_prefix topic ~prefix:"tickers."
          then (
            match Message.ticker_data_of_yojson data with
            | Ok ticker -> Message.Tickers {topic; ts; data= ticker}
            | Error _ -> Message.Unknown msg)
          else if String.is_prefix topic ~prefix:"kline."
          then (
            match data with
            | `List klines ->
              let parsed =
                List.filter_map klines ~f:(fun k ->
