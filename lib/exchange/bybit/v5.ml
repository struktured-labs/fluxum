...
module Orderbook = struct
  module T = struct
    ...
    let request_to_params {category; symbol; limit} =
      let base = [("category", Category.to_string category); ("symbol", symbol)] in
        match limit with
...
module Wallet_balance = struct
  module T = struct
    ...
    type response = {list: account list} [@@deriving sexp, of_yojson]
  end
...
module Create_order = struct
  module T = struct
    ...
    let request_to_params
          {category; symbol; side; orderType; qty; price; timeInForce; orderLinkId}
      =
      let base =
  ...
  module T = struct
    ...
    let request_to_params {category; symbol; orderId; orderLinkId} =
      ...
      let add_opt key = function
        ...
        | Some v -> List.cons (key, v)
      in
        base |> add_opt "orderId" orderId |> add_opt "orderLinkId" orderLinkId
...
module Withdraw_create = struct
  module T = struct
    ...
    let request_to_params
          {coin; chain; address; tag; amount; timestamp; forceChain; accountType}
      =
      let base =
        [ ("coin", coin)
        ; ("chain", chain)
    ...
    type response = {id: string (** Withdrawal ID *)} [@@deriving sexp, of_yojson]
  end
...
module Withdrawal_record = struct
  module T = struct
    ...
    let request_to_params
          {withdrawID; coin; withdrawType; startTime; endTime; limit; cursor}
      =
      let add_opt key = function
        | None -> Fun.id
        | Some v -> List.cons (key, v)
