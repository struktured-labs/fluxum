...
let make_auth_request ~cfg ~method_ ~path ~body =
  ...
  let request_string =
    sprintf "%s\n%s\nrequestBody=%s&signTimestamp=%s" method_ path body timestamp
  in
  ...
  let%bind body_string =
    Deferred.Or_error.try_with (fun () -> Cohttp_async.Body.to_string body_resp)
    |> Deferred.map
         ~f:(Result.map_error ~f:(fun err -> `Network (Core.Error.to_string_hum err)))
  in
...
let ticker ~cfg ~symbol =
  let open Deferred.Result.Let_syntax in
  let%bind json_str =
    make_public_request ~cfg ~path:(sprintf "/markets/%s/ticker24h" symbol)
  in
...
let order_book ~cfg ~symbol =
  let open Deferred.Result.Let_syntax in
  let%bind json_str =
    make_public_request ~cfg ~path:(sprintf "/markets/%s/orderBook" symbol)
  in
...
let trades ~cfg ~symbol =
  let open Deferred.Result.Let_syntax in
  let%bind json_str =
    make_public_request ~cfg ~path:(sprintf "/markets/%s/trades" symbol)
  in
...
let balances ~cfg =
  let open Deferred.Result.Let_syntax in
  let%bind json_str =
    make_auth_request ~cfg ~method_:"GET" ~path:"/accounts/balances" ~body:""
  in
    match Yojson.Safe.from_string json_str with
    | `List balances ->
