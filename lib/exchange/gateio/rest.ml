...
let create_signature ~api_secret ~method_ ~url_path ~query_string ~body ~timestamp =
  ...
  let signature_string =
    String.concat ~sep:"\n" [method_; url_path; query_string; body_hash; timestamp]
  in
...
let ticker ~cfg ~currency_pair =
  ...
  let%bind json_str =
    make_public_request
      ~cfg
      ~endpoint:(sprintf "/spot/tickers?currency_pair=%s" currency_pair)
  in
...
let order_book ~cfg ~currency_pair =
  ...
  let%bind json_str =
    make_public_request
      ~cfg
      ~endpoint:(sprintf "/spot/order_book?currency_pair=%s" currency_pair)
  in
...
let trades ~cfg ~currency_pair =
  ...
  let%bind json_str =
    make_public_request
      ~cfg
      ~endpoint:(sprintf "/spot/trades?currency_pair=%s" currency_pair)
  in
    match Yojson.Safe.from_string json_str with
    | `List trades ->
...
let balances ~cfg =
  let open Deferred.Result.Let_syntax in
  let%bind json_str =
    make_auth_request ~cfg ~method_:"GET" ~endpoint:"/spot/accounts" ~params:[]
  in
    match Yojson.Safe.from_string json_str with
    | `List balances ->
...
let place_limit_order ~cfg ~currency_pair ~side ~amount ~price =
  ...
  in
  let%bind json_str =
    make_auth_request ~cfg ~method_:"POST" ~endpoint:"/spot/orders" ~params
  in
