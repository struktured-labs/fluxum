...
module T = struct
  ...
  let summary_of_orders orders =
    Map.fold
      ~f:
        (fun
          ~key:_ ~(data : Order.t) {total_original; total_executed; total_remaining} ->
        let event = data in
  ...
  let on_order_event (t : t) (event : Order_events.Order_event.t) =
    ...
    let orders =
      ...
      | false ->
        Map.update t.orders order_id ~f:(function _ -> Order.of_order_event event)
    in
  ...
  let on_order_response (t : t) (response : Status.response) =
    ...
    let orders =
      match
        ...
        || (not response.is_live)
        ||
        match
          Fluxum.Normalize_common.Float_conv.qty_of_string response.remaining_amount
        with
        | Ok qty -> Float.equal 0. qty
        | Error _ -> false
      with
      | true -> Map.remove t.orders order_id
      | false ->
        Map.update t.orders order_id ~f:(function _ -> Order.of_order_response response)
    in
    let summary = summary_of_orders t.orders in
      {orders; summary}
end
