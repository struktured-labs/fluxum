  ...
  *)
  ...
  module Change_event = struct
    ...
    module Decorated = struct
      ...
      let create ~event_id ~timestamp ({reason; side; price; remaining; delta} : T.t) =
  ...
  module Trade_event = struct
    ...
    module Decorated = struct
      ...
      let create ~event_id ~timestamp ({tid; price; amount; maker_side} : T.t) =
        ...
        | false -> failwith "logical error: event_id and trade id (tid) should be equal"
    end
    ...
    let of_yojson : Yojson.Safe.t -> (t, string) Result.t = function
      | `Assoc assoc as json ->
        ...
        |> (function
         | None ->
           Result.failf
             "no auction event type in json payload: %s"
             (Yojson.Safe.to_string json)
         | Some event_type ->
           Auction_event_type.of_yojson event_type
           |> (function
  ...
  let event_of_yojson : Yojson.Safe.t -> (event, string) Result.t = function
    | `Assoc assoc as json ->
      ...
      |> (function
       | None ->
         Result.failf "no event type in json payload: %s" (Yojson.Safe.to_string json)
       | Some event_type ->
         Event_type.of_yojson event_type
         |> (function
          | Result.Error _ as e -> e
          | Result.Ok event_type ->
            let json' = `Assoc (List.Assoc.remove ~equal:String.equal assoc "type") in
              (match event_type with
               ...
               | `Trade ->
                 Trade_event.of_yojson json' |> Result.map ~f:(fun event -> `Trade event)
               | `Auction ->
  ...
  let response_of_yojson : Yojson.Safe.t -> (response, string) Result.t = function
    | `Assoc assoc as json ->
      ...
      |> (function
       | None, _ ->
         Result.failf
           "no sequence number in json payload: %s"
           (Yojson.Safe.to_string json)
       | _, None ->
         Result.failf "no message type in json payload: %s" (Yojson.Safe.to_string json)
