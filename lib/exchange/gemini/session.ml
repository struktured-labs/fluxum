...
let status_pipe cfg nonce order_id : status_pipe =
  let request = Order.Status.{order_id} in
    Inf_pipe.unfold ~init:0 ~f:(fun epoch ->
      Order.Status.post cfg nonce request >>| fun response -> (response, epoch + 1))
...
let balance_pipe cfg nonce =
  let request = () in
    Inf_pipe.unfold ~init:0 ~f:(fun epoch ->
      Balances.post cfg nonce request
      >>= fun response ->
      after (Time_float_unix.Span.of_sec 1.0) >>| fun () -> (response, epoch + 1))
...
module Events = struct
  ...
  type t =
    ...
    ; trades: [`Ok of Mytrades.response | Rest.Error.post] Inf_pipe.Reader.t Symbol.Map.t
    ; order_status:
    ...
    ; order_books:
        [`Ok of Order_book.Book.t | Market_data.Error.t] Pipe.Reader.t Symbol.Map.t
    ; symbol_details: Symbol_details.response Symbol.Map.t }

  let create ?(symbols : Symbol.t list = []) ?(order_ids : Order_id.t list = []) cfg nonce
    : t Deferred.t
    =
    let symbols_len = List.length symbols in
    let order_ids_len = List.length order_ids in
      Log.Global.info
        "Events.create: start symbols=%d order_ids=%d"
        symbols_len
        order_ids_len;
      let balance =
      ...
      let trades =
        ...
        List.map symbols ~f:(fun symbol ->
          Log.Global.debug "Events.create: trade_pipe %s" (Symbol.to_string symbol);
      ...
      let order_status =
        ...
        |> Order_map.of_alist_exn
        |> Concurrent_order_map.of_map
      in
        Log.Global.debug "Events.create: wiring market_data for %d symbols" symbols_len;
        let market_data =
          Symbol.Map.of_alist_exn
            (List.map symbols ~f:(fun symbol ->
               let market_data_name =
                 sprintf "market_data[%s]" (Symbol.to_string symbol)
               in
               let market_data_pipe =
                 auto_restart_pipe
                   ~name:market_data_name
                   ~create_pipe:(fun () ->
                     Log.Global.info
                       "auto_restart[%s]: connecting Market_data.client"
                       market_data_name;
                     Market_data.client cfg ?query:None ~uri_args:symbol ()
                     >>| fun pipe ->
                     let event_pipe =
              ...
              (List.map symbols ~f:(fun symbol ->
                 ...
                 let book_pipe =
                   auto_restart_pipe
                     ~name:book_name
                     ~create_pipe:(fun () ->
                       Order_book.Books.pipe cfg ~symbols:[symbol] ()
                       >>| fun books_map -> Map.find_exn books_map symbol)
                     ()
                 in
                   (symbol, book_pipe)))
          in
              ...
              let init =
                Symbol.Map.of_alist_exn
                  (List.map symbols ~f:(fun symbol ->
                     ( symbol
                     , Ledger.Entry.create
                         ~symbol:(Symbol.Enum_or_string.of_enum symbol)
                         () )))
              in
                        ...
                        (Pipe.map
                           ~f:
                             (Poly_ok.ok_exn
                                ?sexp_of_error:None
                                ~here:[%here]
                                ~message:"Order books error"))
                      order_books_exn
                  in
                  let order_events_exn =
                    Pipe.map
                      ~f:
                        (Poly_ok.ok_exn
                           ?sexp_of_error:None
                           ~here:[%here]
                           ~message:"Order events error")
                      order_events_exn
                  in
                    ...
                    >>= fun ledger ->
                    Log.Global.info
                      "Events.create: finished Ledger.pipe, fetching symbol details";
                    Deferred.List.map symbols ~how:`Parallel ~f:(fun symbol ->
                      Symbol_details.get cfg nonce ~uri_args:symbol ()
                      >>| function
                      | `Ok details -> Some (symbol, details)
                      | #Rest.Error.get as error ->
                        Log.Global.error
                          "Failed to fetch symbol details for %s: %s"
                          (Symbol.to_string symbol)
                          (Rest.Error.sexp_of_get error |> Sexp.to_string);
                        None)
                    >>| fun details_list ->
                    let symbol_details =
                    ...
                    in
                      Log.Global.info
                        "Events.create: fetched %d symbol details, assembling record"
...
module Make (C : Cfg.S) = struct
  ...
  let symbol_details t symbol = Map.find t.events.symbol_details symbol
  ...
  let min_order_size t symbol =
    let details = symbol_details t symbol in
      Option.bind details ~f:(fun d ->
        ...
        | Error err ->
          Log.Global.error
            "Gemini session: Failed to parse min_order_size for %s: %s"
            (Symbol.to_string symbol)
            err;
          None)
  ...
  let format_price t symbol price : Common.Decimal_string.t =
    ...
    | None ->
      Log.Global.error
        "No symbol details for %s, using default precision"
        (Symbol.to_string symbol);
    ...
    | Some details ->
      ...
      let formatted_price = sprintf "%.*f" decimal_places price in
        Log.Global.debug
          "Formatting price %.9f for %s with %d decimal places as %s"
          price
          (Symbol.to_string symbol)
          decimal_places
          formatted_price;
  ...
  let format_quantity ?(side = `Buy) t symbol quantity =
    ...
    | Some details ->
      ...
      let formatted =
        match side with
        | `Buy -> sprintf "%.*f" decimal_places quantity
        | `Sell ->
      ...
      in
        Log.Global.debug
          "Formatting quantity %.9f for %s with %d decimal places as %s"
          quantity
          (Symbol.to_string symbol)
          decimal_places
          formatted;
  ...
  let to_state_csv (t : t) : State_csv.t =
    ...
    ; timestamp_iso=
        Time_float_unix.to_string_iso8601_basic
          (Time_float_unix.now ())
  ...
  let get_writer path header : (Writer.t * bool ref) Deferred.t =
    ...
    | None ->
      ...
      >>= fun exists ->
      Writer.open_file ~append:true path
      >>= fun w ->
  ...
  let write_state_csv ?(path = ".gemini_session_state.csv") (t : t) : unit Deferred.t =
    let csv_row = to_state_csv t in
      get_writer path (String.concat ~sep:"," State_csv.csv_header)
      >>= fun (w, _) ->
  ...
  let create
        ...
        ?order_ids
        ~api_nonce
        ?client_order_id_nonce
        ?state_csv_path
        ()
    : t Deferred.t
    =
    let session_id = Option.value session_id ~default:(generate_session_id ()) in
      ...
      Events.create ?symbols ?order_ids (module C) api_nonce
      >>= fun events ->
      Log.Global.info "Session.create: Events.create finished";
      (* Fork the balance pipe for caching and external use *)
      let balance_reader_1, balance_reader_2 =
        Inf_pipe.fork ~pushback_uses:`Fast_consumer_only events.balance
      in
      let t =
        { name
        ; session_id
        ; events= {events with balance= balance_reader_1}
        ; api_nonce
        ; client_order_id_nonce
        ; balances= None }
      in
        ...
        don't_wait_for
          (let rec loop () =
             Inf_pipe.read balance_reader_2
             >>= function
             | `Ok balances ->
               Log.Global.info
                 "Session.create[%s]: balance update %s"
                 session_id
        ...
        in
          write_state_csv ~path:state_csv_path t
          >>| fun () ->
  ...
  module New_order_request = struct
    ...
    let to_api ~client_order_id (t : t) : Order.New.request =
      ...
      let api =
        Order.New.{client_order_id; symbol; amount; price; side; type_; options}
      in
        api
  end
  ...
  let submit_order ?(autoformat = `All) t (req : New_order_request.t)
    ...
    =
    Log.Global.info
      "submit_order[%s]: requested %s"
      t.session_id
      (New_order_request.sexp_of_t req |> Sexp.to_string_hum);
    let req =
      match autoformat with
      | `All ->
        (match
           let open Result.Let_syntax in
           let%bind price =
             Fluxum.Normalize_common.Float_conv.price_of_string req.price
           in
           let%bind amount =
             Fluxum.Normalize_common.Float_conv.qty_of_string req.amount
           in
             Ok (price, amount)
         with
         | Ok (price, amount) ->
    ...
    in
      Log.Global.info
        "submit_order[%s]: formatted %s"
        t.session_id
        (New_order_request.sexp_of_t req |> Sexp.to_string_hum);
      Inf_pipe.read t.client_order_id_nonce
      >>= fun client_order_id ->
      Log.Global.info "submit_order[%s]: client_order_id=%s" t.session_id client_order_id;
      let req = New_order_request.to_api ~client_order_id req in
        Order.New.post (cfg t) t.api_nonce req
        >>| function
        | `Ok response ->
          let order_id = response.order_id in
            Log.Global.info
              "submit_order[%s]: accepted order_id=%Ld"
              t.session_id
              order_id;
  ...
  let cancel_order t req =
    Log.Global.info
      "cancel_order[%s]: order_id=%Ld"
      t.session_id
      req.Order.Cancel.By_order_id.order_id;
    Order.Cancel.By_order_id.post (cfg t) t.api_nonce req
    >>| function
    | `Ok response ->
      Log.Global.info "cancel_order[%s]: ok order_id=%Ld" t.session_id response.order_id;
      Concurrent_order_map.remove t.events.order_status response.order_id;
      `Ok response
    | #Error.post as e ->
      Log.Global.error
        "cancel_order[%s]: error %s"
        t.session_id
        (Error.sexp_of_post e |> Sexp.to_string_hum);
      e
  ...
  let cancel_all (t : t) =
    ...
    Order.Cancel.All.post (cfg t) t.api_nonce ()
    >>| function
    | `Ok response ->
