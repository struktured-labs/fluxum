...
module Adapter = struct
  ...
  let get_order_history (_ : t) ?symbol:_ ?limit:_ () =
    ...
    Deferred.return
      (Error (`Bad_request "Gemini does not support order history endpoint"))
    ...
    let (module Cfg) = t.cfg in
      Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
        V1.Mytrades.post
          (module Cfg)
          t.nonce
          {symbol= V1.Symbol.of_string symbol; limit_trades= limit; timestamp= None}
        >>| function
  ...
  let get_ticker (t : t) ~symbol () =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      ...
      let uri =
        Uri.of_string (sprintf "https://api.gemini.com/v1/pubticker/%s" symbol_lower)
      in
  ...
  let get_order_book (t : t) ~symbol ?limit () =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      ...
      let limit_str =
        match limit with
        | Some n -> sprintf "?limit_bids=%d&limit_asks=%d" n n
        | None -> ""
      in
      let uri =
        Uri.of_string
          (sprintf "https://api.gemini.com/v1/book/%s%s" symbol_lower limit_str)
      in
        Monitor.try_with (fun () ->
          Cohttp_async.Client.get uri
          >>= fun (_, body) -> Cohttp_async.Body.to_string body)
  ...
  let get_recent_trades (t : t) ~symbol ?limit () =
    Exchange_common.Rate_limiter.with_rate_limit_retry t.rate_limiter ~f:(fun () ->
      ...
      let limit_str =
        match limit with
        | Some n -> sprintf "?limit_trades=%d" n
        | None -> ""
      in
      let uri =
        Uri.of_string
          (sprintf "https://api.gemini.com/v1/trades/%s%s" symbol_lower limit_str)
      in
        Monitor.try_with (fun () ->
          Cohttp_async.Client.get uri
          >>= fun (_, body) -> Cohttp_async.Body.to_string body)
  ...
  let get_candles (_ : t) ~symbol:_ ~timeframe:_ ?since:_ ?until:_ ?limit:_ () =
    ...
    Deferred.return
      (Error (`Bad_request "Gemini does not support REST candle/klines endpoint"))
  ...
  module Streams = struct
    let trades (t : t) =
      let (module Cfg) = t.cfg in
        ...
        let query = [`Event_type_filter `Fill] |> List.map ~f:sexp_of_query in
          client (module Cfg) ~nonce:t.nonce ~query ()
          >>| Pipe.filter_map ~f:(function
            | `Ok (`Order_event ev) -> Some (Native.Trade.Ws ev)
            | `Ok (`Order_events evs) ->
              Option.map (List.hd evs) ~f:(fun ev -> Native.Trade.Ws ev)
            | `Ok (`Heartbeat _) -> None
  ...
  module Normalize = struct
    ...
    let order_kind
          (type_ : Common.Order_type.t)
          (price : Common.Decimal_string.t)
          (options : Common.Order_execution_option.t list)
      : (Types.Order_kind.t, string) Result.t
      =
      match type_ with
      | `Exchange_limit ->
        ...
        let%bind p =
          Fluxum.Normalize_common.Float_conv.price_of_string
            (Common.Decimal_string.to_string price)
        in
          Ok
            (match
               List.exists options ~f:(function
    ...
    let order_response (r : Native.Order.response) : (Types.Order.t, string) Result.t =
      let open Result.Let_syntax in
      let%bind qty_orig =
        Fluxum.Normalize_common.Float_conv.qty_of_string
          (Common.Decimal_string.to_string r.original_amount)
      in
      let%bind qty_exec =
        Fluxum.Normalize_common.Float_conv.qty_of_string
          (Common.Decimal_string.to_string r.executed_amount)
      in
      let%bind qty_rem =
        Fluxum.Normalize_common.Float_conv.qty_of_string
          (Common.Decimal_string.to_string r.remaining_amount)
      in
      ...
      let reason_to_status = function
        ...
        | Some `Immediate_or_cancel_would_post ->
          Types.Order_status.Rejected "ioc_would_post"
    ...
    let trade (t : Native.Trade.t) : (Types.Trade.t, string) Result.t =
      ...
      | Ws ev ->
        (match ev.fill with
         | None ->
           ...
           let price_result =
             Option.value_map ev.avg_execution_price ~default:(Ok 0.0) ~f:(fun d ->
               Fluxum.Normalize_common.Float_conv.price_of_string
                 (Common.Decimal_string.to_string d))
           in
           let qty_result =
             Option.value_map ev.executed_amount ~default:(Ok 0.0) ~f:(fun d ->
               Fluxum.Normalize_common.Float_conv.qty_of_string
                 (Common.Decimal_string.to_string d))
           in
         ...
         | Some f ->
           let%bind price =
             Fluxum.Normalize_common.Float_conv.price_of_string
               (Common.Decimal_string.to_string f.price)
           in
           let%bind qty =
             Fluxum.Normalize_common.Float_conv.qty_of_string
               (Common.Decimal_string.to_string f.amount)
           in
           let%bind fee =
             Fluxum.Normalize_common.Float_conv.qty_of_string
               (Common.Decimal_string.to_string f.fee)
           in
             Ok
               ({ venue= Venue.t
                ; symbol= Common.Symbol.Enum_or_string.to_string ev.symbol
      ...
      | Rest tr ->
        let%bind price =
          Fluxum.Normalize_common.Float_conv.price_of_string
            (Common.Decimal_string.to_string tr.price)
        in
        let%bind qty =
          Fluxum.Normalize_common.Float_conv.qty_of_string
            (Common.Decimal_string.to_string tr.amount)
        in
        let%bind fee =
          Fluxum.Normalize_common.Float_conv.qty_of_string
            (Common.Decimal_string.to_string tr.fee_amount)
        in
          Ok
            ({ venue= Venue.t
    ...
    let balance (b : Native.Balance.t) : (Types.Balance.t, string) Result.t =
      ...
      let%bind total =
        Fluxum.Normalize_common.Float_conv.qty_of_string
          (Common.Decimal_string.to_string b.amount)
      in
      let%bind available =
        Fluxum.Normalize_common.Float_conv.qty_of_string
          (Common.Decimal_string.to_string b.available)
      in
        Ok
          ({ venue= Venue.t
      ...
      let levels_of_event = function
        | `Change (c : V1.Market_data.Change_event.t) ->
          (match
             ...
             let%bind price =
               Fluxum.Normalize_common.Float_conv.price_of_string
                 (Common.Decimal_string.to_string c.price)
             in
             let%bind qty =
               Fluxum.Normalize_common.Float_conv.qty_of_string
                 (Common.Decimal_string.to_string c.remaining)
             in
               Ok (price, qty)
           with
           | Ok (price, qty) ->
    ...
    let symbol_info (s : Native.Symbol_info.t) : (Types.Symbol_info.t, string) Result.t =
      ...
      let%bind min_order_size =
        Fluxum.Normalize_common.Float_conv.qty_of_string
          (Common.Decimal_string.to_string s.min_order_size)
      in
        Ok
          ({ venue= Venue.t
    ...
    let ticker (json : Native.Ticker.t) : (Types.Ticker.t, string) Result.t =
      try
        ...
        let%bind symbol_vol =
          match volume |> to_assoc |> List.hd with
          | Some (symbol, vol) ->
            let vol_str = vol |> to_string in
            let%bind vol_float =
              Fluxum.Normalize_common.Float_conv.qty_of_string vol_str
            in
              Ok (symbol, vol_float)
          | None -> Ok ("", 0.)
        in
    ...
    let public_trade (json : Native.Public_trade.t)
      ...
      try
        ...
        let ts =
          Time_float_unix.of_span_since_epoch
            (Time_float_unix.Span.of_ms (Float.of_int timestamp))
        in
    ...
    let prediction_contract (c : Prediction_markets.Contract.t)
      ...
      let open Result.Let_syntax in
      let last_price =
        Option.bind c.prices ~f:(fun p ->
          Option.bind p.last_trade_price ~f:Float.of_string_opt)
      in
      let best_bid =
        Option.bind c.prices ~f:(fun p -> Option.bind p.best_bid ~f:Float.of_string_opt)
      in
      let best_ask =
        Option.bind c.prices ~f:(fun p -> Option.bind p.best_ask ~f:Float.of_string_opt)
      in
      let%bind total_shares =
        Fluxum.Normalize_common.Float_conv.qty_of_string c.total_shares
      in
        Ok
    ...
    let prediction_event (e : Prediction_markets.Event.t)
      ...
      let%bind contracts = List.map e.contracts ~f:prediction_contract |> Result.all in
        Ok
          ({ venue= Venue.t
           ; id= e.id
           ; title= e.title
    ...
    let prediction_order (r : Prediction_markets.Place_order.response)
      ...
      let%bind filled =
        Fluxum.Normalize_common.Float_conv.qty_of_string r.filled_quantity
      in
      let%bind remaining =
        Fluxum.Normalize_common.Float_conv.qty_of_string r.remaining_quantity
      in
      let%bind price = Fluxum.Normalize_common.Float_conv.of_string r.price in
      let%bind avg_execution_price =
        match r.avg_execution_price with
        | None -> Ok None
        | Some s ->
    ...
    let error (e : Native.Error.t) : Types.Error.t =
      ...
      | `Bad_gateway msg ->
        Types.Error.Exchange_specific {venue= Venue.t; code= "502"; message= msg}
      | `Gateway_timeout msg ->
        Types.Error.Exchange_specific {venue= Venue.t; code= "504"; message= msg}
      | `Json_parse_error {message; body= _} ->
        Types.Error.Exchange_specific {venue= Venue.t; code= "json"; message}
      | `Error {reason; message} ->
        Types.Error.Exchange_specific {venue= Venue.t; code= reason; message}

    (** Account operations normalization *)

    let deposit_address (r : Native.Deposit_address.t)
    ...
    let deposit (tr : Native.Deposit.t) : (Types.Deposit.t, string) Result.t =
      ...
      in
      let status =
        match String.lowercase tr.status with
    ...
    let withdrawal (w : Native.Withdrawal.t) : (Types.Withdrawal.t, string) Result.t =
      ...
      | Native.Withdrawal.Withdraw_response r ->
        let%bind amount =
          Fluxum.Normalize_common.Float_conv.amount_of_string
            (Common.Decimal_string.to_string r.amount)
        in
          Ok
            ({ venue= Venue.t
             ; id= r.withdrawalId
      ...
      | Native.Withdrawal.Transfer tr ->
        let%bind amount =
          Fluxum.Normalize_common.Float_conv.amount_of_string
            (Common.Decimal_string.to_string tr.V1.Transfers.amount)
        in
        let%bind fee =
          match tr.V1.Transfers.feeAmount with
          | Some fa ->
            Fluxum.Normalize_common.Float_conv.amount_of_string
              (Common.Decimal_string.to_string fa)
            |> Result.map ~f:Option.some
          | None -> Ok None
        in
        let status =
          match String.lowercase tr.V1.Transfers.status with
          | "complete" | "completed" -> Types.Transfer_status.Completed
          | "pending" -> Types.Transfer_status.Pending
          | "processing" | "advanced" -> Types.Transfer_status.Processing
          | "failed" -> Types.Transfer_status.Failed
      ...
      | `Ok orders ->
        let normalized =
          List.filter_map orders ~f:(fun o ->
            ...
            | Error err ->
              Log.Global.error
                "prediction order normalize error (id=%Ld): %s"
                o.order_id
                err;
              None)
        in
      ...
      | `Ok positions ->
        let normalized =
          List.filter_map positions ~f:(fun p ->
            ...
            | Error err ->
              Log.Global.error
                "prediction position normalize error (symbol=%s): %s"
                p.symbol
                err;
              None)
        in
...
module Builder = struct
  ...
  let make_order_request ~symbol ~side ~kind ~qty =
    ...
    let price, options =
      ...
      | Types.Order_kind.Conditional _ -> (price_str_of 0.0, [])
      (* Gemini doesn't support stop orders via this endpoint *)
    in
      V1.Order.New.
        { client_order_id=
            Client_order_id.of_string
              (sprintf
                 "fluxum-%0.0f"
                 (Time_float_unix.now ()
