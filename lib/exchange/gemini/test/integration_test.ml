...
let get_json path =
  ...
  let uri = Uri.of_string (sprintf "https://api.gemini.com%s" path) in
    Monitor.try_with (fun () ->
      Cohttp_async.Client.get uri >>= fun (_, body) -> Cohttp_async.Body.to_string body)
    >>| function
...
let test_pubticker () =
  ...
  | Ok json ->
    ...
    (try
     ...
     with
     | _ -> fail "Failed to parse ticker response")
  | Error e -> fail (sprintf "Error: %s" e)

let test_book () =
  printf "\n[REST] Order Book (ethusd)\n";
  get_json "/v1/book/ethusd?limit_bids=5&limit_asks=5"
  >>| function
  | Ok json ->
    (try
       match json with
       | `Assoc fields ->
         let bids =
           match List.Assoc.find fields ~equal:String.equal "bids" with
           | Some (`List l) -> l
           | _ -> []
         in
         let asks =
           match List.Assoc.find fields ~equal:String.equal "asks" with
           | Some (`List l) -> l
           | _ -> []
         in
         ...
         let ask_count = List.length asks in
           ...
           (match List.hd bids with
            | Some (`Assoc bid_fields) ->
              let price =
                match List.Assoc.find bid_fields ~equal:String.equal "price" with
                | Some (`String s) -> s
                | _ -> "?"
              in
              let amount =
                match List.Assoc.find bid_fields ~equal:String.equal "amount" with
                | Some (`String s) -> s
                | _ -> "?"
              in
           ...
           (match List.hd asks with
            | Some (`Assoc ask_fields) ->
              let price =
                match List.Assoc.find ask_fields ~equal:String.equal "price" with
                | Some (`String s) -> s
                | _ -> "?"
              in
              let amount =
                match List.Assoc.find ask_fields ~equal:String.equal "amount" with
                | Some (`String s) -> s
                | _ -> "?"
              in
     ...
     with
     | e -> fail (sprintf "Failed to parse book response: %s" (Exn.to_string e)))
  | Error e -> fail (sprintf "Error: %s" e)

let test_trades () =
  printf "\n[REST] Recent Trades (btcusd)\n";
  get_json "/v1/trades/btcusd?limit_trades=5"
  >>| function
  | Ok json ->
    let trades =
      match json with
         ...
         (match List.hd trades with
          | Some trade ->
            ...
            (try
             ...
             with
             | e -> fail (sprintf "Failed to parse trade: %s" (Exn.to_string e)))
          | None -> ())
...
let test_symbol_details () =
  ...
  get_json "/v1/symbols/details/btcusd"
  >>| function
  | Ok json ->
    let open Yojson.Safe.Util in
    (try
     ...
     with
     | _ -> fail "Failed to parse symbol details")
  | Error e -> fail (sprintf "Error: %s" e)
