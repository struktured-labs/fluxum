...
module Contract = struct
  type t =
    ...
    ; terms_and_conditions_url: string option
          [@default None] [@key "termsAndConditionsUrl"]
  ...
  let of_yojson (json : Yojson.Safe.t) =
    ...
    let json' =
      match json with
      | `Assoc assoc -> `Assoc (List.Assoc.remove assoc ~equal:String.equal "description")
      | other -> other
    in
      match json' with
      | `Assoc assoc ->
        ...
        in
          Result.Ok
            ...
            ; prices=
                (match find "prices" with
                 | Some p ->
                   (match Contract_prices.of_yojson p with
                    | Ok v -> Some v
                    | Error e ->
...
module Event = struct
  ...
  let of_yojson (json : Yojson.Safe.t) =
    ...
    | `Assoc assoc ->
      ...
      in
      let is_live =
        match find "isLive" with
        | Some (`Bool b) -> b
        | _ -> false
      in
      let volume =
        match find "volume" with
        | Some (`String s) -> s
...
let http_get uri =
  ...
  | `Bad_gateway -> Cohttp_async.Body.to_string body >>| fun b -> `Bad_gateway b
  | `Gateway_timeout -> Cohttp_async.Body.to_string body >>| fun b -> `Gateway_timeout b
  ...
  let get (module Cfg : Cfg.S) ?status ?category ?search ?limit ?offset () =
    let query =
      List.concat
        [ (match status with
           | None -> []
           | Some statuses -> [("status[]", List.map statuses ~f:Event_status.to_string)])
        ; (match category with
           | None -> []
  ...
  let command =
    ...
    , Command.async
        ...
        [%map_open
          ...
          and search =
            flag "-search" (optional string) ~doc:"STRING search events by text"
          and limit = flag "-limit" (optional int) ~doc:"INT max events to return"
          and offset = flag "-offset" (optional int) ~doc:"INT pagination offset" in
            fun () ->
              let config = public_config_of_env env in
              let status =
                Option.map status ~f:(fun s ->
                  String.split s ~on:',' |> List.filter_map ~f:Event_status.of_string_opt)
              in
              let category = Option.map category ~f:(fun s -> String.split s ~on:',') in
                  ...
                  List.iter events ~f:(fun e ->
                    ...
                    List.iter e.contracts ~f:(fun c ->
                      let price_str =
                        match c.prices with
                        | Some p ->
                          (match p.last_trade_price with
                           | Some ltp -> ltp
                           | None ->
                             (match p.best_ask with
                              | Some ba -> ba
                              | None -> "?"))
                        | None -> "?"
                      in
                ...
                | #Rest.Error.get as err ->
                  failwiths ~here:[%here] "list events failed" err Rest.Error.sexp_of_get]
    )
end
...
module Get_event = struct
  ...
  let get (module Cfg : Cfg.S) ~event_ticker () =
    let path = String.concat ~sep:"/" (base_path @ ["events"; event_ticker]) in
  ...
  let command =
    ...
    , Command.async
        ...
        [%map_open
          let env = cfg_param_public
          and event_ticker =
            flag "-ticker" (required string) ~doc:"STRING event ticker (e.g., BTC100K)"
          in
            fun () ->
              let config = public_config_of_env env in
                get config ~event_ticker ()
                >>= function
                | `Ok event ->
                  printf "%s\n" (Sexp.to_string_hum (Event.sexp_of_t event));
                  Deferred.unit
                | #Rest.Error.get as err ->
                  failwiths ~here:[%here] "get event failed" err Rest.Error.sexp_of_get]
    )
end
...
module List_categories = struct
  ...
  let get (module Cfg : Cfg.S) () =
    let path = String.concat ~sep:"/" (base_path @ ["categories"]) in
    let uri = Uri.make ~scheme:"https" ~host:Cfg.api_host ~path () in
      http_get uri
      >>| function
      | `Ok json -> parse_json_response json response_of_yojson
  ...
  module T = struct
    ...
    type response =
      ...
      ; avg_execution_price: Decimal_string.t option
            [@default None] [@key "avgExecutionPrice"]
      ; created_at: string option [@default None] [@key "createdAt"]
  ...
  let command =
    ...
    , Command.async
        ...
        [%map_open
          ...
          and side = flag "-side" (required string) ~doc:"STRING order side (buy or sell)"
          and quantity =
            flag "-quantity" (required string) ~doc:"STRING number of contracts"
          and price = flag "-price" (required string) ~doc:"STRING price (0.00 to 1.00)"
          and outcome =
            flag "-outcome" (required string) ~doc:"STRING outcome (yes or no)"
          and time_in_force =
            flag
              "-time-in-force"
              (optional_with_default "good-til-cancel" string)
              ~doc:
                "STRING time in force (good-til-cancel, immediate-or-cancel, \
                 fill-or-kill)"
          in
            fun () ->
              let config = Cfg.or_default config in
              let side =
                match Side.of_string_opt (String.lowercase side) with
                | Some s -> s
                | None ->
                  failwiths
                    ~here:[%here]
              ...
              in
              let outcome =
                match Outcome.of_string_opt (String.lowercase outcome) with
                | Some o -> o
                | None ->
                  failwiths
                    ~here:[%here]
                    "invalid outcome (expected: yes, no)"
                    outcome
                    String.sexp_of_t
              in
              let request =
              ...
              in
                ...
                >>= fun nonce ->
                post config nonce request
                >>= function
                | `Ok response ->
                  printf
                    "Order %Ld placed: %s %s %s @ %s (%s)\n"
                    response.order_id
                ...
                | #Rest.Error.post as err ->
                  failwiths ~here:[%here] "place order failed" err Rest.Error.sexp_of_post]
    )
end
...
module Cancel_order = struct
  module T = struct
    ...
    type request = {order_id: Int_number.t [@key "orderId"]} [@@deriving sexp, to_yojson]
  ...
  let command =
    ...
    , Command.async
        ...
        [%map_open
          let config = Cfg.param
          and order_id = flag "-order-id" (required int) ~doc:"INT order ID to cancel" in
            fun () ->
              let config = Cfg.or_default config in
...
module Active_orders = struct
  ...
  let get ?limit ?offset ?symbol cfg nonce () = post cfg nonce {limit; offset; symbol}
  ...
  let command =
    ...
    , Command.async
        ...
        [%map_open
          ...
          and offset = flag "-offset" (optional int) ~doc:"INT pagination offset"
          and symbol =
            flag "-symbol" (optional string) ~doc:"STRING filter by instrument symbol"
          in
            fun () ->
              let config = Cfg.or_default config in
                Nonce.File.(pipe ~init:default_filename) ()
                >>= fun nonce ->
                get ?limit ?offset ?symbol config nonce ()
                >>= function
                | `Ok orders ->
                  List.iter orders ~f:(fun o ->
                    printf
                      "%Ld  %s %s %s @ %s  qty=%s filled=%s  [%s]\n"
                      o.order_id
...
module Book_snapshot = struct
  ...
  let get (module Cfg : Cfg.S) ~instrument_symbol ?limit () =
    ...
    let query =
      match limit with
      | Some n -> [("limit_bids", [Int.to_string n]); ("limit_asks", [Int.to_string n])]
      | None -> []
    in
    let uri = Uri.make ~scheme:"https" ~host:Cfg.api_host ~path ~query () in
      http_get uri
        ...
        [%map_open
          ...
          and symbol =
            flag
              ...
              ~doc:"STRING instrument symbol (e.g., GEMI-BTC100K-YES)"
          and limit =
            flag "-limit" (optional int) ~doc:"INT max bid/ask levels to return"
          in
            fun () ->
              let config = public_config_of_env env in
                ...
                | `Ok snap ->
                  ...
                  List.rev snap.asks
                  |> List.iter ~f:(fun l -> printf "  %s  qty=%s\n" l.price l.amount);
    ...
    , Command.async
        ...
        [%map_open
          ...
          in
            fun () ->
              let config = Cfg.or_default config in
                Order_book.Book.pipe_for_instrument config ~instrument_symbol:symbol ()
                >>= Pipe.iter ~f:(function
                  | `Ok book ->
                    let module B = Order_book.Book in
                    let module PL = Order_book.Price_level in
                    printf
                      "=== %s Prediction Order Book (Epoch: %d) ===\n"
                      (B.symbol book)
                      (B.epoch book);
