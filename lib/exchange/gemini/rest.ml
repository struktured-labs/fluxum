...
module Response = struct
  module Json_result = struct
    ...
    let split = function
      | `Assoc assoc as json ->
        List.Assoc.find assoc ~equal:String.equal "result"
        |> (function
         | None -> Result.Ok (None, json)
         | Some json' ->
           of_yojson json'
           |> Result.map ~f:(fun x ->
             (Some x, `Assoc (List.Assoc.remove assoc ~equal:String.equal "result"))))
  ...
  let parse json ok_of_yojson =
    ...
    | Result.Ok (result, payload) ->
      (match result with
       | None | Some `Ok ->
         ok_of_yojson payload
         |> (function
          | Result.Ok x -> `Ok x
          | Result.Error e ->
            `Json_parse_error Error.{message= e; body= Yojson.Safe.to_string payload})
       | Some `Error ->
         Error.detail_of_yojson payload
         |> (function
          | Result.Ok x -> `Error x
          | Result.Error e ->
            `Json_parse_error Error.{message= e; body= Yojson.Safe.to_string payload}))
