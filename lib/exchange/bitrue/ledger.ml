...
module Entry = struct
  ...
  let empty ~symbol () = create ~symbol ()
  ...
  let recalculate_pnl t ~current_price =
    let pnl =
      match Float.(t.position <> 0.0 && t.avg_price <> 0.0) with
...
module Ledger = struct
  ...
  let create () = {entries= String.Map.empty; state= Disconnected}
  let get_entry t ~symbol = Map.find t.entries symbol
