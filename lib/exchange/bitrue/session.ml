             ...
             Auto_restart.pipe
               ...
               ~create_pipe:(fun () ->
                 ...
                 let streams = [Ws.Stream.Ticker symbol] in
                   Market_data_curl.connect ~streams ()
                   >>| function
           ...
           let pipe =
             Auto_restart.pipe
               ...
               ~create_pipe:(fun () ->
                 ...
                 | Error err ->
                   Log.Global.error
                     "Bitrue order book failed for %s: %s"
                     symbol
           ...
           let entry_ref = ref init in
                    ...
                    | Ok book ->
                      ...
                      let spot = best.price in
                        (match Float.(spot > 0.) with
                         | true ->
                           entry_ref
                           := Ledger.Entry.recalculate_pnl !entry_ref ~current_price:spot;
             ...
             don't_wait_for
               ((* Update from trades *)
                Pipe.iter
                  trade_pipe
                  ~f:(fun (trade : Ws.Message.trade) ->
                    match
                      Fluxum.Normalize_common.Float_conv.price_of_string trade.price
                    with
                    | Ok price ->
                      let side =
                        match String.equal trade.ds "buy" with
                        | true -> Fluxum.Types.Side.Buy
                        | false -> Fluxum.Types.Side.Sell
                      in
                      let qty =
                        match
                          Fluxum.Normalize_common.Float_conv.qty_of_string trade.vol
                        with
                        | Ok v -> v
                        | Error _ -> 0.0
                      in
