...
type metadata = {last_update_id: int64} [@@deriving sexp]
...
module Book = struct
  ...
  let empty ?timestamp:_ ?last_update_id:_ symbol = create ~symbol
  ...
  let apply_depth_update t (depth : Ws.Message.depth) =
    ...
    let bid_levels =
      List.filter_map depth.tick.buys ~f:(fun (price_str, qty_str) ->
    ...
    in
    let ask_levels =
      List.filter_map depth.tick.asks ~f:(fun (price_str, qty_str) ->
        let open Result.Let_syntax in
        match
          let%bind price = Fluxum.Normalize_common.Float_conv.price_of_string price_str in
          let%bind volume = Fluxum.Normalize_common.Float_conv.qty_of_string qty_str in
  ...
  let apply_rest_depth t (book : Rest.Types.order_book) =
    ...
    let bid_levels =
      List.filter_map book.bids ~f:(fun (price_str, qty_str) ->
        let open Result.Let_syntax in
        match
          let%bind price = Fluxum.Normalize_common.Float_conv.price_of_string price_str in
          let%bind volume = Fluxum.Normalize_common.Float_conv.qty_of_string qty_str in
    ...
    in
    let ask_levels =
      List.filter_map book.asks ~f:(fun (price_str, qty_str) ->
        let open Result.Let_syntax in
        match
          let%bind price = Fluxum.Normalize_common.Float_conv.price_of_string price_str in
          let%bind volume = Fluxum.Normalize_common.Float_conv.qty_of_string qty_str in
