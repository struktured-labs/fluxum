...
let connect ~(streams : Ws.Stream.t list) ?(url = Ws.Endpoint.market) ()
                     ...
                     (match msg_opt with
                      ...
                      | Some payload ->
                        ...
                        (match !receive_count mod 50 = 0 with
                         | true ->
                           Log.Global.info
                             "Bitrue: Received %d messages so far"
                             !receive_count
                         | false -> ());
                              ...
                              try
                                let ic, oc = Core_unix.open_process "gunzip -c" in
                                  ...
                                  let result = In_channel.input_all ic in
                                    ...
                                    match String.length result > 0 with
                                    | true -> result
                                    | false -> payload
                              with
                              | exn ->
                                Core.eprintf
                                  "Bitrue: Decompression failed: %s\n%!"
                                  (Exn.to_string exn);
                                payload)
                          ...
                          (try
                             let json = Yojson.Safe.from_string decompressed_payload in
                               ...
                               | `Int ts ->
                                 ...
                                 Log.Global.info
                                   "Bitrue: Received ping #%d, responding with pong"
                                   !pong_count;
                               ...
                               | _ ->
                                 ...
                                 let%bind () =
                                   Pipe.write t.message_writer decompressed_payload
                                 in
                                   receive_loop ()
                           with
                           | _ ->
            ...
            let%bind () =
              Deferred.List.iter ~how:`Sequential streams ~f:(fun stream ->
                ...
                let msg_str = Yojson.Safe.to_string msg in
                  Log.Global.info
                    "Bitrue: Subscribing to %s"
                    (Ws.Stream.to_channel stream);
