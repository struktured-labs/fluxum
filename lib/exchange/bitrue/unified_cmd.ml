...
module Session_cmd = struct
  ...
  let command : string * Command.t =
    ...
    , Command.async
        ...
        (let%map_open.Command symbols_str =
           flag
             "--symbols"
             (required string)
         ...
         in
           fun () ->
             ...
             let symbol_list =
               String.split symbols_str ~on:',' |> List.map ~f:String.strip
             in
                     ...
                     | Some book_pipe ->
                       let count = ref 0 in
                         Pipe.iter book_pipe ~f:(fun book_result ->
                           ...
                           | Ok book ->
                             printf
                               "[%s] Order book update #%d - Best bid: $%.2f, Best ask: \
                                $%.2f\n"
                               symbol
                               !count
                     ...
                     | Some ledger_pipe ->
                       let count = ref 0 in
                         Pipe.iter ledger_pipe ~f:(fun entry ->
                           incr count;
                           printf
                             "[%s] Ledger update #%d - PnL: $%.2f, Position: %.8f\n"
                             symbol
                             !count
                             entry.Ledger.Entry.pnl
                             entry.Ledger.Entry.position;
                           (match !count >= limit with
