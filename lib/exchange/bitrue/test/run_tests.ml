...
let test_ws_parse_depth () =
  ...
  in
    ...
    | Bitrue.Ws.Message.Depth depth ->
      ignore
        (assert_true
           (String.is_substring depth.channel ~substring:"depth")
  ...
  let ticker = Bitrue.Ws.Stream.Ticker "BNBUSDT" in
    ignore
      (assert_string_equal
         ...
         (Bitrue.Ws.Stream.to_channel depth)
         "Depth channel");
    ignore
      (assert_string_equal
         "market_ethusdt_trade_ticker"
         (Bitrue.Ws.Stream.to_channel trade)
         "Trade channel");
    ignore
      (assert_string_equal
         "market_bnbusdt_ticker"
         (Bitrue.Ws.Stream.to_channel ticker)
         "Ticker channel")
...
let test_normalize_error_paths () =
  ...
  let bad_balance : Bitrue.Rest.Account.balance =
    {asset= "BTC"; free= "invalid_number"; locked= "0.5"}
  in
    ...
    in
      ...
      in
        ...
        in
          ...
          in
            (match
               Bitrue.Fluxum_adapter.Adapter.Normalize.order_response bad_order_resp
             with
             | Error msg ->
            ...
            in
              (match
                 Bitrue.Fluxum_adapter.Adapter.Normalize.order_from_status
                   bad_order_status
               with
               | Error msg ->
              ...
              in
                ...
                in
                  match
                    Bitrue.Fluxum_adapter.Adapter.Normalize.symbol_info symbol_info
                  with
                  | Ok info ->
