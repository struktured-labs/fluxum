...
module Types = struct
  ...
  type recent_trades = trade list [@@deriving sexp]
  ...
  let kline_of_yojson json =
    ...
    | `List
        ...
        :: _) ->
      let open_time =
        match open_time_json with
        | `Int ts -> Int64.of_int ts
        | `Intlit ts -> Int64.of_string ts
        | _ -> 0L
      in
      let close_time =
        match close_time_json with
        | `Int ts -> Int64.of_int ts
        | `Intlit ts -> Int64.of_string ts
        | _ -> 0L
      in
      let trades =
        match trades_json with
        | `Int n -> n
        | `Intlit n -> Int.of_string n
        | _ -> 0
      in
        Ok {open_time; open_; high; low; close; volume; close_time; quote_volume; trades}
  ...
  let sexp_of_klines klines = Sexp.List (List.map klines ~f:sexp_of_kline)
...
let exchange_info cfg : (Types.exchange_info, [> Error.t]) result Deferred.t =
  get ~cfg ~path:"/api/v1/exchangeInfo"
  >>| function
  | Error _ as err -> err
  | Ok json ->
...
let depth cfg ~symbol ?(limit = 100) ()
  : (Types.order_book, [> Error.t]) result Deferred.t
  =
...
(** Get recent trades *)
let trades cfg ~symbol ?(limit = 500) ()
  : (Types.recent_trades, [> Error.t]) result Deferred.t
  =
...
let klines cfg ~symbol ~interval ?(limit = 500) ()
  : (Types.klines, [> Error.t]) result Deferred.t
  =
  let path =
    sprintf "/api/v1/klines?symbol=%s&interval=%s&limit=%d" symbol interval limit
  in
    get ~cfg ~path
    >>| function
    | Error _ as err -> err
...
let build_query_string params =
  List.map params ~f:(fun (k, v) -> sprintf "%s=%s" k v) |> String.concat ~sep:"&"
...
let get_authenticated ~cfg ~path ~params : (Yojson.Safe.t, [> Error.t]) result Deferred.t =
  ...
  let headers =
    Cohttp.Header.of_list
      [("X-MBX-APIKEY", Cfg.api_key); ("Content-Type", "application/json")]
  in
    Monitor.try_with (fun () ->
...
let account cfg : (Account.response, [> Error.t]) result Deferred.t =
  get_authenticated ~cfg ~path:"/api/v1/account" ~params:[]
  >>| function
  | Error _ as err -> err
  | Ok json ->
...
let delete_authenticated ~cfg ~path ~params
  : (Yojson.Safe.t, [> Error.t]) result Deferred.t
  =
  let module Cfg = (val cfg : Cfg.S) in
  let timestamp = current_timestamp () in
  let params_with_ts = params @ [("timestamp", timestamp)] in
...
(** Place new order *)
let new_order cfg (req : Order.new_order_request)
  : (Order.new_order_response, [> Error.t]) result Deferred.t
  =
  let params =
    [ ("symbol", req.symbol)
  ...
  in
  let params =
    match req.price with
    | Some p -> params @ [("price", p)]
    | None -> params
  in
  let params =
    match req.time_in_force with
    | Some tif -> params @ [("timeInForce", Order.time_in_force_to_string tif)]
    | None -> params
  in
  let params =
    match req.new_client_order_id with
    | Some id -> params @ [("newClientOrderId", id)]
    | None -> params
  in
    post_authenticated ~cfg ~path:"/api/v1/order" ~params
    >>| function
    | Error _ as err -> err
    | Ok json ->
...
(** Query order status *)
let query_order cfg ~symbol ~order_id
  : (Order.order_status_response, [> Error.t]) result Deferred.t
  =
  let params = [("symbol", symbol); ("orderId", order_id)] in
    get_authenticated ~cfg ~path:"/api/v1/order" ~params
    >>| function
    | Error _ as err -> err
    | Ok json ->
...
(** Cancel order *)
let cancel_order cfg ~symbol ~order_id
  : (Order.cancel_response, [> Error.t]) result Deferred.t
  =
  let params = [("symbol", symbol); ("orderId", order_id)] in
    delete_authenticated ~cfg ~path:"/api/v1/order" ~params
    >>| function
    | Error _ as err -> err
    | Ok json ->
...
(** Get open orders *)
let open_orders cfg ~symbol
  : (Order.order_status_response list, [> Error.t]) result Deferred.t
  =
  let params = [("symbol", symbol)] in
    get_authenticated ~cfg ~path:"/api/v1/openOrders" ~params
    >>| function
    | Error _ as err -> err
    | Ok json ->
      (match json with
