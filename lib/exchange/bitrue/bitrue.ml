...
let depth_command : string * Command.t =
  ...
  , Command.async
      ...
      (let%map_open.Command symbol =
         flag "--symbol" (required string) ~doc:"STRING Trading pair (e.g. BTCUSDT)"
       and limit =
         flag
           "--limit"
  ...
  , Command.async
      ...
      (let%map_open.Command symbol =
       ...
       and interval =
         flag
           "--interval"
           (optional_with_default "1h" string)
       ...
       in
         fun () ->
           Rest.klines cfg ~symbol ~interval ~limit ()
           >>| function
           | Ok klines ->
...
let balances_command : string * Command.t =
  ...
  , Command.async
      ...
      (Command.Param.return (fun () ->
         ...
         | Ok resp ->
           let non_zero =
             List.filter resp.balances ~f:(fun b ->
...
let command : Command.t =
  Command.group
    ~summary:"Bitrue Exchange Commands"
