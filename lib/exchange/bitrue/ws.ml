...
module Stream = struct
  ...
  let to_channel = function
    ...
    | Kline {symbol; period} ->
      sprintf "market_%s_kline_%s" (String.lowercase symbol) period
  ...
  let to_subscribe_message stream =
    let channel = to_channel stream in
      `Assoc
        [ ("event", `String "sub")
        ; ("params", `Assoc [("channel", `String channel); ("cb_id", `String channel)]) ]
...
module Message = struct
  ...
  let depth_tick_of_yojson json =
    ...
    try
      ...
      let asks_json = member "asks" json in
        match
          (price_level_list_of_yojson buys_json, price_level_list_of_yojson asks_json)
        with
        | Ok buys, Ok asks -> Ok {buys; asks}
        | Error e, _ | _, Error e -> Error e
    with
    | _ -> Error "Failed to parse depth_tick"
  ...
  (** Ping/Pong *)
  type ping = {ping: int64} [@@deriving yojson {strict= false}, sexp]
      ...
      | _ ->
        ...
        (match Yojson.Safe.Util.member "pong" json with
         ...
         | _ ->
           ...
           let event_rep =
             Yojson.Safe.Util.member "event_rep" json |> Yojson.Safe.Util.to_string_option
           in
