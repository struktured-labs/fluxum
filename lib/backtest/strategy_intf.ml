...
module Signal = struct
  type t =
    ...
    | Close_short of {qty: float option}
    (** Close short position (qty = None means all) *)
    | Hold
  [@@deriving sexp]
...
module Registry = struct
  ...
  let find name = List.find !strategies ~f:(fun e -> String.equal e.name name)
  let list () = List.map !strategies ~f:(fun e -> (e.name, e.description))
end
