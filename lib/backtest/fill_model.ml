...
module Config = struct
  ...
  let no_costs = {slippage_pct= 0.; commission_pct= 0.; fill_on= `Close}
...
let fill_market ~config ~side ~qty candle =
  ...
  let commission = calculate_commission ~config ~price ~qty in
    `Filled {Fill.price; qty; side; commission; timestamp= candle.Candle.timestamp}
...
let simulate_fill ~config ~signal candle =
  ...
  match signal with
  | Hold -> `Pending
  | Buy {qty; limit= None} -> fill_market ~config ~side:`Buy ~qty candle
  | Buy {qty; limit= Some limit} -> fill_limit ~config ~side:`Buy ~qty ~limit candle
  | Sell {qty; limit= None} -> fill_market ~config ~side:`Sell ~qty candle
  | Sell {qty; limit= Some limit} -> fill_limit ~config ~side:`Sell ~qty ~limit candle
