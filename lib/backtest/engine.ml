...
module Config = struct
  ...
  let default =
    {initial_balance= 10000.; fill_model= Fill_model.Config.default; history_size= 100}
...
let process_fill state (fill : Fill_model.Fill.t) ~symbol ~config =
  let position = state.State.position in
    ...
    | `Buy ->
      ...
      let new_position, trade_record =
        ...
        | true ->
          ...
          let trade =
            ...
            | Some entry ->
              Some
                (Backtest_result.Trade_record.create
                   ...
                   ~commission:
                     (fill.commission
                      +. (entry.entry_price
                          *. close_qty
                          *. config.Config.fill_model.commission_pct)))
            | None -> None
          in
          let new_pos =
          ...
          let new_pos =
            match pending with
            | Some p ->
              { Strategy_intf.Position.qty= p.qty
        ...
        | false ->
          ...
          let total_cost =
            (position.avg_price *. position.qty) +. (fill.price *. fill.qty)
          in
          let new_avg =
          ...
          in
          let new_pos =
            {Strategy_intf.Position.qty= total_qty; avg_price= new_avg; unrealized_pnl= 0.}
          in
          let _ = pending in
            (* Will be used by caller to set pending_entry *)
            (new_pos, None)
      in
      let trades =
      ...
      in
        {state with balance= new_balance; position= new_position; trades; pending_entry}
    | `Sell ->
  ...
  | first :: _ ->
          ...
          let state =
            ...
            | Strategy_intf.Signal.Buy {qty; limit} ->
              let fill_result =
                match limit with
                | None ->
                  Fill_model.fill_market ~config:config.fill_model ~side:`Buy ~qty candle
                | Some l ->
                  Fill_model.fill_limit
                    ~config:config.fill_model
                    ~side:`Buy
                    ~qty
                    ~limit:l
                    candle
            ...
            | Strategy_intf.Signal.Sell {qty; limit} ->
              let fill_result =
                match limit with
                | None ->
                  Fill_model.fill_market ~config:config.fill_model ~side:`Sell ~qty candle
                | Some l ->
                  Fill_model.fill_limit
                    ~config:config.fill_model
                    ~side:`Sell
    ...
    let final_state =
      ...
      | true ->
        ...
        let fill_result =
          Fill_model.simulate_close ~config:config.fill_model ~side ~qty last_candle
        in
          (match fill_result with
    ...
    let result =
      Backtest_result.create
        ...
        ~config:
          { Backtest_result.slippage_pct= config.fill_model.slippage_pct
