...
module Config = struct
  ...
  let default =
    {fast_period= 10; slow_period= 30; position_size= 0.95 (* Use 95% of equity *)}
...
let calculate_sma history ~period =
  ...
  | true ->
    let prices =
      List.take history period |> List.map ~f:(fun c -> c.Backtest.Candle.close)
    in
...
let on_candle state (ctx : Backtest.Strategy_intf.Context.t) =
  ...
  in
  let new_state =
    {state with fast_sma; slow_sma; prev_fast= state.fast_sma; prev_slow= state.slow_sma}
  in
    (signal, new_state)
...
module Make (C : sig
    val config : Config.t
  end) : Backtest.Strategy_intf.S = struct
  let name = "sma-crossover"
