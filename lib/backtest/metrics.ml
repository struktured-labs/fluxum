...
let std_dev returns =
  ...
  | _ ->
    ...
    let variance =
      List.fold returns ~init:0. ~f:(fun acc r -> acc +. Float.square (r -. mean)) /. n
    in
      Float.sqrt variance
...
let calculate ~equity_curve ~trade_pnls ?(risk_free_rate = default_risk_free_rate) () =
  ...
  let profit_factor =
    ...
    | false ->
      (match Float.(gross_profit > 0.) with
       | true -> Float.infinity
       | false -> 0.)
  in
  (* Average P&L *)
