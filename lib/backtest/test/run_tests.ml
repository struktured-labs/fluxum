...
let assert_false condition msg = assert_true (not condition) msg
...
let test_candle_calculations () =
  ...
  let _ =
    assert_float_equal 50000.0 (Backtest.Candle.mid candle) "Mid price calculation"
  in
  (* Typical = (high + low + close) / 3 = (52000 + 48000 + 51000) / 3 = 50333.33 *)
  let _ =
    assert_float_equal
      50333.333333
      (Backtest.Candle.typical candle)
      ~tolerance:0.01
      "Typical price calculation"
  in
  (* Body = |close - open| = |51000 - 50000| = 1000 *)
  let _ =
    assert_float_equal 1000.0 (Backtest.Candle.body candle) "Body size calculation"
  in
...
let test_candle_csv_parsing () =
  printf "\n=== Candle: CSV Parsing ===\n";
  let row =
    ["2024-01-01T00:00:00Z"; "50000.0"; "51000.0"; "49000.0"; "50500.0"; "100.0"]
  in
...
let test_candle_sorting () =
  ...
  let _ =
    assert_equal
      ~equal:Time_ns.equal
      ~sexp_of_t:Time_ns_unix.sexp_of_t
      ts2
      (List.nth_exn sorted 0).timestamp
      "First candle is earliest"
  in
  let _ =
    assert_equal
      ~equal:Time_ns.equal
      ~sexp_of_t:Time_ns_unix.sexp_of_t
      ts3
      (List.nth_exn sorted 1).timestamp
      "Second candle is middle"
  in
  let _ =
    assert_equal
      ~equal:Time_ns.equal
      ~sexp_of_t:Time_ns_unix.sexp_of_t
      ts1
      (List.nth_exn sorted 2).timestamp
      "Third candle is latest"
  in
    ()
...
let test_metrics_std_dev () =
  ...
  (* Single value *)
  let _ =
    assert_float_equal 0.0 (Backtest.Metrics.std_dev [5.0]) "Single value std_dev = 0"
  in
  ...
  let _ =
    assert_float_equal
      2.0
      (Backtest.Metrics.std_dev values)
      ~tolerance:0.01
      "Known std_dev = 2"
  in
    ()
...
let test_metrics_calculate_basic () =
  ...
  in
  let trade_pnls = [500.0; 500.0] in
  (* Two winning trades *)
  let metrics = Backtest.Metrics.calculate ~equity_curve ~trade_pnls () in
  let _ =
    assert_float_equal 0.10 metrics.total_return ~tolerance:0.001 "Total return = 10%"
  in
  let _ =
    assert_equal
      ~equal:Int.equal
...
let test_metrics_win_loss () =
  ...
  let start = Time_ns_unix.of_string "2024-01-01 00:00:00Z" in
  let equity_curve =
    [(start, 10000.0); (Time_ns.add start (Time_ns.Span.of_day 30.0), 10000.0) (* Flat *)]
  in
  (* 3 winners, 2 losers *)
...
let test_metrics_max_drawdown () =
  ...
  let equity_curve =
    [ (start, 10000.0)
    ; (Time_ns.add start (Time_ns.Span.of_day 10.0), 12000.0)
    ; (* Peak *)
      (Time_ns.add start (Time_ns.Span.of_day 20.0), 9000.0)
    ; (* Trough *)
      (Time_ns.add start (Time_ns.Span.of_day 30.0), 11000.0) ]
  in
  let metrics = Backtest.Metrics.calculate ~equity_curve ~trade_pnls:[] () in
  let _ =
    assert_float_equal 0.25 metrics.max_drawdown ~tolerance:0.001 "Max drawdown = 25%"
  in
    ()
...
let test_fill_model_config () =
  ...
  let custom =
    Backtest.Fill_model.Config.create
      ~slippage_pct:0.002
...
let test_fill_model_base_price () =
  ...
  let _ =
    assert_float_equal
      50000.0
      (Backtest.Fill_model.base_price ~config:config_open candle)
      "Base price on Open = 50000"
  in
  let _ =
    assert_float_equal
      51000.0
      (Backtest.Fill_model.base_price ~config:config_close candle)
      "Base price on Close = 51000"
  in
  let _ =
    assert_float_equal
      50000.0
      (Backtest.Fill_model.base_price ~config:config_mid candle)
      "Base price on Midpoint = 50000"
  in
    ()

let test_fill_model_slippage () =
  printf "\n=== Fill Model: Slippage Application ===\n";
  let config = Backtest.Fill_model.Config.create ~slippage_pct:0.01 () in
  (* 1% slippage *)
...
let test_fill_model_commission () =
  ...
  let config = Backtest.Fill_model.Config.create ~commission_pct:0.001 () in
  (* 0.1% commission *)
  (* Commission on $50k trade of 1.0 BTC = $50 *)
  let commission =
    Backtest.Fill_model.calculate_commission ~config ~price:50000.0 ~qty:1.0
  in
  let _ = assert_float_equal 50.0 commission "Commission on $50k = $50" in
  (* Commission on $50k trade of 2.0 BTC = $100 *)
  let commission =
    Backtest.Fill_model.calculate_commission ~config ~price:50000.0 ~qty:2.0
  in
...
let test_fill_model_market_fill () =
  ...
  in
    ...
    | `Filled fill ->
      ...
      let _ =
        assert_float_equal 51051.0 fill.price ~tolerance:0.01 "Fill price with slippage"
      in
      let _ = assert_float_equal 1.0 fill.qty "Fill qty = 1.0" in
      (* Commission = 51051 * 1.0 * 0.001 = 51.051 *)
      let _ =
        assert_float_equal 51.051 fill.commission ~tolerance:0.01 "Fill commission"
      in
        ()
    | _ ->
      incr tests_run;
...
let test_fill_model_limit_fill () =
  ...
  in
    ...
    (match
       Backtest.Fill_model.fill_limit ~config ~side:`Buy ~qty:1.0 ~limit:49000.0 candle
     with
     | `Filled _ ->
       incr tests_run;
       incr tests_passed;
    ...
    (match
       Backtest.Fill_model.fill_limit ~config ~side:`Buy ~qty:1.0 ~limit:47000.0 candle
     with
     | `Pending ->
       incr tests_run;
       incr tests_passed;
    ...
    (match
       Backtest.Fill_model.fill_limit ~config ~side:`Sell ~qty:1.0 ~limit:51500.0 candle
     with
     | `Filled _ ->
       incr tests_run;
       incr tests_passed;
    ...
    match
      Backtest.Fill_model.fill_limit ~config ~side:`Sell ~qty:1.0 ~limit:53000.0 candle
    with
    | `Pending ->
      incr tests_run;
      incr tests_passed;
...
let test_fill_model_max_buy_qty () =
  ...
  let _ =
    assert_float_equal 0.1998 max_qty ~tolerance:0.001 "Max buy qty with commission"
  in
  let config_no_comm = Backtest.Fill_model.Config.create ~commission_pct:0.0 () in
  let max_qty =
    Backtest.Fill_model.max_buy_qty ~balance:10000.0 ~price:50000.0 ~config:config_no_comm
  in
    ...
    let sell_limit = Backtest.Signal.sell ~limit:50000.0 2.0 in
      (match sell_limit with
       | Backtest.Strategy_intf.Signal.Sell {qty; limit} ->
         ...
         let _ =
           assert_float_equal 50000.0 (Option.value_exn limit) "Sell limit = 50000"
         in
           ()
       | _ ->
         incr tests_run;
...
let test_position () =
  ...
  let long =
    {Backtest.Strategy_intf.Position.qty= 1.0; avg_price= 50000.0; unrealized_pnl= 0.0}
  in
...
let test_interval_parsing () =
  ...
  let _ =
    assert_float_equal
      1.0
      (Time_ns.Span.to_min (Backtest.Interval.of_string "1m"))
      "1m interval = 1 minute"
  in
  let _ =
    assert_float_equal
      5.0
      (Time_ns.Span.to_min (Backtest.Interval.of_string "5m"))
      "5m interval = 5 minutes"
  in
  let _ =
    assert_float_equal
      60.0
      (Time_ns.Span.to_min (Backtest.Interval.of_string "1h"))
      "1h interval = 60 minutes"
  in
  let _ =
    assert_float_equal
      1440.0
      (Time_ns.Span.to_min (Backtest.Interval.of_string "1d"))
      "1d interval = 1440 minutes"
  in
    ()
...
let test_synthetic_data_generation () =
  ...
  let candles =
    Backtest.Data_source.Synthetic.random_walk
      ~symbol:"BTCUSD"
      ~start
      ~end_
      ~interval
  ...
  let _ =
    assert_equal
      ...
      25
      (List.length candles)
      "24 hour candles generated (inclusive)"
  in
  ...
  let _ =
    assert_true
      Float.(first.open_ > 45000.0 && first.open_ < 55000.0)
      "First candle open within range"
  in
  (* All candles should have positive volume *)
  let _ =
    assert_true
      (List.for_all candles ~f:(fun c -> Float.(c.volume > 0.0)))
  ...
  let _ =
    assert_true
      (List.equal Backtest.Candle.equal candles sorted)
...
let test_trending_data_generation () =
  ...
  let start = Time_ns_unix.of_string "2024-01-01 00:00:00Z" in
  let end_ = Time_ns_unix.of_string "2024-01-08 00:00:00Z" in
  (* 1 week *)
  ...
  let _ =
    assert_equal
      ...
      8
      (List.length candles)
      "7 daily candles generated (inclusive)"
  in
    ()
...
let test_engine_config () =
  ...
  let _ =
    assert_float_equal 0.0005 custom.fill_model.commission_pct "Custom commission"
  in
    ()
    ...
    don't_wait_for
      (match%map
       ...
       | Ok result ->
         ...
         let _ =
           assert_true
             Float.(result.final_balance > result.initial_balance)
             "Buy and hold in uptrend is profitable"
         in
         let _ =
           assert_equal
             ~equal:Int.equal
...
let test_result_calculations () =
  ...
  let trade =
    Backtest.Result.Trade_record.create
      ~entry_time:start
      ~exit_time:end_
      ~symbol:"BTCUSD"
      ~side:`Long
...
let test_strategy_registry () =
  ...
  let _ =
    assert_true
      (List.mem names "sma-crossover" ~equal:String.equal)
      "sma-crossover registered"
  in
  let _ =
    assert_true (List.mem names "momentum" ~equal:String.equal) "momentum registered"
  in
  let _ =
    assert_true
      (List.mem names "buy-and-hold" ~equal:String.equal)
