(** Exchange Adapter Interface
    ...
    @see <https://github.com/struktured/fluxum/blob/main/CLAUDE.md>
...
module type S = sig
  ...
  val cancel_order : t -> Native.Order.id -> (unit, Native.Error.t) Deferred.Result.t
      ...
      {[
        ...
        let balances = List.map native_balances ~f:Adapter.Normalize.balance in
          List.iter balances ~f:(fun b ->
            if Float.(b.total > 0.0) then printf "%s: %.8f\n" b.currency b.total)
      ]} *)
  val balances : t -> (Native.Balance.t list, Native.Error.t) Deferred.Result.t
  ...
  (** Get closed/historical orders
      ...
      @param limit
        Maximum number of orders to return (default: exchange-specific, typically 100-500)
  ...
  (** Get recent public trades for a symbol
      ...
      @param limit
        Number of trades to return (default: exchange-specific, typically 100-1000)
  ...
  (** Get historical OHLCV candles for a symbol
      ...
      @param limit
        Maximum candles to return (default: exchange-specific, typically 500-1000)
        ...
        {[
          let%bind trade_pipe = Streams.trades adapter in
            Pipe.iter_without_pushback trade_pipe ~f:(fun native_trade ->
              ...
              | Error msg -> Log.error "Failed to normalize trade: %s" msg)
        ]} *)
  ...
  module Normalize : sig
    (** Convert exchange-specific types to unified Types
        ...
        {[
          ...
          | Error msg -> Log.error "Normalization failed: %s" msg
