...
module Book = struct
  ...
  let merge_levels (levels : Attributed_level.t list) : Attributed_level.t =
    ...
    | first :: _ as multiple ->
      ...
      let total_volume =
        List.fold multiple ~init:0. ~f:(fun acc level ->
          acc +. level.Attributed_level.volume)
      in
  ...
  let extract_common_levels_mapped
        ...
        ~(extract_mapped :
           book
           -> f:
                (Exchange_common.Order_book_base.Price_level.t
                 -> float * Attributed_level.t)
           -> (float * Attributed_level.t) list)
        ~(exchange : exchange_source)
    : (float * Attributed_level.t) list
    =
    ...
    | Some book ->
      extract_mapped
        book
        ~f:(fun (level : Exchange_common.Order_book_base.Price_level.t) ->
          ( level.price
          , {Attributed_level.price= level.price; volume= level.volume; exchange} ))
  ...
  let rebuild t =
    ...
    let all_bids =
      ...
      let kraken_bids =
        extract_common_levels_mapped
          ~book_opt:t.kraken_book
          ~extract_mapped:(fun book ~f ->
            Kraken.Order_book.Book.best_n_bids_map book ~n:100 ~f)
          ~exchange:Kraken
      in
      let hyperliquid_bids =
        extract_common_levels_mapped
          ~book_opt:t.hyperliquid_book
          ~extract_mapped:(fun book ~f ->
            Hyperliquid.Order_book.Book.best_n_bids_map book ~n:100 ~f)
      ...
      let bitrue_bids =
        extract_common_levels_mapped
          ...
          ~extract_mapped:(fun book ~f ->
            Bitrue.Order_book.Book.best_n_bids_map book ~n:100 ~f)
          ~exchange:Bitrue
      in
      let binance_bids =
        extract_common_levels_mapped
          ...
          ~extract_mapped:(fun book ~f ->
            Binance.Order_book.Book.best_n_bids_map book ~n:100 ~f)
          ~exchange:Binance
      in
      let coinbase_bids =
        extract_common_levels_mapped
          ...
          ~extract_mapped:(fun book ~f ->
            Coinbase.Order_book.Book.best_n_bids_map book ~n:100 ~f)
          ~exchange:Coinbase
      in
      let mexc_bids =
        extract_common_levels_mapped
          ...
          ~extract_mapped:(fun book ~f ->
            Mexc.Order_book.Book.best_n_bids_map book ~n:100 ~f)
          ~exchange:Mexc
      in
      let bybit_bids =
        extract_common_levels_mapped
          ...
          ~extract_mapped:(fun book ~f ->
            Bybit.Order_book.Book.best_n_bids_map book ~n:100 ~f)
          ~exchange:Bybit
      in
      let okx_bids =
        extract_common_levels_mapped
          ~book_opt:t.okx_book
          ~extract_mapped:(fun book ~f ->
            Okx.Order_book.Book.best_n_bids_map book ~n:100 ~f)
          ~exchange:Okx
      in
        [ gemini_bids
        ; kraken_bids
        ...
        ; okx_bids ]
        |> List.concat
        |> List.sort ~compare:(fun (p1, _) (p2, _) -> Float.compare p2 p1)
          (* Descending *)
    ...
    let all_asks =
      ...
      let kraken_asks =
        extract_common_levels_mapped
          ~book_opt:t.kraken_book
          ~extract_mapped:(fun book ~f ->
            Kraken.Order_book.Book.best_n_asks_map book ~n:100 ~f)
          ~exchange:Kraken
      in
      let hyperliquid_asks =
        extract_common_levels_mapped
          ~book_opt:t.hyperliquid_book
          ~extract_mapped:(fun book ~f ->
            Hyperliquid.Order_book.Book.best_n_asks_map book ~n:100 ~f)
          ~exchange:Hyperliquid
      in
      let bitrue_asks =
        extract_common_levels_mapped
          ~book_opt:t.bitrue_book
          ~extract_mapped:(fun book ~f ->
            Bitrue.Order_book.Book.best_n_asks_map book ~n:100 ~f)
          ~exchange:Bitrue
      in
      let binance_asks =
        extract_common_levels_mapped
          ~book_opt:t.binance_book
          ~extract_mapped:(fun book ~f ->
            Binance.Order_book.Book.best_n_asks_map book ~n:100 ~f)
          ~exchange:Binance
      in
      let coinbase_asks =
        extract_common_levels_mapped
          ~book_opt:t.coinbase_book
          ~extract_mapped:(fun book ~f ->
            Coinbase.Order_book.Book.best_n_asks_map book ~n:100 ~f)
          ~exchange:Coinbase
      in
      let mexc_asks =
        extract_common_levels_mapped
          ~book_opt:t.mexc_book
          ~extract_mapped:(fun book ~f ->
            Mexc.Order_book.Book.best_n_asks_map book ~n:100 ~f)
          ~exchange:Mexc
      in
      let bybit_asks =
        extract_common_levels_mapped
          ~book_opt:t.bybit_book
          ~extract_mapped:(fun book ~f ->
            Bybit.Order_book.Book.best_n_asks_map book ~n:100 ~f)
          ~exchange:Bybit
      in
      let okx_asks =
        extract_common_levels_mapped
          ~book_opt:t.okx_book
          ~extract_mapped:(fun book ~f ->
            Okx.Order_book.Book.best_n_asks_map book ~n:100 ~f)
          ~exchange:Okx
      in
        [ gemini_asks
        ; kraken_asks
  ...
  (** Get best bid *)
  let best_bid t =
    Map.min_elt t.bids |> Option.value_map ~default:Attributed_level.empty ~f:snd

  (** Get best ask *)
  let best_ask t =
    Map.min_elt t.asks |> Option.value_map ~default:Attributed_level.empty ~f:snd
  ...
  let rec exchange_to_string = function
    ...
    | Bybit -> "BYB"
    | Okx -> "OKX"
    | Multiple sources -> List.map sources ~f:exchange_to_string |> String.concat ~sep:"+"
  ...
  let pretty_print ?(max_depth = 10) t () =
    ...
    let clear_screen = "\027[2J\027[H" in
      ...
      printf "Sources: ";
      (match Option.is_some t.gemini_book with
       | true -> printf "[Gemini] "
       | false -> ());
      (match Option.is_some t.kraken_book with
       | true -> printf "[Kraken] "
       | false -> ());
      (match Option.is_some t.hyperliquid_book with
       | true -> printf "[Hyperliquid] "
       | false -> ());
      (match Option.is_some t.bitrue_book with
       | true -> printf "[Bitrue] "
       | false -> ());
      (match Option.is_some t.binance_book with
       | true -> printf "[Binance] "
       | false -> ());
      (match Option.is_some t.coinbase_book with
       | true -> printf "[Coinbase] "
       | false -> ());
      printf "\n\n";
    ...
    in
      ...
      List.iter exchange_asks ~f:(fun (buy_ex, buy_price, buy_vol) ->
        List.iter exchange_bids ~f:(fun (sell_ex, sell_price, sell_vol) ->
          match
            (not (equal_exchange_source buy_ex sell_ex)) && Float.(sell_price > buy_price)
          with
          | false -> ()
          | true ->
...
module Analytics = struct
  ...
  let calculate t =
    ...
    let total_bid_volume =
      List.fold bids ~init:0. ~f:(fun acc l -> acc +. l.Attributed_level.volume)
    in
    let total_ask_volume =
      List.fold asks ~init:0. ~f:(fun acc l -> acc +. l.Attributed_level.volume)
    in
    ...
    let num_exchanges =
      (match Option.is_some t.Book.gemini_book with
       | true -> 1
       | false -> 0)
      + (match Option.is_some t.kraken_book with
        | true -> 1
