...
module Order_kind = struct
  ...
  type conditional =
    ...
    | Stop_limit of
        { stop: Price.t
        ; limit: Price.t } (** Trigger limit at stop *)
    | Take_profit_market of Price.t (** Take profit market order *)
    | Take_profit_limit of
        { trigger: Price.t
        ; limit: Price.t }
...
module Order_book = struct
  ...
  let create ~venue ~symbol = {venue; symbol; bids= []; asks= []; ts= None; epoch= 0}
  ...
  (** Get top N bids *)
  let best_n_bids t ~n = List.take t.bids n

  (** Get top N asks *)
  let best_n_asks t ~n = List.take t.asks n
...
module Transfer_status = struct
  ...
  let of_string_opt = function
    ...
    | "completed" | "Completed" | "COMPLETED" | "complete" | "Complete" | "COMPLETE" ->
      Some Completed
    | "failed" | "Failed" | "FAILED" -> Some Failed
    | "cancelled" | "Cancelled" | "CANCELLED" | "canceled" | "Canceled" | "CANCELED" ->
      Some Cancelled
    | _ -> None
