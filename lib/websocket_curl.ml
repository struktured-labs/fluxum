...
let encode_frame ~opcode payload =
  ...
  let header = Buffer.create 14 in
    ...
    (* Mask bit + length *)
    (match len < 126 with
     | true -> Buffer.add_char header (Char.of_int_exn (0x80 lor len))
     | false ->
        ...
        [ Deferred.Or_error.try_with (fun () ->
            In_thread.run (fun () ->
              ...
              let rec send_all offset =
                ...
                | false ->
                  let sent =
                    Curl_ext.send t.curl bytes offset (Bytes.length bytes - offset)
                  in
                    (match sent = 0 with
      ...
      In_thread.run (fun () ->
            ...
            (match t.leftover_len > 0 with
             | true ->
               let to_copy = min len t.leftover_len in
                 Bytes.blit
                   ~src:t.leftover
                   ~src_pos:(Bytes.length t.leftover - t.leftover_len)
                   ~dst:buf
                   ~dst_pos:offset
                   ~len:to_copy;
        ...
        let read_frame () =
          ...
          in
            ...
            | false ->
              (match masked with
               | true -> recv_exact header_buf 0 4
               | false -> ());
