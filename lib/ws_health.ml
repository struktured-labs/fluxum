...
module Status = struct
  ...
  let to_string = function
    ...
    | Degraded {last_pong_age} ->
      sprintf
        "degraded (last pong %s ago)"
        (Time_float_unix.Span.to_short_string last_pong_age)
    | Unhealthy {reason} -> sprintf "unhealthy: %s" reason
end
                       ...
                       let ping_time = Option.value_exn t.last_ping_sent in
                         (match Time_float_unix.(t.last_pong >= ping_time) with
                          ...
                          | false ->
                            ...
                            Log.Global.info
                              "ws_health: pong timeout detected (attempt %d/%d)"
                              t.consecutive_timeouts
                              t.config.max_reconnect_attempts;
                            let current_status = status t in
                            let%bind () =
                              Pipe.write_if_open t.status_writer current_status
                            in
                              (match
                               ...
                               | true ->
                                 ...
                                 Log.Global.error
                                   "ws_health: max reconnect attempts reached, \
