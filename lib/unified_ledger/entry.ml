...
type t =
  ...
  ; largest_loss: float (* Timestamps *) (** Largest losing trade *)
...
let apply_fill t ~price ~qty ~side ~fee =
  ...
  let buy_volume, sell_volume, buy_notional, sell_notional =
    ...
    | Buy ->
      ( t.buy_volume +. qty
      , t.sell_volume
      , t.buy_notional +. trade_notional
      , t.sell_notional )
    | Sell ->
      ( t.buy_volume
      , t.sell_volume +. qty
      , t.buy_notional
      , t.sell_notional +. trade_notional )
  in
  ...
  let is_reducing =
    Float.(
      (old_position > 0. && signed_qty < 0.) || (old_position < 0. && signed_qty > 0.))
  in
  let realized_pnl_delta, trade_pnl =
    ...
    | true ->
      ...
      let pnl_per_unit =
        match Float.(old_position > 0.) with
...
(** Serialize to binary *)
let serialize t = Bin_prot.Writer.to_string bin_writer_t t

(** Deserialize from binary *)
let deserialize data =
  try Ok (Bin_prot.Reader.of_string bin_reader_t data) with
  | exn -> Error (Error.of_exn exn)

(** Summary string for display *)
let summary t =
  let pnl_sign =
    match Float.(t.total_pnl >= 0.) with
    | true -> "+"
    | false -> ""
  in
    sprintf
      "%s@%s: pos=%.8f avg=%.4f pnl=%s%.2f (r:%.2f u:%.2f) trades=%d"
      t.symbol
...
let report t =
  let buf = Buffer.create 512 in
    ...
    Buffer.add_string
      buf
      (sprintf "Buy Vol:      %.8f (%.2f notional)\n" t.buy_volume t.buy_notional);
    Buffer.add_string
      buf
      (sprintf "Sell Vol:     %.8f (%.2f notional)\n" t.sell_volume t.sell_notional);
