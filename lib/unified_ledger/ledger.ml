...
let create () = {entries= Map.empty (module Key); last_update= Time_ns.now ()}
...
let set_entry t entry =
  let key = {Key.symbol= entry.Entry.symbol; venue= entry.venue} in
    {entries= Map.set t.entries ~key ~data:entry; last_update= Time_ns.now ()}
...
(** Get all entries *)
let all_entries t = Map.data t.entries

(** Get all entries for a specific venue *)
let entries_for_venue t ~venue =
  Map.filter t.entries ~f:(fun entry -> Entry.Venue.equal entry.venue venue) |> Map.data

(** Get all entries for a specific symbol (across venues) *)
let entries_for_symbol t ~symbol =
  Map.filter t.entries ~f:(fun entry -> String.equal entry.symbol symbol) |> Map.data

(** Snapshot all entries for persistence *)
let snapshot t = Map.data t.entries
...
let deserialize_snapshot data =
  try Ok (Bin_prot.Reader.of_string [%bin_reader: Entry.t list] data) with
...
let total_pnl t =
  Map.fold t.entries ~init:0. ~f:(fun ~key:_ ~data:entry acc -> acc +. entry.total_pnl)

(** Get total realized P&L *)
let total_realized t =
  Map.fold t.entries ~init:0. ~f:(fun ~key:_ ~data:entry acc -> acc +. entry.realized_pnl)
...
(** Get total fees paid *)
let total_fees t =
  Map.fold t.entries ~init:0. ~f:(fun ~key:_ ~data:entry acc -> acc +. entry.total_fees)
...
let pnl_by_venue t =
  Map.fold
    t.entries
    ~init:(Map.empty (module Entry.Venue))
    ~f:(fun ~key:_ ~data:entry acc ->
...
let summary t =
  ...
  in
    sprintf
      "Ledger: %d entries, P&L: %s%.2f (realized: %.2f, unrealized: %.2f)"
      (count t)
      sign
      total
      realized
