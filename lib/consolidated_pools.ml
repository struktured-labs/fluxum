...
module Attributed_pool = struct
  ...
  let compare_by_price a b = Float.compare (spot_price a) (spot_price b)
end
  ...
  | Some ap ->
    ...
    let key = (pool.token0.symbol, pool.token1.symbol) in
      ...
      Hashtbl.update t.pools_by_pair key ~f:(function
        | None -> []
        | Some pools ->
          List.filter pools ~f:(fun p -> not (String.equal p.pool.id pool_id)));
...
let get_pools (t : t) ~(token0 : string) ~(token1 : string) : Attributed_pool.t list =
  Hashtbl.find t.pools_by_pair (token0, token1) |> Option.value ~default:[]
...
let quote_for_pool (ap : Attributed_pool.t) ~(amount_in : float)
  ...
  let quote_result =
    ...
    | Pool_intf.Pool_type.Constant_product ->
      let params =
        Amm_pricing.Constant_product.
          {reserve0= pool.reserve0; reserve1= pool.reserve1; fee_bps= pool.fee_bps}
      in
    ...
    | Pool_intf.Pool_type.Weighted ->
      ...
      let balance_in =
        Amm_pricing.Weighted.
          {balance= pool.reserve0; weight= 0.5; decimals= pool.token0.decimals}
      in
      let balance_out =
        Amm_pricing.Weighted.
          {balance= pool.reserve1; weight= 0.5; decimals= pool.token1.decimals}
      in
      let params =
        Amm_pricing.Weighted.
  ...
  in
    ...
    | Error _ -> None
    | Ok quote -> Some (Attributed_quote.create ~venue:ap.venue ~pool_id:pool.id ~quote)
...
let best_quote (t : t) ~(amount : float) ~(token_in : string) ~(token_out : string)
  : Attributed_quote.t option
  =
  best_quotes t ~amount ~token_in ~token_out |> List.hd
      ...
      ~f:(fun ap ->
        let pool = ap.pool in
          ...
          | Pool_intf.Pool_type.Liquidity_bin ->
            ...
            in
              (match
                 Amm_pricing.Constant_product.depth_at_impact params ~max_impact_pct
               with
               | Ok depth -> depth
               | Error _ -> 0.0)
...
let pair_liquidity (t : t) ~(token0 : string) ~(token1 : string) : Pair_liquidity.t option
  ...
  let pools = get_pools t ~token0 ~token1 in
    ...
    | pools ->
      ...
      let best_spot_price, best_venue =
        match best with
      ...
      let weighted_sum =
        List.fold pools ~init:0.0 ~f:(fun acc ap ->
          acc +. (ap.pool.spot_price *. ap.pool.tvl_usd))
      in
      let weighted_avg =
        match Float.(total_tvl > 0.0) with
        | true -> weighted_sum /. total_tvl
        | false -> 0.0
      in
        Some
          { Pair_liquidity.token0
          ; token1
...
let all_pairs (t : t) : (string * string) list = Hashtbl.keys t.pools_by_pair
...
let split_route
  ...
  let pools = get_pools t ~token0:token_in ~token1:token_out in
    ...
    | false ->
      ...
      let sorted_by_tvl =
        List.sort pools ~compare:(fun a b -> Float.compare b.pool.tvl_usd a.pool.tvl_usd)
      in
...
let find_arbitrage (t : t) ~(token0 : string) ~(token1 : string) ~(min_profit_bps : int)
  ...
  let pools = get_pools t ~token0 ~token1 in
  let pool_pairs =
    List.concat_map pools ~f:(fun p1 ->
