...
let test_side_conversions () =
  ...
  let default_result =
    Fluxum.Normalize_common.Side.of_string_exn ~default:Fluxum.Types.Side.Sell "invalid"
  in
...
let test_order_status_conversions () =
  ...
  (match Fluxum.Normalize_common.Order_status.of_string "partially_filled" with
   | Ok Fluxum.Types.Order_status.Partially_filled ->
     pass "partially_filled -> Partially_filled"
...
let test_order_type_conversions () =
  ...
  (match Fluxum.Normalize_common.Order_type.of_string "stop_loss" with
   | Ok (Fluxum.Types.Order_kind.Conditional (Stop_market _)) ->
     pass "stop_loss -> Stop_market"
   | _ -> fail "stop_loss should be Stop_market");
  (match Fluxum.Normalize_common.Order_type.of_string "stop_limit" with
   | Ok (Fluxum.Types.Order_kind.Conditional (Stop_limit _)) ->
     pass "stop_limit -> Stop_limit"
