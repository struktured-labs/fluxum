    ...
    | Ok normalized ->
      ...
      let _ =
        assert_equal
          ...
          Fluxum.Types.Venue.Binance
          normalized.venue
          "Venue is Binance"
      in
        ()
    | Error msg -> failwith (Printf.sprintf "Failed to normalize balance: %s" msg)
...
let test_mexc_balance_normalization () =
  ...
  let native_balance : Mexc.V1.Account.balance =
    {asset= "ETH"; free= "10.25"; locked= "2.75"}
  in
  let normalized =
    match Mexc.Fluxum_adapter.Adapter.Normalize.balance native_balance with
    | Ok bal -> bal
...
let test_coinbase_balance_normalization () =
  ...
  in
  let normalized =
    match Coinbase.Fluxum_adapter.Adapter.Normalize.balance native_balance with
    | Ok bal -> bal
...
let test_bitrue_balance_normalization () =
  ...
  let native_balance : Bitrue.Rest.Account.balance =
    {asset= "USDT"; free= "1000.0"; locked= "250.0"}
  in
  let normalized =
    match Bitrue.Fluxum_adapter.Adapter.Normalize.balance native_balance with
    | Ok bal -> bal
    | Error msg -> failwith (sprintf "Failed to normalize balance: %s" msg)
  in
  let _ =
...
let test_aggregated_balance () =
  ...
  let _ =
    assert_float_equal 1.0 agg1.available_across_exchanges "After 1st: available = 1.0"
  in
  let _ =
    assert_equal
      ~equal:Int.equal
      ...
      1
      (List.length agg1.by_exchange)
      "After 1st: 1 exchange"
  in
  ...
  let _ =
    assert_float_equal 3.0 agg2.available_across_exchanges "After 2nd: available = 3.0"
  in
  let _ =
    assert_equal
      ~equal:Int.equal
      ~sexp_of_t:Int.sexp_of_t
      2
      (List.length agg2.by_exchange)
      "After 2nd: 2 exchanges"
  in
    ()
...
let test_exchange_result () =
  printf "\n=== Exchange Result ===\n";
  let success =
    Consolidated_balance.Exchange_result.success
      ~venue:Fluxum.Types.Venue.Binance
      ~balances:[]
      ~latency_ms:100.0
  in
  ...
  let failure =
    Consolidated_balance.Exchange_result.failure
  ...
  let _ =
    assert_equal
      ...
      ~sexp_of_t:String.sexp_of_t
      "Connection timeout"
      (Option.value_exn failure.error)
...
let test_consolidated_view () =
  ...
  in
  let view =
    Consolidated_balance.Consolidated_view.create ~exchange_results:[result1; result2]
  in
  let _ =
    assert_equal
      ~equal:Int.equal
      ~sexp_of_t:Int.sexp_of_t
      2
      (List.length view.exchange_results)
      "2 exchange results"
  in
  ...
  let _ =
    assert_float_equal 2.3 btc_agg.available_across_exchanges "BTC available = 2.3"
  in
  let _ =
    assert_equal
      ~equal:Int.equal
      ~sexp_of_t:Int.sexp_of_t
      2
      (List.length btc_agg.by_exchange)
      "BTC from 2 exchanges"
  in
  ...
  let _ =
    assert_equal
      ...
      ~sexp_of_t:Int.sexp_of_t
      1
      (List.length eth_agg.by_exchange)
      "ETH from 1 exchange"
  in
  (* Check summary *)
  let summary = Consolidated_balance.Consolidated_view.summary view in
  let _ =
    assert_true (String.is_substring summary ~substring:"2 ok") "Summary shows 2 ok"
  in
  let _ =
    assert_true
      (String.is_substring summary ~substring:"0 failed")
...
let test_non_zero_balances () =
  ...
  let _ =
    assert_equal
      ...
      "Only 1 non-zero balance (dust filtered)"
  in
