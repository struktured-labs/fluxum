...
let%test_module "Attributed_level" =
  (module struct
    ...
    let%test "multiple exchanges aggregation" =
      let level =
        Attributed_level.create
          ~price:50000.
          ~volume:1.0
          ~exchange:(Multiple [Gemini; Kraken])
      in
...
let%test_module "Book - Creation and Basic Operations" =
  (module struct
...
let%test_module "Book - Single Exchange Updates" =
  (module struct
    ...
    let%test "update from kraken book" =
      ...
      let kraken_book =
        Kraken.Order_book.Book.set kraken_book ~side:`Ask ~price:51000. ~size:2.0
      in
    ...
    let%test "update from binance book" =
      ...
      let binance_book =
        Binance.Order_book.Book.set binance_book ~side:`Bid ~price:49999. ~size:3.0
      in
    ...
    let%test "sequential updates increment epoch" =
      ...
      let gemini_book = make_gemini_book "BTC/USD" in
      let gemini_book =
        Gemini.Order_book.Book.set gemini_book ~side:`Bid ~price:50000. ~size:1.0
      in
      let consolidated = Book.update_gemini consolidated gemini_book in
      let epoch1 = Book.epoch consolidated in
      let kraken_book = make_kraken_book "BTC/USD" in
      let kraken_book =
        Kraken.Order_book.Book.set kraken_book ~side:`Bid ~price:50001. ~size:1.0
      in
      let consolidated = Book.update_kraken consolidated kraken_book in
...
let%test_module "Book - Multi-Exchange Aggregation" =
  (module struct
    let%test "best bid from multiple exchanges - simple" =
      ...
      let kraken_book = make_kraken_book "BTC/USD" in
      let kraken_book =
        Kraken.Order_book.Book.set kraken_book ~side:`Bid ~price:50001. ~size:1.5
      in
      let consolidated = Book.update_kraken consolidated kraken_book in
      let best_bid = Book.best_bid consolidated in
    ...
    let%test "best ask from multiple exchanges - simple" =
      ...
      let gemini_book = make_gemini_book "BTC/USD" in
      let gemini_book =
        Gemini.Order_book.Book.set gemini_book ~side:`Ask ~price:51000. ~size:1.0
      in
      let consolidated = Book.update_gemini consolidated gemini_book in
      (* Kraken: ask at 50999 (better) *)
      let kraken_book = make_kraken_book "BTC/USD" in
      let kraken_book =
        Kraken.Order_book.Book.set kraken_book ~side:`Ask ~price:50999. ~size:1.5
      in
      let consolidated = Book.update_kraken consolidated kraken_book in
      let best_ask = Book.best_ask consolidated in
    ...
    let%test "volume aggregation at same price" =
      ...
      let kraken_book = make_kraken_book "BTC/USD" in
      let kraken_book =
        Kraken.Order_book.Book.set kraken_book ~side:`Bid ~price:50000. ~size:2.0
      in
      let consolidated = Book.update_kraken consolidated kraken_book in
...
let%test_module "Book - Spread Calculations" =
  (module struct
    let%test "spread across exchanges" =
      ...
      let kraken_book = make_kraken_book "BTC/USD" in
      let kraken_book =
        Kraken.Order_book.Book.set kraken_book ~side:`Ask ~price:50100. ~size:1.0
      in
      let consolidated = Book.update_kraken consolidated kraken_book in
    ...
    let%test "tight spread detection" =
      ...
      let kraken_book = make_kraken_book "BTC/USD" in
      let kraken_book =
        Kraken.Order_book.Book.set kraken_book ~side:`Ask ~price:50001. ~size:1.0
      in
      let consolidated = Book.update_kraken consolidated kraken_book in
        match Book.spread consolidated with
    ...
    let%test "best prices from different exchanges" =
      ...
      let gemini_book = make_gemini_book "BTC/USD" in
      let gemini_book =
        Gemini.Order_book.Book.set gemini_book ~side:`Bid ~price:49990. ~size:1.0
      in
      let gemini_book =
        Gemini.Order_book.Book.set gemini_book ~side:`Ask ~price:50110. ~size:1.0
      in
      let consolidated = Book.update_gemini consolidated gemini_book in
      (* Kraken: better bid *)
      let kraken_book = make_kraken_book "BTC/USD" in
      let kraken_book =
        Kraken.Order_book.Book.set kraken_book ~side:`Bid ~price:50000. ~size:1.0
      in
      let consolidated = Book.update_kraken consolidated kraken_book in
      (* Binance: better ask *)
      let binance_book = make_binance_book "BTCUSDT" in
      let binance_book =
        Binance.Order_book.Book.set binance_book ~side:`Ask ~price:50100. ~size:1.0
      in
      let consolidated = Book.update_binance consolidated binance_book in
...
let%test_module "Book - Deep Books" =
  (module struct
    let%test "multiple price levels from single exchange" =
      ...
      in
      let gemini_book =
        Gemini.Order_book.Book.set gemini_book ~side:`Bid ~price:49999. ~size:1.5
      in
      let gemini_book =
        Gemini.Order_book.Book.set gemini_book ~side:`Bid ~price:49998. ~size:2.0
      in
      let consolidated = Book.update_gemini consolidated gemini_book in
      let bid_levels = Book.bids consolidated in
    ...
    let%test "interleaved price levels from multiple exchanges" =
      ...
      in
      let gemini_book =
        Gemini.Order_book.Book.set gemini_book ~side:`Bid ~price:49998. ~size:1.0
      in
      let consolidated = Book.update_gemini consolidated gemini_book in
      (* Kraken: 49999, 49997 *)
      let kraken_book = make_kraken_book "BTC/USD" in
      let kraken_book =
        Kraken.Order_book.Book.set kraken_book ~side:`Bid ~price:49999. ~size:1.0
      in
      let kraken_book =
        Kraken.Order_book.Book.set kraken_book ~side:`Bid ~price:49997. ~size:1.0
      in
      let consolidated = Book.update_kraken consolidated kraken_book in
      let bid_levels = Book.bids consolidated in
    ...
    let%test "level removal on zero volume" =
      ...
      (* Remove the level *)
      let gemini_book =
        Gemini.Order_book.Book.set gemini_book ~side:`Bid ~price:50000. ~size:0.0
      in
      let consolidated = Book.update_gemini consolidated gemini_book in
      let bid_levels = Book.bids consolidated in
...
let%test_module "Arbitrage Detection" =
  (module struct
    let%test "no arbitrage when properly ordered" =
      ...
      in
      let gemini_book =
        Gemini.Order_book.Book.set gemini_book ~side:`Ask ~price:50100. ~size:1.0
      in
      let consolidated = Book.update_gemini consolidated gemini_book in
        match Book.spread consolidated with
    ...
    let%test "potential arbitrage - crossed market" =
      ...
      let kraken_book = make_kraken_book "BTC/USD" in
      let kraken_book =
        Kraken.Order_book.Book.set kraken_book ~side:`Bid ~price:51100. ~size:1.0
      in
      let consolidated = Book.update_kraken consolidated kraken_book in
        match Book.spread consolidated with
...
let%test_module "Edge Cases and Stress Tests" =
  (module struct
    let%test "very close prices" =
      let consolidated = Book.empty "BTC/USD" in
      let gemini_book = make_gemini_book "BTC/USD" in
      let gemini_book =
        Gemini.Order_book.Book.set gemini_book ~side:`Bid ~price:50000.0001 ~size:1.0
      in
      let consolidated = Book.update_gemini consolidated gemini_book in
      let kraken_book = make_kraken_book "BTC/USD" in
      let kraken_book =
        Kraken.Order_book.Book.set kraken_book ~side:`Bid ~price:50000.0002 ~size:1.0
      in
      let consolidated = Book.update_kraken consolidated kraken_book in
      let best_bid = Book.best_bid consolidated in
    ...
    let%test "large volume aggregation" =
      ...
      let gemini_book = make_gemini_book "BTC/USD" in
      let gemini_book =
        Gemini.Order_book.Book.set gemini_book ~side:`Bid ~price:50000. ~size:100_000.0
      in
      let consolidated = Book.update_gemini consolidated gemini_book in
      let kraken_book = make_kraken_book "BTC/USD" in
      let kraken_book =
        Kraken.Order_book.Book.set kraken_book ~side:`Bid ~price:50000. ~size:200_000.0
      in
      let consolidated = Book.update_kraken consolidated kraken_book in
      let bid_levels = Book.bids consolidated in
        match Map.find bid_levels 50000. with
    ...
    let%test "exchange override - later update wins for same source" =
      ...
      let gemini_book1 =
        Gemini.Order_book.Book.set gemini_book1 ~side:`Bid ~price:50000. ~size:1.0
      in
      let consolidated = Book.update_gemini consolidated gemini_book1 in
      let gemini_book2 = make_gemini_book "BTC/USD" in
      let gemini_book2 =
        Gemini.Order_book.Book.set gemini_book2 ~side:`Bid ~price:50000. ~size:5.0
      in
