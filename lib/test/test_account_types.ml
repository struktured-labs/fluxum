...
let test_transfer_status_to_string () =
  ...
  ignore
    (assert_string_equal
       "pending"
       (Types.Transfer_status.to_string Types.Transfer_status.Pending)
       "Pending -> pending");
  ignore
    (assert_string_equal
       "processing"
       (Types.Transfer_status.to_string Types.Transfer_status.Processing)
       "Processing -> processing");
  ignore
    (assert_string_equal
       "completed"
       (Types.Transfer_status.to_string Types.Transfer_status.Completed)
       "Completed -> completed");
  ignore
    (assert_string_equal
       "failed"
       (Types.Transfer_status.to_string Types.Transfer_status.Failed)
       "Failed -> failed");
  ignore
    (assert_string_equal
       "cancelled"
       (Types.Transfer_status.to_string Types.Transfer_status.Cancelled)
       "Cancelled -> cancelled")
...
let test_deposit_address_equality () =
  ...
  in
    ignore
      (assert_true (Types.Deposit_address.equal addr1 addr2) "Same addresses are equal");
    ignore
      (assert_true
  ...
  in
    ignore
      (assert_equal
         ~equal:[%equal: string option]
         ~sexp_of_t:[%sexp_of: string option]
         (Some "0x742d35Cc6634C0532925a3b844Bc9e7595f3aEc2")
         deposit.address
         "Address is set");
    ...
    in
      ...
      in
        ignore
          (assert_float_equal
             999999999999.99
             large_deposit.amount
             ~tolerance:0.01
             "Large amount allowed")
...
let test_withdrawal_sexp_roundtrip () =
  ...
  let fixed_time =
    Time_float_unix.of_span_since_epoch (Time_float_unix.Span.of_sec 1700000000.0)
  in
  let withdrawal =
    Types.Withdrawal.create
      ~venue:Types.Venue.Coinbase
...
let test_withdrawal_equality () =
  ...
  let wdl3 =
    Types.Withdrawal.create
      ...
      ()
  in
    ignore (assert_true (Types.Withdrawal.equal wdl1 wdl2) "Same withdrawals are equal");
    ignore
      (assert_true (not (Types.Withdrawal.equal wdl1 wdl3)) "Different IDs are not equal")
  ...
  in
    ...
    in
      ignore
        (assert_true
           (Float.( = )
              (Option.value ~default:0.0 fee_equals_amount.fee)
              fee_equals_amount.amount)
