...
let decode_frame_header frame =
  ...
  let payload_len, header_len =
    ...
    | false ->
      (match len_initial = 126 with
       | true ->
         let len = (Char.to_int frame.[2] lsl 8) lor Char.to_int frame.[3] in
           (len, 4)
       | false ->
...
let%test_module "compute_accept_key" =
  (module struct
...
let%test_module "payload_masking" =
  (module struct
    ...
    let%test "text frame payload round-trips" = roundtrip_payload ~opcode:1 "hello world"

    let%test "pong frame payload round-trips" =
      roundtrip_payload ~opcode:10 "ping-echo-data"

    let%test "empty payload round-trips" = roundtrip_payload ~opcode:1 ""
    let%test "single byte payload" = roundtrip_payload ~opcode:1 "x"
    ...
    let%test "payload with null bytes" = roundtrip_payload ~opcode:1 "hello\x00world\x00"

    let%test "JSON payload (typical exchange message)" =
      let json =
        {|{"event":"heartbeat","channel":"ticker","data":{"price":"50000.00"}}|}
      in
        roundtrip_payload ~opcode:1 json

    let%test "large JSON payload" =
      let json =
        String.init 500 ~f:(fun i ->
          match i mod 10 with
          | 0 -> '{'
          | 9 -> '}'
          | _ -> Char.of_int_exn (Char.to_int 'a' + (i mod 26)))
      in
        roundtrip_payload ~opcode:1 json
  end)
...
let%test_module "frame_length_encoding" =
  (module struct
...
let%test_module "frame_total_size" =
  (module struct
