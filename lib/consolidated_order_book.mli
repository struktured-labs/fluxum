...
type exchange_source =
  ...
  | Bybit
  | Okx
  | Multiple of exchange_source list (** Multiple exchanges at the same price (merged) *)
...
module Attributed_level : sig
  ...
  type t =
    ...
    ; volume: float (** Total volume available (sum if Multiple exchanges) *)
    ; exchange: exchange_source (** Which exchange(s) provide this liquidity *) }
...
module Book : sig
  ...
  type t =
    { symbol: string (** Trading symbol (exchange-agnostic, e.g., "BTCUSD") *)
    ...
    ; gemini_book: Gemini.Order_book.Book.t option (** Gemini-specific order book *)
    ; kraken_book: Kraken.Order_book.Book.t option (** Kraken-specific order book *)
    ; hyperliquid_book: Hyperliquid.Order_book.Book.t option
      (** Hyperliquid-specific order book *)
    ; bitrue_book: Bitrue.Order_book.Book.t option (** Bitrue-specific order book *)
    ; binance_book: Binance.Order_book.Book.t option (** Binance-specific order book *)
    ; coinbase_book: Coinbase.Order_book.Book.t option (** Coinbase-specific order book *)
    ; mexc_book: Mexc.Order_book.Book.t option (** MEXC-specific order book *)
    ; bybit_book: Bybit.Order_book.Book.t option (** Bybit-specific order book *)
    ; okx_book: Okx.Order_book.Book.t option (** OKX-specific order book *)
    ; epoch: int (** Update counter (increments on every rebuild) *)
    ; update_time: Time_float_unix.t (** Last rebuild timestamp *) }
  ...
  (** Get best bid across all exchanges
      ...
      {[
        let best = Book.best_bid book in
          ...
          | false -> printf "No bids available\n"
      ]} *)
  ...
  (** Calculate VWAP for selling given volume
      ...
      {[
        ...
        | Some vwap -> printf "Selling 10 BTC would yield avg price: $%.2f\n" vwap
        | None -> printf "Insufficient bid liquidity for 10 BTC\n"
      ]} *)
...
module Arbitrage : sig
  ...
  type opportunity =
    { buy_exchange: exchange_source (** Where to buy (lowest ask) *)
    ; sell_exchange: exchange_source (** Where to sell (highest bid) *)
    ; buy_price: float (** Best ask on buy exchange *)
    ; sell_price: float (** Best bid on sell exchange *)
    ; profit_per_unit: float (** Gross profit per unit: sell_price - buy_price *)
    ; profit_percentage: float (** Profit as percentage: (profit / buy_price) × 100 *)
    ; max_volume: float
...
module Analytics : sig
  ...
  type metrics =
    { total_bid_volume: float (** Sum of all bid volumes across all levels and exchanges *)
    ; total_ask_volume: float (** Sum of all ask volumes *)
    ; weighted_mid_price: float (** (best_bid + best_ask) / 2 *)
    ; bid_depth_1pct: float (** Total volume within 1% of best bid *)
    ; ask_depth_1pct: float (** Total volume within 1% of best ask *)
    ; spread_bps: float (** Spread in basis points: (spread / best_bid) × 10000 *)
    ; num_exchanges: int (** Number of exchanges with active books *)
    ; best_bid_exchange: exchange_source (** Which exchange has the best bid *)
    ; best_ask_exchange: exchange_source (** Which exchange has the best ask *) }
  [@@deriving sexp, fields]
