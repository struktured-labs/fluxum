...
(** Encode EIP-1559 transaction payload for signing (without signature).
    ...
    EIP-2718 envelope: 0x02 || rlp([chain_id, nonce, max_priority_fee_per_gas, max_fee_per_gas, gas_limit, to, value, data, access_list])
*)
...
let sign (tx : eip1559_tx) ~(private_key_hex : string) : (string, string) Result.t =
  ...
  with
  | e -> Error (sprintf "Transaction signing failed: %s" (Exn.to_string e))
