...
module Constant_product = struct
  ...
  let quote (p : params) ~(amount_in : float) ~(pool_id : string)
    : Pool_intf.Quote.t pricing_result
    =
    let open Result.Let_syntax in
    let%bind amount_out = get_amount_out p ~amount_in in
...
module Concentrated = struct
  ...
  let price_from_sqrt_price_x96
        ~(sqrt_price_x96 : string)
        ~(decimals0 : int)
        ~(decimals1 : int)
    : float pricing_result
    =
    let open Result.Let_syntax in
    let%bind sqrt_price_raw =
      Fluxum.Normalize_common.Float_conv.of_string sqrt_price_x96
    in
  ...
  let get_amount_out (p : params) ~(amount_in : float) : float pricing_result =
    let open Result.Let_syntax in
    let%bind sqrt_price_raw =
      Fluxum.Normalize_common.Float_conv.of_string p.sqrt_price_x96
    in
    let sqrt_price = sqrt_price_raw /. q96 in
...
module Liquidity_bin = struct
  ...
  let get_amount_out (p : params) ~(amount_in : float) : float pricing_result =
    ...
    let sorted_bins =
      List.sort p.bins ~compare:(fun a b -> Int.compare a.bin_id b.bin_id)
    in
...
module Weighted = struct
  ...
  let spot_price ~(balance_in : token_balance) ~(balance_out : token_balance)
    : float pricing_result
    =
    match
      Float.(
        balance_in.balance > 0.0 && balance_in.weight > 0.0 && balance_out.weight > 0.0)
    with
    ...
    | true ->
      ...
      let spot =
        balance_out.balance
        /. balance_out.weight
        /. (balance_in.balance /. balance_in.weight)
      in
        Ok spot
  ...
  let get_amount_out
        (p : params)
        ~(balance_in : token_balance)
        ~(balance_out : token_balance)
        ~(amount_in : float)
    : float pricing_result
    =
    ...
    | true ->
      ...
      let balance_ratio =
        balance_in.balance /. (balance_in.balance +. amount_in_after_fee)
      in
...
module Stable = struct
  ...
  let get_d ~(balances : float list) ~(amp : int) : float =
    ...
    let rec newton d iterations =
      ...
      | false ->
        let d_p =
          List.fold balances ~init:d ~f:(fun acc x ->
        ...
        let new_d =
          match Float.(denominator > 0.0) with
  ...
  let get_y
    ...
    let s =
      List.foldi balances ~init:0.0 ~f:(fun i acc x ->
    ...
    let c =
      List.foldi balances ~init:d ~f:(fun i acc x ->
        match i = token_in_idx with
  ...
  let get_amount_out
    ...
    | true ->
      ...
      let%bind balance_in =
        Result.of_option
          (List.nth p.balances token_in_idx)
          ~error:
            (sprintf
               "token_in_idx %d out of bounds (pool has %d tokens)"
               token_in_idx
               (List.length p.balances))
      in
      let%bind balance_out =
        Result.of_option
          (List.nth p.balances token_out_idx)
          ~error:
            (sprintf
               "token_out_idx %d out of bounds (pool has %d tokens)"
               token_out_idx
               (List.length p.balances))
      in
      ...
      let%bind new_balance_out =
        get_y ~balances:p.balances ~amp:p.amp ~d ~token_in_idx ~new_balance_in
      in
  ...
  let spot_price (p : params) ~(token_in_idx : int) ~(token_out_idx : int)
    : float pricing_result
    =
    ...
    let epsilon = 0.0001 in
      let open Result.Let_syntax in
      let%bind amount_out =
        get_amount_out p ~amount_in:epsilon ~token_in_idx ~token_out_idx
      in
