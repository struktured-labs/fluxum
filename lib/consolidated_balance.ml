...
let query_exchange (config : Query_config.exchange_config) : Exchange_result.t Deferred.t =
  ...
  | Query_config.Binance cfg ->
    ...
    | Ok balances ->
      let normalized = List.map balances ~f:Binance.Fluxum_adapter.Adapter.Normalize.balance in
  ...
  | Query_config.Coinbase cfg ->
    ...
    | Ok balances ->
      let normalized = List.map balances ~f:Coinbase.Fluxum_adapter.Adapter.Normalize.balance in
