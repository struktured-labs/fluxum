...
module Exchange_result = struct
  ...
  let success ~venue ~balances ~latency_ms = {venue; balances; error= None; latency_ms}
...
module Consolidated_view = struct
  ...
  let summary t =
    ...
    in
      sprintf
        "Queried %d exchanges (%d ok, %d failed) in %.0fms"
        (successful + failed)
        successful
        failed
        total_latency
...
module Query_config = struct
  ...
  let default =
    {exchanges= []; timeout= Time_ns.Span.of_sec 30.0; include_zero_balances= false}

  let with_gemini cfg t = {t with exchanges= Gemini cfg :: t.exchanges}
  let with_kraken cfg t = {t with exchanges= Kraken cfg :: t.exchanges}
  let with_binance cfg t = {t with exchanges= Binance cfg :: t.exchanges}
  let with_mexc cfg t = {t with exchanges= Mexc cfg :: t.exchanges}
  let with_coinbase cfg t = {t with exchanges= Coinbase cfg :: t.exchanges}
  let with_bitrue cfg t = {t with exchanges= Bitrue cfg :: t.exchanges}
end
...
let query_exchange (config : Query_config.exchange_config) : Exchange_result.t Deferred.t =
  ...
  let calc_latency () = Time_ns.Span.to_ms (Time_ns.diff (Time_ns.now ()) start) in
    match config with
    | Query_config.Gemini cfg ->
      (* Gemini requires a nonce pipe for authentication *)
      Gemini.Nonce.Counter.pipe ~init:1 ()
      >>= fun nonce ->
    ...
    | Query_config.Binance cfg ->
      let adapter = Binance.Fluxum_adapter.Adapter.create ~cfg () in
        Binance.Fluxum_adapter.Adapter.balances adapter
        >>| (function
         | Ok balances ->
           let normalized =
             List.filter_map balances ~f:(fun b ->
    ...
    | Query_config.Coinbase cfg ->
      let adapter = Coinbase.Fluxum_adapter.Adapter.create ~cfg () in
        Coinbase.Fluxum_adapter.Adapter.balances adapter
        >>| (function
         | Ok balances ->
           let normalized =
             List.filter_map balances ~f:(fun b ->
    ...
    | Query_config.Bitrue cfg ->
      let adapter = Bitrue.Fluxum_adapter.Adapter.create ~cfg () in
        Bitrue.Fluxum_adapter.Adapter.balances adapter
        >>| (function
         | Ok balances ->
           let normalized =
             List.filter_map balances ~f:(fun b ->
      ...
      List.iter non_zero ~f:(fun (currency, agg) ->
        ...
        List.iter agg.by_exchange ~f:(fun ex ->
          Buffer.add_string
            ...
            (sprintf
               "      %-10s: %.8f (avail: %.8f)\n"
               (Types.Venue.to_string ex.venue)
               ex.total
               ex.available)));
      Buffer.contents buf
