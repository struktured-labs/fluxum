...
let run ~market ~depth () =
  ...
  let%bind book_pipe = Dydx.Order_book.Book.pipe (module Cfg) ~symbol:market () in
  (* Track update count *)
...
let () =
  Command.async
    ~summary:"Stream dYdX v4 order book"
    (let%map_open.Command market =
       flag
         "--market"
         (optional_with_default "BTC-USD" string)
