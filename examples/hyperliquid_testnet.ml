...
let () =
  Command.async
    ~summary:"Hyperliquid testnet trading example"
    Command.Let_syntax.(
      let%map_open address =
        flag
          ...
          (optional string)
          ~doc:
            "ADDRESS Your Hyperliquid wallet address (or set HYPERLIQUID_TESTNET_ADDRESS)"
      and private_key =
        flag
          "--private-key"
          (optional string)
      ...
      in
        fun () ->
          ...
          let address =
            ...
            | None ->
              (match Sys.getenv "HYPERLIQUID_TESTNET_ADDRESS" with
               ...
               | None ->
                 failwith
                   "Address required: use --address or set HYPERLIQUID_TESTNET_ADDRESS")
          in
          let private_key =
            ...
            | None ->
              (match Sys.getenv "HYPERLIQUID_TESTNET_PRIVATE_KEY" with
               ...
               | None ->
                 failwith
                   "Private key required: use --private-key or set \
          ...
          in
            ...
            let adapter =
              Hyperliquid.Fluxum_adapter.Adapter.create ~cfg ~user:address ~private_key ()
            in
              printf "Step 1: Fetching account state...\n";
              let%bind account_result =
                Hyperliquid.Fluxum_adapter.Adapter.balances adapter
              in
                match account_result with
                | Error err ->
                  printf
                    "❌ Failed to fetch account: %s\n"
                ...
                | Ok states ->
                  ...
                  (match List.hd states with
                   ...
                   | Some state ->
                     ...
                     printf "\n";
                     (* Show positions *)
                     if List.is_empty state.assetPositions
                     then printf "No open positions\n\n"
                     else (
                       printf "Open Positions:\n";
                       List.iter state.assetPositions ~f:(fun ap ->
                         let pos = ap.position in
                           printf
                             "  • %s: %s contracts (PnL: %s USD)\n"
                             pos.coin
                             pos.szi
                             pos.unrealizedPnl);
                       printf "\n");
                       ...
                       (match meta_result with
                        ...
                        | Ok meta ->
                          ...
                          List.iteri meta.universe ~f:(fun idx item ->
                            printf
                              "  %d. %s (decimals: %d, max leverage: %s)\n"
                              idx
                              item.name
                              item.szDecimals
                              (match item.maxLeverage with
                          ...
                          let%bind mids_result = Hyperliquid.Rest.all_mids cfg in
                            (match mids_result with
                             | Error err ->
                               printf
                                 "❌ Failed to fetch prices: %s\n"
                                 (Sexp.to_string_hum
                                    (Hyperliquid.Rest.Error.sexp_of_t err));
                               return ()
                             | Ok mids ->
                               let btc_price =
                               ...
                               in
                                 (match btc_price with
                                  ...
                                  | Some price ->
                                    ...
                                    let limit_price = price_float *. 0.8 in
                                      (* 20% below market - won't fill *)
                                      printf
                                        "═══════════════════════════════════════════════════════════════\n";
                                      ...
                                      printf "Type:        LIMIT\n";
                                      printf
                                        "Size:        0.001 contracts (~$%s)\n"
                                        (Float.to_string (limit_price *. 0.001));
                                      printf
                                        "Limit Price: $%s (20%% below market)\n"
                                        (Float.to_string limit_price);
                                      printf "TIF:         GTC (Good-Til-Cancel)\n";
                                      printf "\n";
                                      printf
                                        "This order is intentionally priced far from \
                                         market\n";
                                      printf
                                        "so it will NOT execute - it's just a signing \
                                         test.\n";
                                      printf "\n";
                                      (* Create order request *)
                                      let order : Hyperliquid.Signing.order_request =
                                        { Hyperliquid.Signing.asset= 0
                                        ; (* BTC is index 0 *)
                                          is_buy= true
                                      ...
                                      in
                                        printf
                                          "Step 4: Placing test order with EIP-712 \
                                           signature...\n";
                                        let%bind order_result =
                                          Hyperliquid.Fluxum_adapter.Adapter.place_order
                                            adapter
                                            order
                                        in
                                          (match order_result with
                                           | Error err ->
                                             ...
                                             printf "\nThis is normal if:\n";
                                             printf
                                               "  - Your testnet account has no funds\n";
                                             printf
                                               "  - You haven't visited \
                                                https://app.hyperliquid-testnet.xyz/ to \
                                                get tokens\n";
                                             printf
                                               "  - The testnet is temporarily down\n";
                                             return ()
                                           | Ok response ->
                                             printf "\n✅ Order placed successfully!\n\n";
                                             (match response.status with
                                              | Hyperliquid.Rest.ExchangeResponse
                                                .Ok_status ->
                                                printf "Status: OK\n";
