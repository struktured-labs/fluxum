...
let validate_exchanges names = List.filter_map names ~f:parse_exchange_name
...
type book_update =
  | Gemini_update of
      [ `Ok of Gemini.Order_book.Book.t
...
let get_exchange_best_bid_ask (book : Consolidated_order_book.Book.t) exchange =
  ...
  | Hyperliquid ->
    (match book.hyperliquid_book with
     ...
     | Some ex_book ->
       let bids =
         Hyperliquid.Order_book.Book.bids_alist ex_book
         |> (fun list -> List.take list 1)
         |> List.map ~f:snd
       in
       let asks =
         Hyperliquid.Order_book.Book.asks_alist ex_book
         |> (fun list -> List.take list 1)
         |> List.map ~f:snd
       in
         (match (bids, asks) with
  ...
  | Bitrue ->
    (match book.bitrue_book with
     ...
     | Some ex_book ->
       let bids =
         Bitrue.Order_book.Book.bids_alist ex_book
         |> (fun list -> List.take list 1)
         |> List.map ~f:snd
       in
       let asks =
         Bitrue.Order_book.Book.asks_alist ex_book
         |> (fun list -> List.take list 1)
         |> List.map ~f:snd
       in
         (match (bids, asks) with
  ...
  | Binance ->
    (match book.binance_book with
     ...
     | Some ex_book ->
       let bids =
         Binance.Order_book.Book.bids_alist ex_book
         |> (fun list -> List.take list 1)
         |> List.map ~f:snd
       in
       let asks =
         Binance.Order_book.Book.asks_alist ex_book
         |> (fun list -> List.take list 1)
         |> List.map ~f:snd
       in
         (match (bids, asks) with
  ...
  | Coinbase ->
    (match book.coinbase_book with
     ...
     | Some ex_book ->
       let bids =
         Coinbase.Order_book.Book.bids_alist ex_book
         |> (fun list -> List.take list 1)
         |> List.map ~f:snd
       in
       let asks =
         Coinbase.Order_book.Book.asks_alist ex_book
         |> (fun list -> List.take list 1)
         |> List.map ~f:snd
       in
         (match (bids, asks) with
...
let record_arbitrage_history (opp : arb_opportunity) =
  let key = (opp.buy_exchange, opp.sell_exchange) in
    ...
    | Some history ->
      ...
      let time_diff =
        Time_float_unix.diff opp.last_seen history.best_opportunity.last_seen
      ...
      in
        ...
        (match Float.(opp.net_profit_pct > history.best_opportunity.net_profit_pct) with
         ...
         | false ->
           ...
           <- {history.best_opportunity with last_seen= opp.last_seen})
    | None ->
      (* New arbitrage pair - create history *)
      Hashtbl.set
        arb_history_table
        ~key
        ~data:{best_opportunity= opp; occurrences= 1; total_duration_sec= 0.0}

(** Detect arbitrage opportunities across all exchange pairs *)
let detect_arbitrage ?(fees = default_fees) (book : Consolidated_order_book.Book.t)
  : arb_opportunity list
  =
  ...
  let now = Time_float_unix.now () in
    ...
    List.iter exchanges ~f:(fun buy_ex ->
      List.iter exchanges ~f:(fun sell_ex ->
        ...
        | false ->
          (match
             ( get_exchange_best_bid_ask book buy_ex
             , get_exchange_best_bid_ask book sell_ex )
           with
...
let pretty_print_with_arb ?(max_depth = 10) (book : Consolidated_order_book.Book.t) () =
  ...
  let arb_bg = "\027[43m\027[30m" in
  (* Yellow background, black text *)
  let arb_highlight = "\027[1m\027[33m" in
    (* Bold yellow *)
    ...
    in
      ...
      (match List.is_empty arb_opps with
       ...
       | false ->
         printf
           ...
           reset;
         printf
           "%s%s║  🚨 CURRENT ARBITRAGE OPPORTUNITIES 🚨                                 ║%s\n"
           bold
           arb_bg
           reset;
         printf
         ...
         List.iteri arb_opps ~f:(fun i opp ->
           ...
           in
             printf
               "%s[CURRENT #%d]%s Buy on %s%-9s%s @ %s$%.2f%s → Sell on %s%-9s%s @ \
                %s$%.2f%s\n"
               arb_highlight
               (i + 1)
               reset
             ...
             printf
               ...
               opp.buy_fee_pct
               opp.sell_fee_pct
               (opp.buy_fee_pct +. opp.sell_fee_pct);
             printf
               "       %sNet Profit: $%.2f (%.3f%%)%s"
               profit_color
               opp.net_profit
             ...
             printf "\n";
             printf
               "       Buy volume: %.4f BTC | Sell volume: %.4f BTC | Max size: %.4f BTC\n\n"
               opp.buy_volume
               opp.sell_volume
      ...
      (match List.is_empty historical_arbs with
       ...
       | false ->
         ...
         printf
           "%s%s║  ⭐ HISTORICAL BEST ARBITRAGE OPPORTUNITIES (RETAINED FOREVER) ⭐      ║%s\n"
           bold
           "\027[45m\027[37m"
           reset;
         printf
         ...
         List.iteri historical_arbs ~f:(fun i history ->
           ...
           in
             printf
               "%s[BEST #%d]%s Buy on %s%-9s%s @ %s$%.2f%s → Sell on %s%-9s%s @ %s$%.2f%s\n"
               "\027[35m"
               (i + 1)
               reset (* Magenta for historical *)
               cyan
               (exchange_to_string opp.buy_exchange)
               reset
               green
               ...
               reset;
             printf
               "       %sPEAK Net Profit: $%.2f (%.3f%%)%s | Occurrences: %d | Last \
                seen: %s\n"
               profit_color
               opp.net_profit
               opp.net_profit_pct
               ...
               time_str;
             printf
               "       Buy volume: %.4f BTC | Sell volume: %.4f BTC\n"
               opp.buy_volume
               opp.sell_volume;
             (* Show if this opportunity is currently active *)
             let is_current =
               List.exists arb_opps ~f:(fun curr ->
         ...
         printf "%s%s%s\n\n" reset (String.make 75 '-') reset;
         printf
           "%sTotal unique profitable arbitrage pairs ever seen: %d%s\n\n"
           bold
           (Hashtbl.length arb_history_table)
           reset);
      ...
      printf "Sources: ";
      (match Option.is_some book.gemini_book with
       | true -> printf "[Gemini] "
       | false -> ());
      (match Option.is_some book.kraken_book with
       | true -> printf "[Kraken] "
       | false -> ());
      (match Option.is_some book.hyperliquid_book with
       | true -> printf "[Hyperliquid] "
       | false -> ());
      (match Option.is_some book.bitrue_book with
       | true -> printf "[Bitrue] "
       | false -> ());
      printf "\n\n";
      (* Print asks (highest to lowest for display) *)
      let asks =
        Consolidated_order_book.Book.best_n_asks book ~n:max_depth () |> List.rev
      in
        ...
        List.iter asks ~f:(fun level ->
          printf
            "  %s%-4s%s  %.8f @ %.2f\n"
            blue
            (Consolidated_order_book.Book.exchange_to_string
               level.Consolidated_order_book.Attributed_level.exchange)
            reset
            level.volume
            level.price);
        (* Print spread *)
  ...
  in
    ...
    let%bind gemini_pipe =
      ...
      | true ->
        ...
        let%bind pipe =
          Gemini.Order_book.Book.pipe (module Gemini_cfg) ~symbol:`Btcusd ()
        in
          return (Pipe.map pipe ~f:(fun result -> Gemini_update result))
      | false -> return (create_closed_pipe ())
    in
    ...
    let%bind bitrue_pipe_result =
      ...
      | false -> return (Ok (create_closed_pipe ()))
    in
    let bitrue_pipe =
      match bitrue_pipe_result with
              ...
              (match book_result with
               | `Ok gemini_book ->
                 (match !gemini_first_update with
                  ...
                  | false -> ());
                 consolidated
                 := Consolidated_order_book.Book.update_gemini !consolidated gemini_book;
                 return ()
              ...
              (match book_result with
               ...
               | Ok kraken_book ->
                 (match !kraken_first_update with
                  ...
                  | false -> ());
                 consolidated
                 := Consolidated_order_book.Book.update_kraken !consolidated kraken_book;
                 return ())
              ...
              (match book_result with
               ...
               | Ok hyperliquid_book ->
                 (match !hyperliquid_first_update with
                  ...
                  | false -> ());
                 consolidated
                 := Consolidated_order_book.Book.update_hyperliquid
                      !consolidated
                      hyperliquid_book;
                 return ())
              ...
              (match book_result with
               ...
               | Ok bitrue_book ->
                 (match !bitrue_first_update with
                  ...
                  | false -> ());
                 consolidated
                 := Consolidated_order_book.Book.update_bitrue !consolidated bitrue_book;
                 return ())
              ...
              (match book_result with
               ...
               | Ok binance_book ->
                 (match !binance_first_update with
                  ...
                  | false -> ());
                 consolidated
                 := Consolidated_order_book.Book.update_binance !consolidated binance_book;
                 return ())
              ...
              (match book_result with
               ...
               | Ok coinbase_book ->
                 (match !coinbase_first_update with
                  ...
                  | false -> ());
                 consolidated
                 := Consolidated_order_book.Book.update_coinbase
                      !consolidated
                      coinbase_book;
                 return ())
          in
  ...
  Command.async_spec
    ...
    Command.Spec.(
      ...
      +> flag
           ...
           (listed string)
           ~doc:
             "EXCHANGE Enable exchange \
              (gemini|kraken|hyperliquid|bitrue|binance|coinbase). Can be repeated. \
              Short form: -e. Default: all"
      +> flag "-e" (listed string) ~doc:"" (* Hidden alias for --exchanges *)
      +> flag
           "--depth"
